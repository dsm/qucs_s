...
vFile::vFile() {
  ...
  // wave
  Arcs.append(new qucs::Arc(-3, -7, 7, 7, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2, Qt::SolidLine, Qt::FlatCap)));
  Arcs.append(new qucs::Arc(-3, 0, 7, 7, 16 * 90, 16 * 180,
                            QPen(Qt::darkBlue, 2, Qt::SolidLine, Qt::FlatCap)));
  // pins
...
QString vFile::spice_netlist(
  ...
  if (getProperty("Interpolator")->Value != "linear") {
    step = "true";
  }
  s += QStringLiteral(
           ".MODEL %1 filesource (file=\"%2\" amplscale=[%3] amplstep=%4 "
