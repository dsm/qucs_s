...
inline constexpr double distance(PointLike auto&& point_a,
...
constexpr bool is_near_line(P1&& point_m, P2&& point_a, P3&& point_b,
                            int dist) {
  using ::internal::get_x;
  using ::internal::get_y;
  ...
  if (gt_lt(get_x(point_m), std::minmax(ax, bx)) &&
      gt_lt(get_y(point_m), std::minmax(ay, by))) {
...
std::vector<P> on_line(P first, P last, It begin, It end) {
  ...
  std::copy_if(
      begin, end, std::back_inserter(in_between),
