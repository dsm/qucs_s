...
tunerElement::tunerElement(QWidget* parent, Component* component, Property* pp,
  ...
  // **************************************
  // First of all we need to check whether the property is tunable (number +
  // units) or not (random string) There may be an arbitrary number of spaces
  // between magnitude and unit, so in first place we need to normalize that In
  // this sense, all blank spaces are removed
  ...
  if (units_index != -1) {
    ...
    unit = val.mid(
        units_index); // unit should be sth like mA, mm, uV, nA, etc... here
    SeparateMagnitudeFromSuffix(unit, magnitudeIndex);
  } else { // The property value contains only numbers
    numValue = val.toFloat();
    has_unit = false;
  }
  if ((unit.length() <= 1) &&
      (!pp->Description.isEmpty())) { // It comes with no units... so let's try
                                      // to find a suitable unit
    QMap<QString, QStringList> Keywords; // Map containing the keywords that may
                                         // appear in the property description
    ...
    // If one of the keywords appears in the description text, then the key of
    // the map is used as unit...
    bool found = false;
    for (auto e : Keywords.keys()) {
      for (int i = 0; i < Keywords[e].length(); i++) {
        if (pp->Description.indexOf(Keywords[e].at(i), Qt::CaseInsensitive) !=
            -1) {
          unit  = e;
          found = true;
          if ((i != 3) && (i != 4)) { // Not a voltage nor a current
            minValueValidator =
                0; // Allow positive numbers only in the lineedits
          }
          break;
        }
      }
    ...
    has_unit = found;
  } else { // The value comes with unit
    if (magnitudeIndex != 5) {
      unit = unit.mid(1);
    }
    has_unit = true;
  }
  // Finally, add the unit to the QStringList for adding it to the scale
  // comboboxes later
  if (has_unit) {
    for (int i = 0; i < ScaleFactorList.length(); i++) {
      ScaleFactorList[i] += unit;
    }
  }
  ...
  // UI setup
  setAttribute(Qt::WA_DeleteOnClose); // This attribute forces the widget to be
  ...
  auto* maximumValidator =
      new QDoubleValidator(minValueValidator, PTRDIFF_MAX, 2, this);
  maximumValidator->setLocale(cDoubleLocale);
  maximum->setValidator(maximumValidator); // Prevent the user from entering
                                           // text
  ...
  auto* minumumValidator =
      new QDoubleValidator(minValueValidator, 100, 2, this);
  minumumValidator->setLocale(cDoubleLocale);
  minimum->setValidator(minumumValidator); // Prevent the user from entering
                                           // text
  MinUnitsCombobox = new QComboBox(this);
  MinUnitsCombobox->setSizeAdjustPolicy(
  ...
  stepValue = (maxValue - minValue) / 20; // 20 steps between minimum and
                                          // maximum
  ...
  connect(maximum, SIGNAL(editingFinished()), this,
          SLOT(slotMaxValueChanged()));
  connect(MaxUnitsCombobox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotMaxValueChanged()));

  connect(minimum, SIGNAL(editingFinished()), this,
          SLOT(slotMinValueChanged()));
  connect(MinUnitsCombobox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotMinValueChanged()));

  connect(step, SIGNAL(editingFinished()), this, SLOT(slotStepChanged()));
  connect(StepUnitsCombobox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotStepChanged()));
  ...
  connect(ValueUnitsCombobox, SIGNAL(currentIndexChanged(int)), this,
  ...
  // So far, minimum, maximum, stepValue and value has the same scale. Let's set
  // the right value
  bool aux;
...
/*
Given a QString containing number + suffix (i.e. 2.5p, 3.9n), this function
separates '2.5' from 'p' and gives the corresponding index to that suffix
*/
QString SeparateMagnitudeFromSuffix(QString num, int& index) {
  ...
  if (sp == -1) {
    return num; // It doesn't come with a suffix
  }
...
/*
 * The control reaches this function when either the maximum value QLineedit or
 * its corresponding combobox are edited
 */
void tunerElement::slotMaxValueChanged() {
  ...
  if (!ok || (v <= minValue)) {
    QMessageBox::warning(this, "ERROR", "Maximum value not correct",
                         QMessageBox::Ok);
    // Restore values
  ...
  if (v < numValue) {
    maxValue = numValue;
    // Update text... since maxValue was set to numValue and differs from what
    // the user entered
    QString val = misc::num2str(maxValue);
    int index   = 5; // By default, no scaling
    val         = SeparateMagnitudeFromSuffix(val, index);
...
/*
 * The control reaches this function when either the minimum value QLineedit or
 * its corresponding combobox are edited
 */
void tunerElement::slotMinValueChanged() {
  ...
  if (!ok || (v >= maxValue)) {
    QMessageBox::warning(this, "ERROR", "Minimum value not correct",
                         QMessageBox::Ok);
    // Restore values
  ...
  if (v > numValue) {
    minValue = numValue;
    // Update text... since minValue was set to numValue and differs from what
    // the user entered
    QString val = misc::num2str(minValue);
    int index   = 5; // By default, no scaling
    val         = SeparateMagnitudeFromSuffix(val, index);
...
/*
 * The control reaches this function when either the step QLineedit or its
 * corresponding combobox are edited
 */
void tunerElement::slotStepChanged() {
  ...
  if (!ok) {
    QMessageBox::warning(this, tr("ERROR"), tr("Entered step is not correct"),
                         QMessageBox::Ok);
...
/*
 * The control reaches this function when the slider position (i.e. the value of
...
void tunerElement::slotSliderChanged() {
  ...
  float slider_v =
      minValue + ((slider->value() / 100.0) * (maxValue - minValue));
  ...
  numValue = slider_v;
  // Update text & combobox. This is specially needed in case the input value
  // exceeds the minumum and max. limits
...
/*
 * The variable can be modified by moving the slider, editing the corresponding
 * QLineedit box or changing the scale factor. The control reaches this function
 * when one of the events above is triggered. It checks if the input value is
 * correct, updates it and finally runs the simulation
 */
void tunerElement::slotValueChanged(bool simulate) {
  ...
  ValueUnitsCombobox->blockSignals(true);
  if (!ok || (v < 0)) {
    QMessageBox::warning(this, tr("ERROR"), tr("Value not correct"),
                         QMessageBox::Ok);
    // Restore values
    QString val = misc::num2str(numValue);
    int index   = 5; // By default, no scaling
  ...
  if (v > maxValue) {
    numValue = maxValue, updateText = true;
  }
  if (v < minValue) {
    numValue = minValue, updateText = true;
  }

  if (updateText) {
    // Update text... because of the lines above numValue may be set to maxValue
    // or minValue so it may differ from what the user entered
    QString val = misc::num2str(numValue);
    int index   = 5; // By default, no scaling
    val         = SeparateMagnitudeFromSuffix(val, index);
    value->setText(val);
...
void tunerElement::slotUpClicked() {
  ...
  if (numValue > maxValue) {
    numValue = getMaxValue(ok);
  }

  value->setText(QString::number(
      numValue / getScale(ValueUnitsCombobox->currentIndex()), 'f', 3));

  updateSlider(); // First, update the slider. Then update the value since this
                  // slot will take the value from the slider
  slotValueChanged();
}
...
void tunerElement::slotDownClicked() {
  ...
  numValue = getValue(ok) - s;
  if (numValue < getMinValue(ok)) {
    numValue = getMinValue(ok);
  }
...
/*
The control reaches this function whenever the user changes the value-related
inputs, i.e. textbox, scale combo or the slider For this reason, some of the
values given by the user may be out of range and there's a need to check that
...
// Given the current index of a combobox, this function returns the scale factor
// according to the selection. Notice that this function does not belong neither
// tunerElement nor tunerDialog
float getScale(int index) {
  switch (index) {
  case 0:
    return pow(10, -15); // femto
  case 1:
    return pow(10, -12); // pico
  case 2:
    return pow(10, -9); // nano
  case 3:
    return pow(10, -6); // micro
  case 4:
    return pow(10, -3); // mili
  case 5:
    return 1;
  case 6:
    return pow(10, 3); // kilo
  case 7:
    return pow(10, 6); // mega
  case 8:
    return pow(10, 9); // giga
  }
  return 0;
}
...
float tunerElement::getValue(bool& ok) {
  return value->text().toFloat(&ok) *
         getScale(ValueUnitsCombobox->currentIndex());
}

// Reads the maximum value from the user interface
float tunerElement::getMaxValue(bool& ok) {
  return maximum->text().toFloat(&ok) *
         getScale(MaxUnitsCombobox->currentIndex());
}

// Reads the minimum value from the user interface
float tunerElement::getMinValue(bool& ok) {
  return minimum->text().toFloat(&ok) *
         getScale(MinUnitsCombobox->currentIndex());
}

// Reads the step from the user interface
float tunerElement::getStep(bool& ok) {
  return step->text().toFloat(&ok) *
         getScale(StepUnitsCombobox->currentIndex());
}
...
TunerDialog::TunerDialog(QWidget* _w, QWidget* parent)
  ...
  updateValues->setEnabled(
      false); // It doesn't make sense to activate it at first... only when at
              // least a tuning element is active...
  resetValues = new QPushButton(tr("Reset Values"), this);
  resetValues->setEnabled(false);

  // When pressing the Enter key at the Maxium lineedit focus is automatically
  // set to the reset button leading to confunsing and unexpected behavior. In
  // this sense, the lines below are a workaround. The focus is rejected for
  ...
  // Management of the Esc shortcut. Otherwise, it will exit the tuner and leave
  // the toogle button activated
...
void TunerDialog::addTunerElement(tunerElement* element) {
  ...
  connect(element, SIGNAL(elementValueUpdated()), this,
          SLOT(slotElementValueUpdated()));
  connect(element, SIGNAL(removeElement(tunerElement*)), this,
          SLOT(slotRemoveTunerElement(tunerElement*)));
  ...
  info->clearMessage(); // The user has already selected a component. It makes
                        // no longer sense to display the help message
}
...
void TunerDialog::slotRemoveTunerElement(tunerElement* e) {
  ...
  currentElements.removeAll(
      e); // This will also destroy the element in QSplitter:
          // https://stackoverflow.com/questions/371599/how-to-remove-qwidgets-from-qsplitter
  delete e;
  if (currentProps.size() == 0) { // No tuning boxes
...
bool TunerDialog::checkChanges() {
  for (int i = 0; i < currentElements.count(); i++) {
    int index   = -1;
    QString val = SeparateMagnitudeFromSuffix(
        currentElements.at(i)->originalValue, index);
    float initial_val = val.toFloat() * getScale(index);
    if (initial_val != currentElements.at(i)->numValue) {
      return true;
    }
  }
  return false;
...
void TunerDialog::closeEvent(QCloseEvent* event) {
  ...
  if (!valuesUpdated && checkChanges()) {
    msg = QMessageBox::question(
        this, "Update values before closing?",
