...
DiagramDialog::DiagramDialog(Diagram* d, QWidget* parent, Graph* currentGraph)
  ...
  if (Diag->Name == "Tab") {
    ...
    connect(PropertyBox, SIGNAL(activated(int)), SLOT(slotSetGraphStyle(int)));
    Box2Layout->setStretchFactor(new QWidget(Box2),
                                 5); // stretchable placeholder
  ...
  connect(ChooseData, SIGNAL(activated(int)),
          SLOT(slotReadVarsAndSetSimulator(int)));
  // connect(ChooseData,
  ...
  connect(ChooseSimulator, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotReadVars(int)));
  ...
  connect(ChooseVars, SIGNAL(itemDoubleClicked(QTableWidgetItem*)),
          SLOT(slotTakeVar(QTableWidgetItem*)));
  ...
  connect(GraphList, SIGNAL(itemClicked(QListWidgetItem*)),
          SLOT(slotSelectGraph(QListWidgetItem*)));
  connect(GraphList, SIGNAL(itemDoubleClicked(QListWidgetItem*)),
  ...
  if (Diag->Name.at(0) != 'T') { // not tabular or timing diagram
    ...
    if ((Diag->Name.left(4) == "Rect") || (Diag->Name == "Curve")) {
      ...
      connect(GridLogY, SIGNAL(toggled(bool)), LogUnitsY,
              SLOT(setEnabled(bool)));
      connect(LogUnitsY, SIGNAL(currentIndexChanged(int)), this,
              SLOT(slotRecalcDbLimitsY()));
      ...
      connect(GridLogZ, SIGNAL(toggled(bool)), LogUnitsZ,
              SLOT(setEnabled(bool)));
      connect(LogUnitsZ, SIGNAL(currentIndexChanged(int)), this,
              SLOT(slotRecalcDbLimitsZ()));
  ...
  if (ColorButt) {
    if (!currentGraph) {
      QColor selectedColor(
          DefaultColors[GraphList->count() % NumDefaultColors]);
