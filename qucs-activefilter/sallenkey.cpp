...
SallenKey::SallenKey(Filter::FilterFunc ffunc_, Filter::FType type_,
                     FilterParam par)
    : Filter(ffunc_, type_, par) {
  if ((ftype == Filter::LowPass) || (ftype == Filter::BandPass)) {
    Nr1 = 4;
  }
...
void SallenKey::calcLowPass() {
  ...
  for (int k = 1; k <= order / 2; k++) {
    ...
    R1 = 2 / (Wc * (B * C2 + sqrt((B * B + 4 * C * (Kv1 - 1)) * C2 * C2 -
                                  4 * C * C1 * C2)));
...
void SallenKey::calcBandPass() {
  ...
  if (order == 1) { // Filter contains only 1 1st-order section
    ...
    R1 = 2.0 / (rho * W0 * C1);
    R2 = 2.0 /
         ((-beta + sqrt((rho - beta) * (rho - beta) + 8.0 * gamma)) * W0 * C1);
  ...
  for (int k = 1; k <= order / 2; k++) { // Usually 2nd-order section
    ...
    double H = C + 4.0 * Q * Q;
    double E =
        (1.0 / B) * sqrt(0.5 * (H + sqrt(H * H - (4.0 * B * B * Q * Q))));
    double F = (B * E) / Q;
...
void SallenKey::createHighPassSchematic(QString& s) {
  ...
  s += ">\n";
  s += "<Components>\n";
  s += QStringLiteral("<.AC AC1 1 30 410 0 61 0 0 \"lin\" 1 \"1 Hz\" 1 \"%1 "
                      "kHz\" 1 \"501\" 1 \"no\" 0>\n")
           .arg((10.0 * Fc) / 1000.0);
  s += "<.DC DC1 1 280 410 0 61 0 0 \"26.85\" 0 \"0.001\" 0 \"1 pA\" 0 \"1 "
       "uV\" 0 \"no\" 0 \"150\" 0 \"no\" 0 \"none\" 0 \"CroutLU\" 0>\n";
  s += "<Eqn Eqn1 1 270 540 -30 14 0 0 \"K=(out.v/in.v)\" 1 \"yes\" 0>\n";
  s += QStringLiteral("<Vac V1 1 %1 260 18 -26 0 1 \"1 V\" 1 \"1 kHz\" 0 \"0\" "
                      "0 \"0\" 0>\n")
           .arg(20 + dx);
  ...
  for (int i = 1; i <= N2ord; i++) {
    ...
    s += QStringLiteral(
             "<OpAmp OP%1 1 %2 160 -26 42 0 0 \"1e6\" 1 \"15 V\" 0>\n")
             .arg(stage.N)
             .arg(370 + dx);
    ...
    s += QStringLiteral(
             ...
             .arg(suffix2);
    s += QStringLiteral(
             "<C C%1 1 %2 190 -26 17 0 0 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
             .arg(1 + (i - 1) * N_C)
             .arg(100 + dx)
             .arg(C1, 0, 'f', 3)
             .arg(suffix1);
    s += QStringLiteral("<R R%1 1 %2 240 -75 -26 1 1 \"%3k\" 1 \"26.85\" 0 "
             ...
             .arg(270 + dx)
             .arg(stage.R2, 0, 'f', 3);
    s += QStringLiteral("<R R%1 1 %2 70 -26 15 0 0 \"%3k\" 1 \"26.85\" 0 "
                        "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
             .arg(1 + (i - 1) * N_R)
             .arg(330 + dx)
             .arg(stage.R1, 0, 'f', 3);
...
void SallenKey::createLowPassSchematic(QString& s) {
  ...
  for (int i = 1; i <= N2ord; i++) {
    ...
    s += QStringLiteral("<GND * 1 %1 370 0 0 0 0>\n").arg(320 + dx);
    s += QStringLiteral(
             "<C C%1 1 %2 70 -26 15 0 0 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
             .arg(2 + (i - 1) * N_C)
             .arg(330 + dx)
             .arg(C2, 0, 'f', 3)
             .arg(suffix2);
    s += QStringLiteral(
             "<C C%1 1 %2 240 -75 -26 1 1 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
             .arg(1 + (i - 1) * N_C)
             .arg(270 + dx)
             .arg(C1, 0, 'f', 3)
             .arg(suffix1);
    s += QStringLiteral("<R R%1 1 %2 190 -26 -45 1 0 \"%3k\" 1 \"26.85\" 0 "
                        "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
             .arg(2 + (i - 1) * N_R)
             .arg(200 + dx)
             .arg(stage.R2, 0, 'f', 3);
    s += QStringLiteral("<R R%1 1 %2 190 -26 17 0 0 \"%3k\" 1 \"26.85\" 0 "
                        "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
             .arg(1 + (i - 1) * N_R)
             .arg(100 + dx)
             .arg(stage.R1, 0, 'f', 3);
...
void SallenKey::createBandPassSchematic(QString& s) {
  ...
  for (int i = 1; i <= Sections.count(); i++) {
    ...
    s += QStringLiteral(
             "<C C%1 1 %2 280 17 -26 0 1 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
             .arg(1 + (i - 1) * Nc1)
             .arg(150 + dx)
             .arg(C1, 0, 'f', 3)
             .arg(suffix1);
