...
DIODE_SPICE::DIODE_SPICE() {
  Description =
      QObject::tr("SPICE D:\nMultiple line ngspice or Xyce D model "
...
Element* DIODE_SPICE::info_DIODE3(QString& Name, char*& BitmapFile,
                                  bool getNewOne) {
...
QString DIODE_SPICE::spice_netlist(
  ...
  if (D.length() > 0) {
    s += QStringLiteral("%1").arg(D);
  }
  if (D_Line_2.length() > 0) {
    s += QStringLiteral("\n%1").arg(D_Line_2);
  }
  if (D_Line_3.length() > 0) {
    s += QStringLiteral("\n%1").arg(D_Line_3);
  }
  if (D_Line_4.length() > 0) {
    s += QStringLiteral("\n%1").arg(D_Line_4);
  }
  if (D_Line_5.length() > 0) {
    s += QStringLiteral("\n%1").arg(D_Line_5);
  }
