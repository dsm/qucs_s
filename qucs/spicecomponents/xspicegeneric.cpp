...
XspiceGeneric::XspiceGeneric() {
  ...
  Props.append(new Property("Model", "generic_model", false,
                            QObject::tr(".MODEL definition reference")));
...
void XspiceGeneric::createSymbol() {
  ...
  for (QString t_port : t_ports) {
    ...
    if ((t_port.remove(' ').endsWith('d')) && (t_port != "d")) {
...
QString XspiceGeneric::spice_netlist(
