...
SP_Sim::SP_Sim() {
  ...
  Props.append(new Property(
      "saveCVs", "no", false,
      QObject::tr("put characteristic values into dataset") + " [yes, no]"));
  Props.append(new Property(
      "saveAll", "no", false,
...
QStringList SP_Sim::getNgspiceExtraVariables() {
  QStringList vars;
  bool donoise = false;
  if (getProperty("Noise")->Value == "yes") {
    donoise = true;
  }
...
QString SP_Sim::getSweepString() {
  ...
  QString unit;
  if (Props.at(0)->Value ==
      "log") { // convert points number for spice compatibility
  ...
  QString fstart =
      spicecompat::normalize_value(Props.at(1)->Value); // Start freq.
  QString fstop =
      spicecompat::normalize_value(Props.at(2)->Value); // Stop freq.
...
QString SP_Sim::xyce_netlist() {
  ...
  for (int i = 0; i < ports_num; i++) {
      for (int j = 0; j < ports_num; j++) {
          s += QStringLiteral(" sdb(%1,%2) s(%1,%2) sp(%1,%2) y(%1,%2) z(%1,%2)
  ").arg(i+1).arg(j+1);
      }
  }
  s += "\n";*/
