...
AbstractSpiceKernel::AbstractSpiceKernel(Schematic* schematic, QObject* parent)
    : QObject(parent), a_workdir(), a_simulator_cmd(), a_simulator_parameters(),
      a_output(), a_simProcess(new QProcess(this)), a_console(nullptr),
      a_sims(), a_vars(), a_output_files(),
      a_DC_OP_only(schematic->getShowBias() == 0 ? true : false),
      a_needsPrefix(false), a_schematic(schematic), a_parseFourTHD(false),
  ...
  connect(a_simProcess, SIGNAL(errorOccurred(QProcess::ProcessError)), this,
          SLOT(slotErrors(QProcess::ProcessError)));
...
bool AbstractSpiceKernel::prepareSpiceNetlist(QTextStream& stream,
  ...
  if (a_schematic->prepareNetlist(stream, collect, err) ==
      -10) { // Broken netlist
  ...
  delete err;
  if (isSubckt) {
    a_schematic->clearSignals();
  } else {
    a_schematic->clearSignalsAndFileList(); // for proper build of subckts
  }
...
bool AbstractSpiceKernel::checkSchematic(QStringList& incompat) {
  ...
  for (Component* pc : a_schematic->a_DocComps) {
    if ((!pc->isEquation) && !(pc->isProbe)) {
      if (pc->SpiceModel.isEmpty() && pc->isActive) {
...
/*!
 * \brief AbstractSpiceKernel::checkGround Check if schematic contain at least
 * one ground. \return True if ground found, false otherwise
 */
bool AbstractSpiceKernel::checkGround() {
  return std::ranges::any_of(a_schematic->a_DocComps,
...
bool AbstractSpiceKernel::checkDCSimulation() {
  ...
  // bool r = false;
  // for(Component *pc = a_schematic->a_DocComps.first(); pc != 0; pc =
  // a_schematic->a_DocComps.next()) {
 ...
 *        and .OPTIONS sections to netlist. These sections are placed on
 * schematic directly or converted form Equation components. Then outputs common
 *        components to netlist.
 * \param stream QTextStream that associated with spice netlist file
 * \param xyce Default is false. Should be set in true if netlist is
 *        prepared for Xyce simulator. For Ngspice should be false.
 */
void AbstractSpiceKernel::startNetlist(QTextStream& stream,
                                       spicecompat::SpiceDialect dialect) {
  QString s;

  // User-defined functions
  for (Component* pc : a_schematic->a_DocComps) {
    if ((pc->SpiceModel == ".FUNC") || (pc->SpiceModel == "INCLSCR")) {
      s = pc->getExpression();
      stream << s;
    }
  }
  ...
  // Components
  for (Component* pc : a_schematic->a_DocComps) {
    if (a_schematic->getIsAnalog() && !(pc->isSimulation) &&
        !(pc->isEquation)) {
...
/*!
 * \brief AbstractSpiceKernel::createNetlist Writes netlist in stream
 * TextStream. This is overloaded method. Should be reimplemted for Ngspice and
 * Xyce.
 */
...
void AbstractSpiceKernel::createSubNetlist(QTextStream& stream, bool lib) {
  ...
  if (lib) {
    header += " gnd "; // Ground node forwarding for Library
  }
  ...
  const spicecompat::SpiceDialect dialect(QucsSettings.DefaultSimulator ==
                                                  spicecompat::simXyce
                                              ? spicecompat::SPICEXyce
                                              : spicecompat::SPICEDefault);
...
/*!
 * \brief AbstractSpiceKernel::parseNgSpiceSimOutput This method parses text raw
 * spice output. Extracts a simulation points array and variables names and
 * types (Real or Complex) from output. \param ngspice_file Spice output file
 * name \param sim_points 2D array in which simulation points should be
 * extracted \param var_list This list is filled by simulation variables. There
 * is a list of dependent and independent variables. An independent variable is
 * the first in list. \param isComplex Type of variables. True if complex. False
 * if real.
 */
void AbstractSpiceKernel::parseNgSpiceSimOutput(
    QString ngspice_file, QList<QList<double>>& sim_points,
    QStringList& var_list, bool& isComplex, QStringList& extra_vars,
  ...
  while (!ngsp_data.atEnd()) { // Parse header;
    ...
    if (lin.contains("Flags") &&
        lin.contains("complex")) { // output consists of
    ...
    if (lin == "Variables:") {
      var_list.clear();
      QString indep_var =
          ngsp_data.readLine().section(sep, 1, 1, QString::SectionSkipEmpty);
...
/*!
 * \brief AbstractSpiceKernel::parseHBOutput Parse Xyce Harmonic balance (HB)
 * simulation output. \param ngspice_file Spice output file name \param
 * sim_points 2D array in which simulation points should be extracted \param
 * var_list This list is filled by simulation variables. There is a list of
 * dependent variables. Independent hbfrequency variable is always the first in
 * this list. \param hasParSweep[out] Set to true if dataset contains parameter
 * sweep output.
 */
...
/*!
 * \brief AbstractSpiceKernel::parseFourierOutput Parse output of fourier
 * simulation. \param ngspice_file[in] Spice output file name \param
 * sim_points[out] 2D array in which simulation points should be extracted
 * \param var_list[out] This list is filled by simulation variables. There is a
 * list of dependent and independent variables. An independent variable is the
 * first in list.
 */
void AbstractSpiceKernel::parseFourierOutput(QString ngspice_file,
                                             QList<QList<double>>& sim_points,
                                             QStringList& var_list) {
  QFile ofile(ngspice_file);
  if (ofile.open(QFile::ReadOnly)) {
    ...
    while (!ngsp_data.atEnd()) {
      ...
      if (lin.contains("No. Harmonics:")) {
        ...
        Nharm = ss.toInt();
        while (!ngsp_data.readLine().contains(
            QRegularExpression("Harmonic\\s+Frequency")))
          ;
        ...
        for (int i = 0; i < Nharm; i++) {
          ...
          if (!firstgroup) {
            sim_point.clear();
            sim_point.append(lin.section(sep, 1, 1, QString::SectionSkipEmpty)
                                 .toDouble()); // freq
            sim_point.append(lin.section(sep, 2, 2, QString::SectionSkipEmpty)
                                 .toDouble()); // magnitude
            sim_point.append(lin.section(sep, 3, 3, QString::SectionSkipEmpty)
                                 .toDouble()); // phase
            sim_point.append(lin.section(sep, 4, 4, QString::SectionSkipEmpty)
                                 .toDouble()); // normalized magnitude
            sim_point.append(lin.section(sep, 5, 5, QString::SectionSkipEmpty)
                                 .toDouble()); // normalized phase
            sim_points.append(sim_point);
          } else {
            sim_points[i].append(
                lin.section(sep, 2, 2, QString::SectionSkipEmpty)
                    .toDouble()); // magnitude
            sim_points[i].append(
                lin.section(sep, 3, 3, QString::SectionSkipEmpty)
                    .toDouble()); // phase
            sim_points[i].append(
                lin.section(sep, 4, 4, QString::SectionSkipEmpty)
                    .toDouble()); // normalized magnitude
            sim_points[i].append(
                lin.section(sep, 5, 5, QString::SectionSkipEmpty)
                    .toDouble()); // normalized phase
          }
        }
        firstgroup = true;
...
/*!
 * \brief AbstractSpiceKernel::parseNoiseOutput Parse output of .NOISE
 * simulation. \param[in] ngspice_file Spice output file name \param[out]
 * sim_points 2D array in which simulation points should be extracted. All
 * simulation points from all sweep variable steps are extracted in a single
 * array \param[out] var_list This list is filled by simulation variables. There
 * is a list of dependent and independent variables. An independent variable is
 * the first in list. \param[out] ParSwp Set to true if there was parameter
 * sweep
 */
void AbstractSpiceKernel::parseNoiseOutput(QString ngspice_file,
                                           QList<QList<double>>& sim_points,
                                           QStringList& var_list,
                                           bool& ParSwp) {
  var_list.clear();
...
void AbstractSpiceKernel::parsePZOutput(QString ngspice_file,
                                        QList<QList<double>>& sim_points,
...
/*!
 * \brief AbstractSpiceKernel::parseSENSOutput Parse output after DC sensitivity
 * analysis. \param[in] ngspice_file Spice output file name \param[out]
 * sim_points 2D array in which simulation points should be extracted. All
 * simulation points from all sweep variable steps are extracted in a single
 * array \param[out] var_list This list is filled by simulation variables. There
 * is a list of dependent and independent variables. An independent variable is
 * the first in list.
 */
void AbstractSpiceKernel::parseSENSOutput(QString ngspice_file,
                                          QList<QList<double>>& sim_points,
                                          QStringList& var_list) {
  QFile ofile(ngspice_file);
  if (ofile.open(QFile::ReadOnly)) {
...
/*!
 * \brief AbstractSpiceKernel::parseDC_OPoutput Parse DC OP simulation result
 * and setup schematic node names to show DC bias \param ngspice_file[in] DC OP
 * results test file
 */
...
/*!
 * \brief AbstractSpiceKernel::parseDC_OPoutputXY Parse DC OP simulation result
 * for XYCE and setup schematic node names to show DC bias \param
 * ngspice_file[in] DC OP results test file
 */
void AbstractSpiceKernel::parseDC_OPoutputXY(QString xyce_file) {
  ...
  if (ofile.open(QFile::ReadOnly)) {
    ...
    if (lines.count() >= 2) {
      QStringList nods =
          lines.at(0).split(QRegularExpression("\\s"), Qt::SkipEmptyParts);
      QStringList vals =
          lines.at(1).split(QRegularExpression("\\s"), Qt::SkipEmptyParts);
      QStringList::iterator n, v;
      for (n = nods.begin(), v = vals.begin();
           n != nods.end() || v != vals.end(); n++, v++) {
        if ((*n).startsWith("I(")) {
          (*n).remove(0, 2).chop(1);
          (*n) += "#branch"; // Ngspice compatible
        } else {
          (*n).remove(0, 2).chop(1); // Remove (I|V), starting ( and closing )
        }
 ...
 *        output from Parameter sweep analysis. Can parse data that uses
 * appedwrite. Extracts a simulation points array and variables names and types
 * (Real or Complex) from output. \param ngspice_file Spice output file name
 * \param sim_points 2D array in which simulation points should be extracted.
 * All simulation points from all sweep variable steps are extracted in a single
 * array \param var_list This list is filled by simulation variables. There is a
 * list of dependent and independent variables. An independent variable is the
 * first in list. \param isComplex Type of variables. True if complex. False if
 * real.
 */
void AbstractSpiceKernel::parseSTEPOutput(QString ngspice_file,
  ...
  while (!ngsp_data.atEnd()) {
    ...
    if (lin.contains("Plotname:") && // skip operating point
      ...
      for (bool t = false; !t;
           t      = (ngsp_data.readLine().startsWith("Plotname:")))
        ;
    }
    if (!header_parsed) {
      if (lin.contains("Flags") &&
      ...
      if (lin == "Variables:") {
        ...
        for (int i = 1; i < NumVars; i++) {
          ...
          if (lin.contains("dims=")) {
            extra_vars.append(dep_var); // XSPICE digital node or scalar
            QString tail =
                lin.section("dims=", 1, 1, QString::SectionSkipEmpty);
            extra_vars_dims.append(tail.toInt());
          }
        }
        header_parsed = true;
    ...
    if (start_values_sec) {
      if (!extractASCIISamples(lin, ngsp_data, sim_points, NumVars,
                               isComplex)) {
        continue;
      }
    }
 ...
 * \param[out] sim_points 2D array in which simulation points should be
 * extracted. All simulation points from all sweep variable steps are extracted
 * in a single array \param[out] var_list This list is filled by simulation
 * variables. There is a list of dependent and independent variables. An
 * independent variable is the first in list.
 */
...
void AbstractSpiceKernel::extractBinSamples(QDataStream& dbl,
                                            QList<QList<double>>& sim_points,
                                            int NumPoints, int NumVars,
                                            bool isComplex) {
  int cnt = NumPoints;
  while (cnt > 0) {
    QList<double> sim_point;
    ...
    sim_point.append(re);
    if (isComplex) {
      dbl >> im; // drop Im part of indep.var
    }
...
bool AbstractSpiceKernel::extractASCIISamples(QString& lin,
                                              QTextStream& ngsp_data,
                                              QList<QList<double>>& sim_points,
  ...
  if (!dataline_patter.match(lin).hasMatch()) {
    return false;
  }
  double indep_val =
      lin.section(sep, 1, 1, QString::SectionSkipEmpty).toDouble(&ok);
  // double indep_val =
  // lin.split(sep,QString::SkipEmptyParts).at(1).toDouble(&ok); // only real
  // indep vars
  if (!ok) {
    return false;
  }
 ...
 * \param sim_points[out] 2D array in which simulation points should be
 * extracted \param var_list[out] This list is filled by simulation variables.
 * There is a list of dependent and independent variables. An independent
 * variable is the first in list. \param isComplex[out] Type of variables. True
 * if complex. False if real.
 */
void AbstractSpiceKernel::parseXYCESTDOutput(QString std_file,
                                             QList<QList<double>>& sim_points,
                                             QStringList& var_list,
                                             bool& isComplex,
                                             bool& hasParSweep) {
  ...
  while (!ngsp_data.atEnd()) { // Parse header;
    ...
    if (lin.startsWith("Index ", Qt::CaseInsensitive)) {
      ...
      for (int i = 0; i < var_list.count() - 1; i++) {
        ...
        if (var_re.startsWith("Re(") && var_im.startsWith("Im(")) {
          QString var = var_re;
          var.remove(0, 3);
          var.chop(1);
 ...
 * \param var_list
 */
void AbstractSpiceKernel::parseXYCENoiseLog(QString logfile,
                                            QList<QList<double>>& sim_points,
                                            QStringList& var_list) {
  var_list.clear();
  var_list.append(""); // dummy indep var
 ...
 */
void AbstractSpiceKernel::parseResFile(QString resfile, QString& var,
                                       QStringList& values) {
  var.clear();
  values.clear();
...
/*!
 * \brief AbstractSpiceKernel::checkRawOutupt Determine Ngspice Raw output
 * contains parameter sweep or is XYCE STD output. \param ngspice_file[in] Raw
 * output file name \param values[out] Numbers of parameter sweep steps \return
 * true if parameter sweep presents, false otherwise
 */
int AbstractSpiceKernel::checkRawOutupt(QString ngspice_file,
                                        QStringList& values) {
  values.clear();
  ...
  if (ofile.open(QFile::ReadOnly)) {
    ...
    while (!ngsp_data.atEnd()) {
      ...
      if (lin.startsWith("End of Xyce(TM)")) {
        isXyce = true;
      }
...
/*!
 * \brief AbstractSpiceKernel::convertToQucsData Put data extracted from spice
 * raw text output files (given in outputs_files property) into single XML Qucs
 * Dataset. \param qucs_dataset A file name of Qucs Dataset to create \param
 * xyce True if Xyce simulator was used.
 */
void AbstractSpiceKernel::convertToQucsData(const QString& qucs_dataset) {
  if (a_DC_OP_only) { // Don't touch existing datasets when only DC was
                      // simulated
  ...
  for (const QString& ngspice_output_filename :
       a_output_files) { // For every simulation convert results to Qucs dataset
    QList<QList<double>> sim_points;
    ...
    if (ngspice_output_filename.startsWith("spice4qucs.")) {
      ...
      dataset_prefix = dataset_prefix_rx.match(ngspice_output_filename)
                           .captured(0)
                           .toLower();
      isCustomPrefix = !dataset_prefix.isEmpty();
    }
    QRegularExpression four_rx(".*\\.four[0-9]+$");
    QString full_outfile =
        a_workdir + QDir::separator() + ngspice_output_filename;
    if (ngspice_output_filename.endsWith("HB.FD.prn")) {
      ...
      parseXYCESTDOutput(full_outfile, sim_points, var_list, isComplex,
                         hasParSweep);
      if (hasParSweep) {
        QString res_file = QDir::toNativeSeparators(
            a_workdir + QDir::separator() + "spice4qucs.hb.cir.res");
      ...
      if (hasParSweep) {
        QString res_file = QDir::toNativeSeparators(
            a_workdir + QDir::separator() + "spice4qucs." + dataset_prefix +
            ".cir.res");
        parseResFile(res_file, swp_var, swp_var_val);
      }
      ...
      if (type == xyceSTDswp) {
        hasParSweep      = true;
        QString res_file = QDir::toNativeSeparators(
            a_workdir + QDir::separator() + "spice4qucs.sens.cir.res");
        parseResFile(res_file, swp_var, swp_var_val);
      }
    } else if (ngspice_output_filename.endsWith("_swp.plot")) {
      hasParSweep = true;
      if (ngspice_output_filename.endsWith(
              "_swp_swp.plot")) { // 2-var parameter sweep
        hasDblParSweep    = true;
        QString res2_file = QDir::toNativeSeparators(
            a_workdir + QDir::separator() + "spice4qucs." + dataset_prefix +
            ".cir.res1");
      ...
      QString res_file =
          QDir::toNativeSeparators(a_workdir + QDir::separator() +
                                   "spice4qucs." + dataset_prefix + ".cir.res");
      parseResFile(res_file, swp_var, swp_var_val);

      parseSTEPOutput(full_outfile, sim_points, var_list, isComplex, extra_vars,
                      extra_vars_dims);
    } else {
    ...
    if (hasParSweep) {
      int indep_cnt;
      if (swp_var_val.isEmpty()) {
        continue;
      }
      if (hasDblParSweep && swp_var2_val.isEmpty()) {
        continue;
      }
      if (hasDblParSweep) {
        indep_cnt =
            sim_points.count() / (swp_var_val.count() * swp_var2_val.count());
      } else {
        indep_cnt = sim_points.count() / swp_var_val.count();
      }
      if (!indep.isEmpty()) {
        ds_stream
            << QStringLiteral("<indep %1 %2>\n")
                   .arg(indep)
                   .arg(indep_cnt); // output indep var: TODO: parameter sweep
      ...
      ds_stream << QStringLiteral("<indep %1 %2>\n")
                       .arg(indep)
                       .arg(sim_points.count()); // output indep var: TODO:
                                                 // parameter sweep
        ...
        if (is_extra_var && !var.endsWith("_steps")) { // XSPICE digital node
          ...
          if (var_list.contains(var2)) {
            ...
            if (hasParSweep) {
              ...
              ds_stream << QStringLiteral("<indep %1 %2>\n")
                               .arg(var)
                               .arg(extra_vars_dims.at(var_idx));
              is_scalar = true;
            }
          }
        } else if (is_extra_var &&
                   var.endsWith("_steps") && // indep XSPICE digital var
      ...
      for (int idx = 0; idx < sim_points.count(); idx++) {
        ...
        if (!extra_vars_dims.isEmpty()) {
          if (hasParSweep) {
            ...
            if (is_extra_var && idx >= extra_vars_dims.at(var_idx)) {
              break;
            }
          }
        }
...
/*!
 * \brief AbstractSpiceKernel::removeAllSimulatorOutputs Clean temporary
 * simulator datasets.
 */
...
/*!
 * \brief AbstractSpiceKernel::normalizeVarsNames Convert spice-style variable
 * names to Qucs style and add simualation type prefix (i.e. AC, TRAN, DC).
 * Conversion for harmonic balance variable and current probes variables are
 * supported. \param var_list This list contains variable names that need
 * normalization.
 */
void AbstractSpiceKernel::normalizeVarsNames(QStringList& var_list,
                                             const QString& dataset_prefix,
                                             bool isCustom) {
  QString prefix  = "";
  QString iprefix = "";
  QString indep   = var_list.first();
  ...
  for (auto& it : var_list) { // For subcircuit nodes output i.e. v(X1:n1)
    it.replace(":",
               "_"); // colon symbol is reserved in Qucs as dataset specifier
  }
  ...
  QRegularExpression ivprobe_pattern_ngspice(
      "^(ac\\.|tran\\.)[Vv][Pp][Rr][0-9]+.*");
    ...
    if ((!(it->startsWith(prefix) || it->startsWith(iprefix))) || (HB)) {
      if (HB) {
        ...
        *it += suffix;
        ...
        if (iprobe_pattern.match(*it).hasMatch()) {
          (*it).remove(0, 1);
        }
      } else {
        *it = prefix + *it;
...
/*!
 * \brief AbstractSpiceKernel::slotFinished Simulation process normal finish
 * handler
 */
...
/*!
 * \brief AbstractSpiceKernel::slotProcessOutput Process a_simProcess output and
 * report progress
 */
void AbstractSpiceKernel::slotProcessOutput() {}

/*!
 * \brief AbstractSpiceKernel::getOutput Get sdtout and stderr output of
 * simulation process. \return Simulation process output
 */
 ...
 * \param cmd Simulator executable absolute path. For example -plugin
 * lib/Xyce_ADMS_Plugin.so
 */
...
QStringList AbstractSpiceKernel::collectSpiceLibraryFiles(Schematic* sch) {
  ...
  for (Component* pc : sch->a_DocComps) {
    ...
    if (pc->Model == "Sub") {
      Schematic* sub =
          new Schematic(nullptr, ((Subcircuit*)pc)->getSubcircuitFile());
      if (!sub->loadDocument()) // load document if possible
      {
        delete sub;
        continue;
