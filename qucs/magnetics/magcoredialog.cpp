...
MagCoreDialog::MagCoreDialog(Component* pc, Schematic* sch) : QDialog(sch) {
  ...
  vl1->addWidget(new QLabel(tr("Saturation magnetization, MS (A/m)")), 6, 0, 1,
                 2);
  edtMs = new QLineEdit;
  ...
  connect(cbxCoreType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotSetCoreImage()));
...
void MagCoreDialog::slotSetCoreImage() {
  ...
  case 0:
    ...
    sBar->showMessage("Core type not defined; Enter magnetic path length and "
                      "core area manually");
    break;
  case 1:
    coreImg->load(QString(":/bitmaps/svg/ringcore.svg"));
