...
SpiceLibComp::SpiceLibComp() {
  ...
  Description = QObject::tr(
      "SPICE library device. You can attach symbol patterns to it.");
  ...
  p_str += "]";
  Props.append(
      new Property("File", "", false, QObject::tr("SpiceLibrary file")));
  Props.append(new Property("Device", "", false,
                            QObject::tr("Subcircuit entry (.SUBCKT) name")));
  Props.append(new Property("SymPattern", "auto", false, p_str));
  Props.append(
      new Property("Params", "", false, QObject::tr("Extra parameters list")));
  Props.append(
      new Property("PinAssign", "", false, QObject::tr("Pins assignment")));
...
void SpiceLibComp::createSymbol() {
  ...
  QString FileName;
  QString symname =
      misc::properAbsFileName(Props.at(2)->Value, containingSchematic);
  if (QFileInfo::exists(symname)) {
    FileName = symname;
  } else {
    FileName = QucsSettings.BinDir;
    FileName += QStringLiteral("/../share/" QUCS_NAME "/symbols/%1.sym")
                    .arg(Props.at(2)->Value);
  }

  // Default symbol: LM358 in opamps.lib ---> opamps/LM358.sym
  QString LibName =
      misc::properAbsFileName(Props.at(0)->Value, containingSchematic);
  QString DefSym = LibName;
  QFileInfo inf(LibName); // Remove extension
  int l = inf.suffix().size();
  DefSym.chop(l + 1);
  DefSym += "/" + Props.at(1)->Value + ".sym";
  QString CommonSym = inf.canonicalPath() + "/" + inf.baseName() + "/" +
                      inf.baseName() + ".sym";
...
void SpiceLibComp::remakeSymbol(int No, QStringList& pin_names) {
  ...
  while (i < No) {
    ...
    if (i <= pin_names.count()) {
      nam = pin_names.at(i - 1);
    }
...
int SpiceLibComp::loadSymbol(const QString& DocName) {
  ...
  while (!stream.atEnd()) {
    ...
    if (Line.startsWith(".ID")) {
      continue; // Do not adjust properties for SpiceLibComp
    }
...
QString SpiceLibComp::spice_netlist(
...
QStringList SpiceLibComp::getSpiceLibraryFiles() {
  QString f =
      misc::properAbsFileName(getProperty("File")->Value, containingSchematic);
  QStringList files;
  files.append(f);
  return files;
}
