...
Switch::Switch() {
  ...
  Props.append(b.property(
      "MaxDuration", "1e-6",
      QObject::tr("Max possible switch transition time (transition time 1/100 "
                  "smallest value in 'time', or this number)")));

  b.hidden().simulator(spicecompat::simQucsator);
  Props.append(
      b.property("Transition", "spline",
                 QObject::tr("Resistance transition shape (Qucsator only)") +
                     " [abrupt, linear, spline]"));
...
QString Switch::spice_netlist(
  ...
  QString Max_duration =
      spicecompat::normalize_value(getProperty("MaxDuration")->Value);
  ...
  s += QStringLiteral("V%1 control_net%1 0 DC %2 PWL(0 %2")
           .arg(Name)
           .arg(oddValue);
