...
bool OctaveWindow::startOctave() {
  ...
  if (QucsSettings.QucsOctave.isEmpty()) {
    Program = OctavePath;
  } else {
    Program = QucsSettings.QucsOctave;
  }

  Arguments << "--no-history" << "-i" << "-f" << "-p"
            << QDir::toNativeSeparators(
                   QucsSettings.OctaveDir); // m-files location

  disconnect(&octProcess, 0, 0, 0);
  connect(&octProcess, SIGNAL(readyReadStandardError()),
          SLOT(slotDisplayErr()));
  connect(&octProcess, SIGNAL(readyReadStandardOutput()),
          SLOT(slotDisplayMsg()));
