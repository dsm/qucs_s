...
QucsAttenuator::QucsAttenuator() {
  ...
  DoubleValPower->setBottom(
      -1e9); // The default power unit is dBm, so Pin < 0 is expected
  ...
  connect(QSpinBox_InputPower, SIGNAL(valueChanged(double)), this,
          SLOT(slotCalculate()));
  ...
  powerunits.append(QStringLiteral("dB%1V [75%2]")
                        .arg(QChar(0xbc, 0x03))
                        .arg(QChar(0xa9, 0x03)));
  powerunits.append(QStringLiteral("dB%1V [50%2]")
                        .arg(QChar(0xbc, 0x03))
                        .arg(QChar(0xa9, 0x03)));
  ...
  Combo_InputPowerUnits->setSizePolicy(QSizePolicy::Minimum,
                                       QSizePolicy::Preferred);
  Combo_InputPowerUnits->setCurrentIndex(
      2); // Input power is mostly given in dBm
  ...
  QSpinBox_Freq->hide();
  connect(QSpinBox_Freq, SIGNAL(valueChanged(double)), this,
          SLOT(slotCalculate()));
  ...
  // Option for transforming a quarter wavelength transmission line into its
  // lumped element equivalent
  Check_QW_CLC = new QCheckBox("Use lumped components");
  Check_QW_CLC->hide();
  connect(Check_QW_CLC, SIGNAL(clicked(bool)), this,
          SLOT(slotTopologyChanged()));
  ...
  ComboR1_PowerUnits->setSizePolicy(QSizePolicy::Minimum,
                                    QSizePolicy::Preferred);
  ...
  ComboR2_PowerUnits->setSizePolicy(QSizePolicy::Minimum,
                                    QSizePolicy::Preferred);
  ...
  ComboR3_PowerUnits->setSizePolicy(QSizePolicy::Minimum,
                                    QSizePolicy::Preferred);
  ...
  ComboR4_PowerUnits->setSizePolicy(QSizePolicy::Minimum,
                                    QSizePolicy::Preferred);
  ...
  // This variable is necessary to provide the power unit conversion when the
  // corresponding power comboboxes are changed
  ...
  LabelResult =
      new QLabel(tr("")); // It is not needed to provide a "Success" message.
                          // The synthesis will fail only for certain
                          // attenuation-Z0 ratios in Pi or Tee type attenuators
...
void QucsAttenuator::slotHelpAbout() {
  QMessageBox::about(
      ...
      "QucsAttenuator Version " PACKAGE_VERSION +
          tr("\nAttenuator synthesis program\n") + tr("Copyright (C) 2006 by") +
          " Toyoyuki Ishikawa"
          "\n" +
          tr("Copyright (C) 2006 by") +
          " Stefan Jahn"
          "\n" +
          tr("Copyright (C) 2024 by") +
...
void QucsAttenuator::slotTopologyChanged() {
  ...
  case REFLECTION_TYPE:
    ...
    break;
  case QW_SERIES_TYPE:
    if (Check_QW_CLC->isChecked()) {
      pixTopology->setPixmap(QPixmap((":/bitmaps/qw_series_CLC.png")));
    } else {
  ...
  case QW_SHUNT_TYPE:
    if (Check_QW_CLC->isChecked()) {
      pixTopology->setPixmap(QPixmap((":/bitmaps/qw_shunt_CLC.png")));
    } else {
  ...
  case R_SERIES:
  case R_SHUNT:
    (ComboTopology->currentIndex() == R_SERIES)
...
void QucsAttenuator::slotCalculate() {
  ...
  // Frequency scale
  if (Combo_FreqUnits->currentText() == "GHz") {
    Values.freq *= 1e9;
  } else if (Combo_FreqUnits->currentText() == "MHz") {
    Values.freq *= 1e6;
  } else if (Combo_FreqUnits->currentText() == "kHz") {
    Values.freq *= 1e3;
  }

  // Calculate the input power
  Values.Pin = ConvertPowerUnits(QSpinBox_InputPower->value(),
                                 Combo_InputPowerUnits->currentText(), "W");
  ...
  if (result != -1) {
    ...
    lineEdit_R1_Pdiss->setText(
        QString::number(ConvertPowerUnits(Values.PR1, QStringLiteral("W"),
                                          ComboR1_PowerUnits->currentText()),
                        'f', 5));
    lineEdit_R2_Pdiss->setText(QString::number(
        ConvertPowerUnits(Values.PR2, "W", ComboR2_PowerUnits->currentText()),
        'f', 5));
    lineEdit_R3_Pdiss->setText(QString::number(
        ConvertPowerUnits(Values.PR3, "W", ComboR3_PowerUnits->currentText()),
        'f', 5));
    lineEdit_R4_Pdiss->setText(QString::number(
        ConvertPowerUnits(Values.PR4, "W", ComboR4_PowerUnits->currentText()),
        'f', 5));
    ...
    LabelResult->setText(tr("Error: Set Attenuation less than %1 dB")
                             .arg(QString::number(Values.MinimumATT, 'f', 3)));
    lineEdit_R1->setText("--");
    lineEdit_R2->setText("--");
    lineEdit_R3->setText("--");
...
void QucsAttenuator::slot_ComboInputPowerUnits_Changed(
    const QString& new_units) {
  // Convert power
...
// This function is called when the units of the power dissipated by R1 are
// changed
...
// This function is caled when the units of the power dissipated by R2 are
// changed
...
// This function is caled when the units of the power dissipated by R3 are
// changed
...
// This function is caled when the units of the power dissipated by R4 are
// changed
...
// This function performs the power units conversion. It receives two arguments:
// the original units and the new units.
double QucsAttenuator::ConvertPowerUnits(double Pin, QString from_units,
                                         QString to_units) {
  // Convert "from_units" to Watts
  if (from_units == "W")
    ; // Do nothing, this step is not needed
  else if (from_units == "dBm") {
    Pin = pow(10, 0.1 * (Pin - 30)); // dBm -> W
  } else if (from_units == QStringLiteral("dB%1V [75%2]")
                               .arg(QChar(0xbc, 0x03))
                               .arg(QChar(0xa9, 0x03))) {
    Pin = pow(10, (0.1 * Pin - 12)) / 75; // dBuV [75Ohm] -> W
  } else if (from_units == QStringLiteral("dB%1V [50%2]")
                               .arg(QChar(0xbc, 0x03))
                               .arg(QChar(0xa9, 0x03))) {
    Pin = pow(10, (0.1 * Pin - 12)) / 50; // dBuV [50Ohm] -> W
  } else if (from_units ==
             QStringLiteral("dBmV [75%2]").arg(QChar(0xa9, 0x03))) {
    Pin = pow(10, (0.1 * Pin - 6)) / 75; // dBmV [75Ohm] -> W
  } else if (from_units ==
             QStringLiteral("dBmV [50%2]").arg(QChar(0xa9, 0x03))) {
    Pin = pow(10, (0.1 * Pin - 6)) / 50; // dBmV [50Ohm] -> W
  } else if (from_units == "mW") {
    Pin = Pin * 1e-3; // mW -> W
  }
  ...
  if (to_units == "dBm") {
    return Pin; // Already done
  } else if (to_units == QStringLiteral("dB%1V [75%2]")
                             .arg(QChar(0xbc, 0x03))
                             .arg(QChar(0xa9, 0x03))) {
    Pin +=
        108.7506126339170004686755011380612925566374910126647878220; // W ->
                                                                     // dBuV
                                                                     // [75Ohm]
  } else if (to_units == QStringLiteral("dB%1V [50%2]")
                             .arg(QChar(0xbc, 0x03))
                             .arg(QChar(0xa9, 0x03))) {
    Pin +=
        106.9897000433601880478626110527550697323181011853789145868; // W ->
                                                                     // dBuV
                                                                     // [50Ohm]
  } else if (to_units == QStringLiteral("dBmV [75%2]").arg(QChar(0xa9, 0x03))) {
    Pin +=
        48.7506126339170004686755011380612925566374910126647878220; // W -> dBmV
                                                                    // [75Ohm]
  } else if (to_units == QStringLiteral("dBmV [50%2]").arg(QChar(0xa9, 0x03))) {
    Pin +=
        46.9897000433601880478626110527550697323181011853789145868; // W -> dBmV
                                                                    // [50Ohm]
  }
