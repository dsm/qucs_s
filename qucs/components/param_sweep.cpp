...
Param_Sweep::Param_Sweep() {
  ...
  Props.append(
      new Property("Sim", "", true,
                   QObject::tr("simulation to perform parameter sweep on")));
  Props.append(
      new Property("Type", "lin", true,
                   QObject::tr("sweep type") + " [lin, log, list, const]"));
  Props.append(
      new Property("Param", "R1", true, QObject::tr("parameter to sweep")));
...
QString Param_Sweep::getNgspiceBeforeSim(QString sim, int lvl) {
  ...
  QStringList parameter_list =
      getProperty("Param")->Value.split(this->param_split_str);
  QStringList::const_iterator constListIterator;
  QString type     = getProperty("Type")->Value;
  QString step_var = parameter_list.begin()
                         ->toLower(); // use first element name as variable name
  ...
  QString nline_char('\n');
  for (constListIterator = parameter_list.begin();
       constListIterator != parameter_list.end(); ++constListIterator) {
    ...
    if (temper_sweep) { // Sweep temperature
      s += QStringLiteral("option temp = $%1_act%2")
               .arg(step_var)
               .arg(nline_char);
    } else if (compfound) { // Sweep device
      s += QStringLiteral("alter %1 = $%2_act%3")
               .arg(par)
               .arg(step_var)
               .arg(nline_char);
    } else if (par.startsWith("@")) { // Sweep model
      s += QStringLiteral("altermod %1 = $%2_act%3")
               .arg(par)
               .arg(step_var)
               .arg(nline_char);
    } else { // Sweep .PARAM variable
      s += QStringLiteral("alterparam %1 = $%2_act%3reset%3")
               .arg(par)
               .arg(step_var)
               .arg(nline_char);
    }
...
QString Param_Sweep::getNgspiceAfterSim(QString sim, int lvl) {
  ...
  if (lvl == 0) {
    s += QStringLiteral(
             "echo \"$&number_%1  $%2_act\" >> spice4qucs.%3.cir.res\n")
             .arg(par)
             .arg(par)
             .arg(sim);
  } else {
    s += QStringLiteral(
             "echo \"$&number_%1\" $%1_act >> spice4qucs.%2.cir.res%3\n")
             .arg(par)
             .arg(sim)
             .arg(lvl);
  }
...
QString Param_Sweep::spice_netlist(
  ...
  if (getProperty("Type")->Value != "list" &&
      getProperty("Type")->Value != "const") {
