...
const char* empty_xpm1[] = { // for drag n'drop1
    "1 1 1 1", "  c None", " "};
...
SymbolWidget::SymbolWidget(QWidget* parent) : QWidget(parent) {
  ...
  QFontMetrics metrics(QucsSettings.font,
                       0); // use the the screen-compatible metric
...
QString SymbolWidget::theModel() {
  ...
  return "<Lib " + Prefix + " 1 0 0 " + QString::number(Text_x) + " " +
         QString::number(Text_y) + " 0 0 \"" + LibraryPath + "\" 0 \"" +
         ComponentName + "\" 0>";
}
...
void SymbolWidget::mouseMoveEvent(QMouseEvent* event) {
  ...
  drag->setPixmap(QPixmap(empty_xpm1));
...
void SymbolWidget::paintEvent(QPaintEvent*) {
  ...
  Painter.drawText(2, metrics.height(), 1, 1, Qt::AlignLeft | Qt::TextDontClip,
                   Warning);
  ...
  Painter.drawText(dx, y2 - y1 + 2, 1, 1, Qt::AlignLeft | Qt::TextDontClip,
                   DragNDropText);
  ...
  for (int i = 0; i < Arcs.size(); i++) {
    ...
    Painter.drawArc(cx + pc->x, cy + pc->y, pc->w, pc->h, pc->angle,
                    pc->arclen);
  }
 ...
 */
int SymbolWidget::createStandardSymbol(const QString& Lib_,
                                       const QString& Comp_) {
  Arcs.clear();
  Lines.clear();
  Rects.clear();
  ...
  if (Comp == "_BJT") {
    ...
    Arcs.append(
        new struct qucs::Arc(-34, -4, 8, 8, 0, 16 * 360, QPen(Qt::red, 1)));
    Arcs.append(
        new struct qucs::Arc(-4, -34, 8, 8, 0, 16 * 360, QPen(Qt::red, 1)));
    Arcs.append(
        new struct qucs::Arc(-4, 26, 8, 8, 0, 16 * 360, QPen(Qt::red, 1)));
    ...
    Arcs.append(
        new struct qucs::Arc(-34, -4, 8, 8, 0, 16 * 360, QPen(Qt::red, 1)));
    Arcs.append(
        new struct qucs::Arc(26, -4, 8, 8, 0, 16 * 360, QPen(Qt::red, 1)));
 ...
 */
int SymbolWidget::setSymbol(QString& SymbolString, const QString& Lib_,
                            const QString& Comp_) {
  Warning.clear(); // clear any message possibly previously set by
                   // createStandardSymbol()

  if (ModelString.count('\n') <
      2) { // single-line model: is a standard component
    return createStandardSymbol(Lib_,
                                Comp_); // build symbol from component type
  }

  if (SymbolString.isEmpty()) // Whenever SymbolString is empty, it tries to
                              // load the default symbol
  {
    // Load the default symbol for the current Qucs library
    ComponentLibrary parsedlib;
    int result = parseComponentLibrary(
        Lib_, parsedlib,
        QUCS_COMP_LIB_HEADER_ONLY); // need just the default symbol
  ...
  while (!stream.atEnd()) {
    ...
    if (Line.at(0) != '<') {
      return -1; // check for start char
    }
    if (Line.at(Line.length() - 1) != '>') {
      return -1; // check for end char
    }
    Line   = Line.mid(1, Line.length() - 2); // cut off start and end character
    Result = analyseLine(Line);
    if (Result < 0) {
      return -6; // line format error
    }
    z += Result;
  }
...
int SymbolWidget::analyseLine(const QString& Row) {
  ...
  if (s == ".PortSym") {      // here: ports are open nodes
    if (!getCompLineIntegers(Row, &i1, &i2, &i3)) {
      return -1;
    }
    ...
    if ((i1 - 4) < x1) {
      x1 = i1 - 4; // keep track of component boundings
    }
    if ((i1 + 4) > x2) {
      x2 = i1 + 4;
    }
    ...
    if (!getCompLineIntegers(Row, &i1, &i2, &i3, &i4)) {
      return -1;
    }
    if (!getPen(Row, Pen, 5)) {
      return -1;
    ...
    if (!getCompLineIntegers(Row, &i1, &i2, &i3, &i4, &i5, &i6)) {
      return -1;
    }
    if (!getPen(Row, Pen, 7)) {
      return -1;
    ...
    if (!getCompLineIntegers(Row, &i1, &i2)) {
      return -1;
    }
    Text_x = i1;
    Text_y = i2;
