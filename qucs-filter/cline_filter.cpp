...
// -------------------------------------------------------------------
QString* CoupledLine_Filter::createSchematic(tFilter* Filter,
  ...
  x = 60;
  *s += "<Components>\n";
  *s += QStringLiteral("<Pac P1 1 %1 380 18 -26 0 1 \"1\" 1 \"%2 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
  ...
  *s += QStringLiteral("<GND * 1 %1 410 0 0 0 0>\n").arg(x);
  ...
  for (i = 0; i <= Filter->Order; i++) {
    ...
    if (isMicrostrip) {
      ...
      *s += QStringLiteral(
                "<MCOUPLED MS1 1 %1 %2 -26 37 0 0 \"Sub1\" 1 \"%3\" 1 \"%4\" 1 "
                "\"%5\" 1 \"Kirschning\" 0 \"Kirschning\" 0 \"26.85\" 0>\n")
                .arg(x)
                .arg(y)
                .arg(num2str(width))
                .arg(num2str(len))
                .arg(num2str(gap));
      *s += QStringLiteral(
                "<MOPEN MS1 1 %1 %2 -12 15 1 2 \"Sub1\" 1 \"%3\" 1 "
                "\"Hammerstad\" 0 \"Kirschning\" 0 \"Kirschning\" 0>\n")
                .arg(x - 60)
                .arg(y + 30)
                .arg(num2str(width));
      *s += QStringLiteral(
                "<MOPEN MS1 1 %1 %2 -26 -81 1 0 \"Sub1\" 1 \"%3\" 1 "
                "\"Hammerstad\" 0 \"Kirschning\" 0 \"Kirschning\" 0>\n")
                .arg(x + 60)
                .arg(y - 30)
                .arg(num2str(width));
      ...
      y += 20;
      *s += QStringLiteral(
                "<CTLIN Line1 1 %1 %2 -26 16 0 0 \"%3\" 1 \"%4\" 1 \"%5\" 1 "
  ...
  *s += QStringLiteral("<Pac P2 1 %1 %2 18 -26 0 1 \"2\" 1 \"%3 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
            .arg(y + 70)
            .arg(Filter->Impedance);
  ...
  *s += QStringLiteral("<.SP SP1 1 70 460 0 67 0 0 \"lin\" 1 \"%2Hz\" 1 "
            ...
            .arg(num2str(2.0 * Filter->Frequency - Filter->Frequency2))
            .arg(num2str(2.0 * Filter->Frequency2 - Filter->Frequency));
  if (isMicrostrip) {
    *s += QStringLiteral("<SUBST Sub1 1 310 500 -30 24 0 0 \"%1\" 1 \"%2m\" 1 "
                         "\"%3m\" 1 \"%4\" 1 \"%5\" 1 \"%6\" 1>\n")
              .arg(Substrate->er)
  ...
  *s += QStringLiteral(
      "<Text 70 650 12 #000000 0 \"Coupled-line bandpass filter \\n ");
  switch (Filter->Type) {
  case TYPE_BESSEL:
