...
SpiceLibCompDialog::SpiceLibCompDialog(Component* pc, Schematic* sch)
    : QDialog(sch), a_symbolPinsCount(0), a_isChanged(false), a_libError(false),
      a_prev_row(-1), a_prev_col(-1), a_lastSymbolDir(), a_lastLibDir(),
      a_comp(pc), a_Doc(sch), a_symbol(new SymbolWidget),
      a_edtLibPath(new QLineEdit), a_edtParams(new QLineEdit),
      a_edtSymFile(new QLineEdit), a_edtSPICE(new QPlainTextEdit),
      ...
      a_tbwPinsTable(new QTableWidget), a_cbxSelectSubcir(new QComboBox),
      ...
      a_chbShowParams(new QCheckBox(tr("Show"))), a_subcirPins(),
      a_subcirSPICE() {
  ...
  connect(a_edtLibPath, SIGNAL(textChanged(QString)), this,
          SLOT(slotFillSubcirComboBox()));
  ...
  connect(a_cbxSelectSubcir, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotFillPinsTable()));
  ...
  connect(a_listSymPattern, SIGNAL(currentRowChanged(int)), this,
          SLOT(slotSetSymbol()));
  ...
  connect(a_tbwPinsTable, SIGNAL(cellDoubleClicked(int, int)), this,
          SLOT(slotTableCellDoubleClick()));
  ...
  if (!items.isEmpty()) {
    a_listSymPattern->setCurrentItem(items.at(0));
  }
  ...
  connect(a_rbSymFromTemplate, SIGNAL(toggled(bool)), this,
          SLOT(slotSetSymbol()));
  ...
  connect(a_tbwPinsTable, SIGNAL(cellChanged(int, int)), this,
          SLOT(slotChanged()));
  connect(a_listSymPattern, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotChanged()));
  connect(a_cbxSelectSubcir, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotChanged()));
  connect(a_edtSymFile, SIGNAL(textChanged(QString)), this,
          SLOT(slotSetSymbol()));
  connect(a_edtSymFile, SIGNAL(textChanged(QString)), this,
          SLOT(slotChanged()));
}
  ...
  if (r != noError) {
    ...
    case noSUBCKT:
      msg = tr("SPICE library parse error.\n"
               "No SUBCKT directive found in library ") +
            libfile;
      break;
    default:
      msg = tr("SPICE library parse error");
...
void SpiceLibCompDialog::slotFillPinsTable() {
  ...
  if (a_subcirPins.find(subcir_name) == a_subcirPins.end()) {
    return;
  }
  ...
  while (!ts.atEnd()) {
    ...
    if (line.startsWith(".SUBCKT") && !subcir_start) {
      ...
      a_subcirPins[subname] = pin_names;
               ...
               subcir_start) { // nested subcircuit
      nested_count++;
    }
    if (subcir_start) {
      subcir_body += line + "\n";
...
void SpiceLibCompDialog::slotTableCellDoubleClick() {
  ...
  if (c == 0) {
    return; // do not edit the first column
  }

  if (a_prev_col >= 0 &&
      a_prev_row >= 0) { // remove combo box from previous cell
    QTableWidgetItem* itm = new QTableWidgetItem("NC");
...
void SpiceLibCompDialog::slotBtnOpenLib() {
  QString s =
      QFileDialog::getOpenFileName(this, tr("Open SPICE library"), a_lastLibDir,
...
bool SpiceLibCompDialog::setCompProps() {
  ...
  if (a_rbUserSym->isChecked() && !QFileInfo::exists(a_edtSymFile->text())) {
    QMessageBox::warning(this, tr("Warning"),
                         tr("Set a valid symbol file name"));
    return false;
  }
