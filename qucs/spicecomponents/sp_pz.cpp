...
SpicePZ::SpicePZ() {
  ...
  Props.append(
      new Property("Input", "in 0", true,
                   QObject::tr("Two input nodes list (space separated)")));
  Props.append(
      new Property("Output", "out 0", true,
  ...
  Props.append(new Property(
      "PZ_mode", "pz", true,
      QObject::tr("Analysis mode (Pole-Zero, Poles only, Zeros only)") +
          " [pz, pol, zer]"));
}
...
QString SpicePZ::spice_netlist(
  ...
  if (dialect != spicecompat::SPICEXyce) {
    ...
    s += "let dummy_var = 0.0\n"; // To overcome featurebug of Ngspice when
