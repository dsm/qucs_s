...
QString* LC_Filter::createSchematic(tFilter* Filter, bool piType) {
  ...
  *s += QStringLiteral("<GND * 1 %1 350 0 0 0 0>\n").arg(x);
  ...
  for (i = 0; i < Filter->Order; i++) {
    ...
    case CLASS_BANDSTOP:
      ...
      Value *= 0.5 * fabs(Filter->Frequency2 / Filter->Frequency -
                          Filter->Frequency / Filter->Frequency2);
      if (i & 1) {
        *s += QStringLiteral("<L L1 1 %1 %2 -26 -44 0 0 \"%3H\" 1>\n")
                  .arg(x)
                  .arg(yl - 35)
  ...
  *s += QStringLiteral("<.SP SP1 1 70 %1 0 67 0 0 \"log\" 1 \"%2Hz\" 1 "
                       "\"%3Hz\" 1 \"201\" 1 \"no\" 0 \"1\" 0 \"2\" 0>\n")
            .arg(yc)
            .arg(num2str(Value))
            .arg(num2str(Value2));
  ...
  case spicecompat::simQucsator:
    eqn_string =
        QStringLiteral("<Eqn Eqn1 1 290 %1 -28 15 0 0 \"dBS21=dB(S[2,1])\" 1 "
                       "\"dBS11=dB(S[1,1])\" 1 \"yes\" 0>\n")
            .arg(yc + 10);
    break;
  case spicecompat::simNgspice:
    eqn_string =
        QStringLiteral("<NutmegEq NutmegEq1 1 290 %1 -28 15 0 0 \"SP1\" 1 "
                       "\"dBS21=dB(S_2_1)\" 1 \"dBS11=dB(S_1_1)\" 1>\n")
            .arg(yc + 10);
    break;
