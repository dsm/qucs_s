...
// -------------------------------------------------------------------------
QString* Line_Filter::createSchematic(tFilter* Filter, tSubstrate* Substrate,
                                      bool isMicrostrip) {
  ...
  x = 60;
  *s += "<Components>\n";
  *s += QStringLiteral("<Pac P1 1 %1 320 18 -26 0 1 \"1\" 1 \"%2 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
  ...
  *s += QStringLiteral("<GND * 1 %1 350 0 0 0 0>\n").arg(x);
  ...
  for (i = 0; i <= Filter->Order; i++) {
    ...
    if (isMicrostrip) {
      gap /=
          0.04598e-12 * (0.03 + pow(Wh, 1.23)) * (0.272 + 0.07 * Substrate->er);
    ...
    if (isMicrostrip) {
      if (gap < 1e-7) {
        QMessageBox::critical(
            ...
            "Using a substrate with larger thickness or with smaller "
            "permitivity may also help a little bit.");
        delete s;
        return NULL;
      }

      dl = 0.107 * (Wh + 9.0) * pow(gap / Substrate->height, 3.23) +
           2.09 * pow(gap / Substrate->height, 1.05) * (1.5 + 0.3 * Wh) /
               (1.0 + 0.6 * Wh);
    ...
    if (i > 0) {
      ...
      if (isMicrostrip) {
        *s += QStringLiteral(
                  "<MLIN MS1 1 %1 %2 -26 15 0 0 \"Subst1\" 0 \"%3\" 1 \"%4\" 1 "
        ...
        *s += QStringLiteral("<TLIN Line1 1 %1 %2 -26 20 0 0 \"%3\" 1 \"%4\" 1 "
                             "\"0 dB\" 0 \"26.85\" 0>\n")
                  .arg(x)
                  .arg(yc)
                  .arg(Filter->Impedance)
    ...
    if (isMicrostrip) {
      *s += QStringLiteral(
                "<MGAP MS1 1 %1 %2 -26 15 0 0 \"Subst1\" 0 \"%3\" 1 \"%4\" 1 "
                "\"%5\" 1 \"Hammerstad\" 0 \"Kirschning\" 0>\n")
                .arg(x)
                .arg(yc)
                .arg(num2str(width))
                .arg(num2str(width))
  ...
  x += 80;
  *s += QStringLiteral("<Pac P2 1 %1 320 18 -26 0 1 \"2\" 1 \"%2 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
            .arg(Filter->Impedance);
  *s += QStringLiteral("<GND * 1 %1 350 0 0 0 0>\n").arg(x);
  ...
  *s += QStringLiteral("<.SP SP1 1 70 420 0 67 0 0 \"lin\" 1 \"%2Hz\" 1 "
                       "\"%3Hz\" 1 \"300\" 1 \"no\" 0 \"1\" 0 \"2\" 0>\n")
            .arg(num2str(Value))
            .arg(num2str(Value2));
  if (isMicrostrip) {
    *s += QStringLiteral("<SUBST Subst1 1 300 460 -30 24 0 0 \"%1\" 1 \"%2m\" "
                         "1 \"%3m\" 1 \"%4\" 1 \"%5\" 1 \"%6\" 1>\n")
              .arg(Substrate->er)
  ...
  *s += QStringLiteral("<Eqn Eqn1 1 450 520 -28 15 0 0 \"S21_dB=dB(S[2,1])\" 1 "
  ...
  *s += QStringLiteral("<Text 420 420 12 #000000 0 \"end-coupled, "
                       "half-wavelength bandpass filter \\n ");
  switch (Filter->Type) {
  case TYPE_BESSEL:
