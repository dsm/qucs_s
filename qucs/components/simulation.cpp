...
QString wrapLabel(const QString& label) {
  ...
  int prev_break = 0;
  for (int curr_ix = line_len_hint; curr_ix < wrapped.length();
       curr_ix += line_len_hint) {
    ...
    if (prev_space != not_found && next_space != not_found) {
      ...
      curr_ix =
          curr_ix - prev_space > next_space - curr_ix ? next_space : prev_space;
...
void SimulationComponent::initSymbol(const QString& label) {
  ...
  updateComponentBounds(
      QRect{{0, 0}, QFontMetrics{labelFont()}.size(0, label_text)});
}
...
void SimulationComponent::drawSymbol(QPainter* p) {
  ...
  const std::vector<QPoint> CEFGA{ABCD.topRight(), ABCD.topRight() + box_height,
