...
VASettingsDialog::VASettingsDialog(TextDoc* Doc_) : QDialog(Doc_) {
  Doc = Doc_;
  ...
  QRadioButton* nonRadio = new QRadioButton(tr("unspecified device"));
  QRadioButton* bjtRadio = new QRadioButton(tr("NPN/PNP polarity"));
  QRadioButton* mosRadio = new QRadioButton(tr("NMOS/PMOS polarity"));
  ...
  all->addWidget(nonRadio, 4, 4, 0, 1);
  all->addWidget(bjtRadio, 4, 2);
  all->addWidget(mosRadio, 4, 4, 3, 4);

  toggleGroupTyp         = new QButtonGroup();
  QRadioButton* anaRadio = new QRadioButton(tr("analog only"));
  QRadioButton* digRadio = new QRadioButton(tr("digital only"));
  QRadioButton* allRadio = new QRadioButton(tr("both"));
  ...
  all->addWidget(anaRadio, 5, 5, 0, 1);
  all->addWidget(allRadio, 5, 2);
  all->addWidget(digRadio, 5, 5, 3, 4);
...
void VASettingsDialog::slotBrowse() {
  QString s = QFileDialog::getOpenFileName(
      this, tr("Enter an Icon File Name"),
