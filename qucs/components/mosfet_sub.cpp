...
Basic_MOSFET::Basic_MOSFET() {
  ...
  Props.append(
      new Property("Phi", "0.6 V", false, QObject::tr("surface potential")));
  Props.append(
      new Property("Lambda", "0.0", true,
  ...
  Props.append(
      new Property("Mjsw", "0.33", false,
                   QObject::tr("bulk junction periphery grading coefficient")));
  Props.append(
      new Property("Tt", "0.0 ps", false, QObject::tr("bulk transit time")));
  Props.append(
      new Property("Nsub", "0.0", false,
...
QString MOSFET_sub::spice_netlist(
  ...
  spice_incompat << "Type" << "Temp" << "L" << "W" << "Ad" << "As" << "Pd"
                 << "Ps" << "Rg" << "N" << "Tt" << "Nrd" << "Nrs" << "Ffe"
                 << "UseGlobTemp";
  ...
  for (const QString& parnam :
       check_defaults_list) { // Check some parameters for default value (zero)
...
Element* MOSFET_sub::info(QString& Name, char*& BitmapFile, bool getNewOne) {
  ...
  if (getNewOne) {
    return new MOSFET_sub();
  }
  return 0;
}

// -------------------------------------------------------
Element* MOSFET_sub::info_p(QString& Name, char*& BitmapFile, bool getNewOne) {
...
// -------------------------------------------------------
Element* MOSFET_sub::info_depl(QString& Name, char*& BitmapFile,
                               bool getNewOne) {
