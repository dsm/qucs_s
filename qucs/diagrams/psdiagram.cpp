...
/*!
  \class PSDiagram
  \brief The PSDiagram class implements the Polar and Smith (combi) diagrams
*/
...
PSDiagram::PSDiagram(int _cx, int _cy, bool _polarUp) : Diagram(_cx, _cy) {
  ...
  y2      = 200;
  if (_polarUp) {
    Name = "PS"; // polar diagram upper half ?
  } else {
    Name = "SP";
  }
...
Element* PSDiagram::info_sp(QString& Name, char*& BitmapFile, bool getNewOne) {
  ...
  if (getNewOne) {
    return new PSDiagram(0, 0, false);
  }
  return 0;
}
