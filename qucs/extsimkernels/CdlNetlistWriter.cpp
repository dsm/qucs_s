...
CdlNetlistWriter::CdlNetlistWriter(QTextStream& netlistStream,
                                   Schematic* schematic,
                                   bool resolveSpicePrefix)
    : a_netlistStream(netlistStream), a_schematic(schematic),
      a_resolveSpicePrefix(resolveSpicePrefix), a_netListString(),
      a_netListStringStream(&a_netListString, QIODeviceBase::WriteOnly),
      a_effectiveNetlistStream(resolveSpicePrefix ? a_netListStringStream
                                                  : a_netlistStream) {
...
bool CdlNetlistWriter::write() {
  a_effectiveNetlistStream << "* Qucs " << PACKAGE_VERSION << "  "
                           << a_schematic->getDocName() << "\n";

  a_effectiveNetlistStream << AbstractSpiceKernel::collectSpiceLibs(
      a_schematic);
...
int CdlNetlistWriter::prepareNetlist() {
  ...
  if (!a_schematic->giveNodeNames(&a_effectiveNetlistStream, countInit, collect,
                                  &errorText, numPorts)) {
...
".END"
  ...
  while ((idx = netList.indexOf(subcktPattern, from)) != -1) {
    ...
    const QString subcktKey(
        QString::fromUtf8("%1_%2").arg(subcktComponents[1]).arg(numPins));
  ...
  while ((idx = netList.indexOf(subcktInstPattern, from)) != -1) {
    netList[idx].replace("=", " = ");
    netList[idx].replace("/", " / ");
    QStringList subcktInstComponents(
        netList[idx].split(' ', Qt::SkipEmptyParts));
    ...
    const QString subcktInstKey(QString::fromUtf8("%1_%2")
                                    .arg(subcktInstComponents[modelIdx])
                                    .arg(numPins));
