...
S4Q_W::S4Q_W() {
  Description =
      QObject::tr("SPICE W:\nMultiple line ngspice or Xyce W specifications "
                  "allowed using \"+\" continuation lines.\nLeave continuation "
                  "lines blank when NOT in use.  ");
  ...
  Lines.append(new qucs::Line(
      5, 0, 0, 8, QPen(Qt::darkRed, 3, Qt::SolidLine, Qt::FlatCap)));
...
QString S4Q_W::spice_netlist(
  ...
  if (W.length() > 0) {
    s += QStringLiteral("%1").arg(W);
  }
  if (W_Line_2.length() > 0) {
    s += QStringLiteral("\n%1").arg(W_Line_2);
  }
  if (W_Line_3.length() > 0) {
    s += QStringLiteral("\n%1").arg(W_Line_3);
  }
  if (W_Line_4.length() > 0) {
    s += QStringLiteral("\n%1").arg(W_Line_4);
  }
  if (W_Line_5.length() > 0) {
    s += QStringLiteral("\n%1").arg(W_Line_5);
  }
  s += "\n";
