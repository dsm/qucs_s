...
// ------------------------------------------------------------
void CurveDiagram::calcCoordinate(const double*, const double* yD,
                                  const double*, float* px, float* py,
                                  Axis const* pa) const {
  double yr = yD[0];
  double yi = yD[1];
  if (xAxis.log) {
    *px =
        float(log10(yr / xAxis.low) / log10(xAxis.up / xAxis.low) * double(x2));
  } else {
    *px = float((yr - xAxis.low) / (xAxis.up - xAxis.low) * double(x2));
  }

  if (pa->log) {
    *py = float(log10(yi / pa->low) / log10(pa->up / pa->low) * double(y2));
  } else {
...
int CurveDiagram::calcDiagram() {
  ...
  if (xAxis.log) {
    ...
    while ((z <= x2) && (z >= 0)) { // create all grid lines
      ...
      if ((zD < 1.5 * zDstep) || (z == 0) || (z == x2)) {
        ...
        Lines.append(
            new qucs::Line(z, 5, z, -5, QPen(Qt::black, 0))); // x tick marks
      }
      ...
      if (back) {
        z = int(corr * log10(zD / fabs(xAxis.up)) +
                0.5); // int() implies floor()
        z = x2 - z;
      } else {
        z = int(corr * log10(zD / fabs(xAxis.low)) +
                0.5); // int() implies floor()
      }
    }
    ...
    if (xAxis.up >= 0.0) {
      if (xAxis.low <= 0.0) { // paint origin cross ?
  ...
  if (yAxis.numGraphs > 0) {
    if (calcYAxis(&yAxis, 0)) {
      valid |= 1;
      if (yAxis.up >= 0.0) {
        if (yAxis.low <= 0.0) { // paint origin cross ?
