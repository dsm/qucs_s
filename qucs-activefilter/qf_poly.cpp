...
qf_poly::qf_poly() : rep(NONE), d(0), krts(0), p(NULL), rts(NULL) {}

// Creates default polynoms
qf_poly::qf_poly(unsigned o) : rep(NONE), d(o), krts(0), p(NULL), rts(NULL) {}
...
void qf_poly::div(qf_double_t r, qf_double_t i) {
  ...
  if (d == 1) {
    if (((rep == ROOTS) || (rep == BOTH)) && (fabs(rts[0] - r) < ROOT_TOL) &&
        (fabs(rts[1]) < ROOT_TOL)) {
      d = 0;
      delete[] rts;
      rts = NULL;
...
#ifdef _QF_POLY_DEBUG
    std::cout << "Div: " << fabs(rts[k] - r) << " " << fabs(rts[k + 1] - i)
              << "\n";
#endif
...
void smpf(qf_poly& N, qf_poly& D) {
  ...
  for (i = 0; i < 2 * dN; i += 2) {
    for (j = 0; j < 2 * dD; j += 2) {
      ...
      if ((Ld[j / 2]) && (fabs(N.rts[i] - D.rts[j]) < ROOT_TOL) &&
...
void qf_poly::roots_to_complex(QVector<std::complex<float>>& roots) {
  roots.clear();
  for (unsigned int i = 0; i < d; i++) {
    roots.append(std::complex<float>(rts[2 * i], rts[2 * i + 1]));
  }
}
