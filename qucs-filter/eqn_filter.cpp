...
QString* Equation_Filter::createSchematic(tFilter* Filter) {
  ...
  *s += "<Components>\n";
  *s += QStringLiteral("<Pac P1 1 100 30 18 -26 0 1 \"1\" 1 \"%1 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
  ...
  *s += QStringLiteral("<RFEDD X1 1 260 0 -26 21 0 0 \"G\" 0 \"2\" 0 \"open\" "
                       "0 \"%1\" 1 \"0\" 1 \"%2\" 1 \"%3\" 1>\n")
            .arg(1.0 / Filter->Impedance)
            .arg(eqn)
            .arg(Filter->Impedance);

  *s += QStringLiteral("<Pac P2 1 400 30 18 -26 0 1 \"2\" 1 \"%1 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(Filter->Impedance);
  ...
  *s += QStringLiteral("<.SP SP1 1 110 160 0 67 0 0 \"log\" 1 \"%1Hz\" 1 "
                       "\"%2Hz\" 1 \"200\" 1 \"no\" 0 \"1\" 0 \"2\" 0>\n")
            .arg(num2str(0.1 * Filter->Frequency))
            .arg(num2str(a));
  *s += QStringLiteral("<Eqn Eqn1 1 350 170 -28 15 0 0 \"S21_dB=dB(S[2,1])\" 1 "
  ...
  *s += QStringLiteral("<Text 450 150 12 #000000 0 \"");
  ...
  switch (Filter->Class) {
  case CLASS_LOWPASS:
    *s += QStringLiteral("low-pass filter, %1Hz cutoff")
              .arg(num2str(Filter->Frequency));
    break;
  case CLASS_HIGHPASS:
    *s += QStringLiteral("high-pass filter, %1Hz cutoff")
              .arg(num2str(Filter->Frequency));
    break;
  case CLASS_BANDPASS:
    *s += QStringLiteral("band-pass filter, %1Hz...%2Hz")
              .arg(num2str(Filter->Frequency))
              .arg(num2str(Filter->Frequency2));
    break;
  case CLASS_BANDSTOP:
    *s += QStringLiteral("band-reject filter, %1Hz...%2Hz")
              .arg(num2str(Filter->Frequency))
              .arg(num2str(Filter->Frequency2));
    break;
  }
