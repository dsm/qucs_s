...
EqnDefined::EqnDefined() {
  ...
  // first properties !!!
  Props.append(
      new Property("Type", "explicit", false,
                   QObject::tr("type of equations") + " [explicit, implicit]"));
  Props.append(
      new Property("Branches", "1", false, QObject::tr("number of branches")));

  // last properties
  Props.append(
      new Property("I1", "0", true, QObject::tr("current equation") + " 1"));
  Props.append(
      new Property("Q1", "0", false, QObject::tr("charge equation") + " 1"));
...
QString EqnDefined::netlist() {
  ...
  for (int i = 2; i < Props.size(); i++) {
    s +=
        " " + Props.at(i)->Name + "=\"" + Name + "." + Props.at(i)->Name + "\"";
    e += "  Eqn:Eqn" + Name + Props.at(i)->Name + " " + Name + "." +
         Props.at(i)->Name + "=\"" + Props.at(i)->Value + "\" Export=\"no\"\n";
  }

  return s + e;
}

QString EqnDefined::spice_netlist(
  ...
  if (Props.at(0)->Value == "explicit") {
    ...
    for (int i = 0; i < Nbranch; i++) {
      ...
      spicecompat::convert_functions(Itokens,
                                     dialect == spicecompat::SPICEXyce);
      ...
      if (used_currents.contains(
              i)) { // if current is used add sensing source V=0
      ...
      QString Qeqn = Props.at(2 * (i + 1) + 1)
                         ->Value; // parse charge equation only for Xyce
      if (Qeqn != "0") {
        // if (dialect == spicecompat::SPICEXyce) {
        Qeqn.replace("^", "**");
        QStringList Qtokens;
        spicecompat::splitEqn(Qeqn, Qtokens);
        spicecompat::convert_functions(Qtokens,
                                       dialect == spicecompat::SPICEXyce);
...
/*!
 * \brief EqnDefined::subsVoltages Substitute voltages in spice Notation in
 * token list \param[in/out] tokens Token list. Should be obtained from
 * spicecompat::splitEqn(). This list is modified. \param[in] Nbranch Number of
 * branched of EDD
 */
 ...
 * \param tokens[in] Token list. Should be obtained from
 * spicecompat::splitEqn(). This list is not modified. \param branches[out] The
 * list of numbers of branches that currents are in use. Branches are numbered
 * from zero.
 */
...
/*!
 * \brief EqnDefined::subsCurrents Substitute currents in spice Notation in
 * token list \param tokens  Token list. Should be obtained from
 * spicecompat::splitEqn(). This list is modified.
 */
...
void EqnDefined::createSymbol() {
  ...
  // Font.setPointSizeF(Font.pointSizeF()/1.2);  // symbol text size
  // proportional to default font size
  ...
  // draw symbol
  int h = (PortDistance / 2) * ((Num - 1)) +
          PortDistance / 2; // total component half-height
  Lines.append(
      new qucs::Line(-15, -h, 15, -h, QPen(Qt::darkBlue, 2))); // top side
  Lines.append(
      new qucs::Line(15, -h, 15, h, QPen(Qt::darkBlue, 2))); // right side
  Lines.append(
      new qucs::Line(-15, h, 15, h, QPen(Qt::darkBlue, 2))); // bottom side
  Lines.append(
      new qucs::Line(-15, -h, -15, h, QPen(Qt::darkBlue, 2))); // left side
  ...
  while (i < Num) {                 // for every branch
    ...
    Texts.append(
        new Text(25, y - fHeight - 2, QString::number(i))); // left-aligned
  ...
  QFontMetrics metrics(QucsSettings.font,
                       0); // use the screen-compatible metric
  tx = x1 + 4;
