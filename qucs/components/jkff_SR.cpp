...
void jkff_SR::createSymbol() {
  ...
  Polylines.append(new qucs::Polyline(
      std::vector<QPointF>{{-30, -10}, {-20, 0}, {-30, 10}},
  ...
  Arcs.append(
      new qucs::Arc(-5, -50, 10, 10, 0, 16 * 360, QPen(Qt::darkBlue, 2)));
  Arcs.append(
      new qucs::Arc(-5, 40, 10, 10, 0, 16 * 360, QPen(Qt::darkBlue, 2)));
...
QString jkff_SR::vhdlCode(int) {
  ...
  s = "\n  " + Name + ":process (" + S + ", " + CLK + ", " + R + ") is\n" +
      ...
      " = '1' and " + K + " = '0') then state := '1';\n" + "      elsif (" + J +
      " = '0' and " + K + " = '1') then state := '0';\n" + "      end if;\n" +
      "    end if;\n" + "    " + Q + " <= state" + td + "    " + QB +
      " <= not state" + td + "  end process;\n";
  return s;
}
...
QString jkff_SR::spice_netlist(
  ...
  s += " " + J + " " + K + " " + CLK + " " + SET + " " + RESET + " " + Q + " " +
       QB;

  s += " " + tmp_model + "\n";
  s += QStringLiteral(".model %1 d_jkff(clk_delay=%2 set_delay=%2 "
                      "reset_delay=%2 rise_delay=%2 fall_delay=%2)\n")
           .arg(tmp_model)
           .arg(td);
  return s;
}
