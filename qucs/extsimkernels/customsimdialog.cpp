...
CustomSimDialog::CustomSimDialog(SpiceCustomSim* pc, Schematic* sch)
    : QDialog(sch), a_isXyceScr(false), a_isChanged(false), a_comp(pc),
      a_schematic(sch), a_edtCode(new QTextEdit(this)),
  ...
  connect(a_edtVars, SIGNAL(textChanged(const QString&)), this,
          SLOT(slotChanged()));

  QLabel* lblOut =
      new QLabel(tr("Extra outputs (semicolon separated; raw-SPICE or XYCE-STD "
                    "or scalars print format)"));
  connect(a_edtOutputs, SIGNAL(textChanged(const QString&)), this,
          SLOT(slotChanged()));
  ...
  if (a_isXyceScr) {
    ...
    for (const QString& line : strings) {
      if (write_ex.match(line).hasMatch()) {
        outp = line.section(QRegularExpression("\\s"), 1, 1,
