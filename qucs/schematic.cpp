...
inline QRect includePoint(const QRect& rect, const QPoint& point) {
  return rect.contains(point) ? rect : rect.united(QRect{point, point});
}

Schematic::Schematic(QucsApp* App_, const QString& Name_)
    : QucsDoc(App_, Name_), a_Wires(&a_DocWires), a_DocWires(),
      a_Nodes(&a_DocNodes), a_DocNodes(), a_Diagrams(&a_DocDiags), a_DocDiags(),
      a_Paintings(&a_DocPaints), a_DocPaints(), a_Components(&a_DocComps),
      a_DocComps(), a_SymbolPaints(), a_PostedPaintEvents(),
      a_symbolMode(false), a_isSymbolOnly(false), a_GridX(10), a_GridY(10),
      a_ViewX1(0), a_ViewY1(0), a_ViewX2(1), a_ViewY2(1),
      a_showFrame(0), // don't show
      a_Frame_Text0(tr("Title")), a_Frame_Text1(tr("Drawn By:")),
      a_Frame_Text2(tr("Date:")), a_Frame_Text3(tr("Revision:")),
      a_tmpScale(1.0), a_tmpViewX1(-200), a_tmpViewY1(-200), a_tmpViewX2(200),
      a_tmpViewY2(200), a_undoActionIdx(0),
      // The 'i' means state for being unchanged.
      a_undoAction(
          (QVector<QString*>() << new QString(" i\n</>\n</>\n</>\n</>\n"))),
      a_undoSymbolIdx(0),
      // The 'i' means state for being unchanged.
      a_undoSymbol(
          (QVector<QString*>() << new QString(" i\n</>\n</>\n</>\n</>\n"))),
      a_previousCursorPosition(), a_dragIsOkay(false), a_FileInfo(),
      a_Signals(), a_PortTypes(), a_isAnalog(false), a_isVerilog(false),
      a_creatingLib(false) {
  ...
  if (App_) {
    connect(this, SIGNAL(signalCursorPosChanged(int, int, QString)), App_,
            SLOT(printCursorPosition(int, int, QString)));

    connect(this, SIGNAL(horizontalSliderPressed()), App_,
            SLOT(slotHideEdit()));
    connect(this, SIGNAL(verticalSliderPressed()), App_, SLOT(slotHideEdit()));
    connect(this, SIGNAL(signalUndoState(bool)), App_,
            SLOT(slotUpdateUndo(bool)));
    connect(this, SIGNAL(signalRedoState(bool)), App_,
            SLOT(slotUpdateRedo(bool)));
    connect(this, SIGNAL(signalFileChanged(bool)), App_,
            SLOT(slotFileChanged(bool)));
  }
}
...
bool Schematic::createSubcircuitSymbol() {
  ...
  if (a_SymbolPaints.size() != port_count) {
    return false;
  }
...

  {
    ...
    for (std::size_t port_n = 0; port_n < port_count; port_n++) {
      ...
      if (put_on_left_side) {
        ...
        a_SymbolPaints.push_back(new GraphicLine(
            -10 - symbol_rect_half_width, port_y, -symbol_rect_half_width,
            port_y, QPen(Qt::darkBlue, 2)));
      } else {
        (*port_painting)->moveCenterTo(30, port_y);
        (*port_painting)->mirrorY();
        a_SymbolPaints.push_back(new GraphicLine(
            symbol_rect_half_width, port_y, symbol_rect_half_width + 10, port_y,
            QPen(Qt::darkBlue, 2)));
  ...
  a_SymbolPaints.push_back(new GraphicLine(
      -symbol_rect_half_width, -symbol_rect_half_height, symbol_rect_half_width,
      -symbol_rect_half_height, QPen(Qt::darkBlue, 2)));
  a_SymbolPaints.push_back(new GraphicLine(
      symbol_rect_half_width, -symbol_rect_half_height, symbol_rect_half_width,
      symbol_rect_half_height, QPen(Qt::darkBlue, 2)));
  a_SymbolPaints.push_back(new GraphicLine(
      ...
      symbol_rect_half_height, QPen(Qt::darkBlue, 2)));
  a_SymbolPaints.push_back(new GraphicLine(
      -symbol_rect_half_width, -symbol_rect_half_height,
      -symbol_rect_half_width, symbol_rect_half_height, QPen(Qt::darkBlue, 2)));
...
void Schematic::paintFrame(QPainter* painter) {
  ...
  painter->drawRect(frame_margin, frame_margin, frame_width - 2 * frame_margin,
                    frame_height - 2 * frame_margin);
  ...
  // Column labels
  {
    ...
    for (int x = h_step; x <= frame_width; x += h_step) {
      ...
      painter->drawText(tx, frame_height - frame_margin + 3, 1, 1,
                        Qt::TextDontClip, cn);
  ...
  painter->drawText(x1_ + d, y1_ + (d >> 1), 1, 1, Qt::TextDontClip,
                    a_Frame_Text2);
  painter->drawLine(x1_ + z, y1_, x1_ + z,
                    y1_ + painter->fontMetrics().lineSpacing() + d);
  painter->drawText(x1_ + d + z, y1_ + (d >> 1), 1, 1, Qt::TextDontClip,
                    a_Frame_Text3);
  y1_ -= painter->fontMetrics().lineSpacing() + d;
  painter->drawLine(x1_, y1_, x2_, y1_);
  painter->drawText(x1_ + d, y1_ + (d >> 1), 1, 1, Qt::TextDontClip,
                    a_Frame_Text1);
  y1_ -=
      (a_Frame_Text0.count('\n') + 1) * painter->fontMetrics().lineSpacing() +
      d;
...
void Schematic::drawContents(QPainter* p, int, int, int, int) {
  ...
  trf.scale(a_Scale, a_Scale).translate(-a_ViewX1, -a_ViewY1);
  p->setTransform(trf);

  auto renderHints = p->renderHints();
  renderHints.setFlag(QPainter::Antialiasing)
...
void Schematic::drawDcBiasPoints(QPainter* painter) {
  ...
  for (auto* pn : *a_Nodes) {
    ...
    painter->drawRoundedRect(QRectF(rectX, rectY, rectWidth, rectHeight), 15,
                             15, Qt::RelativeSize);

    painter->setPen(z & 0x10 ? Qt::darkGreen : Qt::blue);
    painter->drawText(x - textRect.width() / 2, y + textRect.height() / 4,
                      value);
  }
  painter->restore();
}
...
void Schematic::PostPaintEvent(PE pe, int x1, int y1, int x2, int y2, int a,
                               int b, bool PaintOnViewport) {
...
void Schematic::contentsMouseMoveEvent(QMouseEvent* Event) {
  ...
  if (a_Diagrams == nullptr) {
    return; // fix for crash on document closing; appears time to time
  }
  ...
  if (Event->button() == Qt::RightButton) {
    ...
    if (a_App->MouseMoveAction == &MouseActions::MMoveMoving2 ||
      ...
      if (a_App->MousePressAction) {
        (a_App->view->*(a_App->MousePressAction))(this, Event, inModel.x(),
                                                  inModel.y());
      }
      return;
    }
...
void Schematic::zoomAroundPoint(double offeredScaleChange, QPoint coords,
...
void Schematic::showAll() {
  ...
  renderModel(newScale, newModelBounds, newModelBounds.center(),
              viewportRect().center());
}
...
void Schematic::zoomToSelection() {
  ...
  renderModel(newScale, modelBounds, selectedBoundingRect.center(),
              viewportRect().center());
}
...
void Schematic::showNoZoom() {
  ...
  if (usedArea.isNull()) {
    ...
    renderModel(noScale, includePoint(modelRect(), displayedInCenter),
                displayedInCenter, vpCenter);
    return;
  }
...
void Schematic::enlargeView(const Element* e) {
  ...
  QRect newModel = modelRect().united(br.marginsAdded({40, 40, 40, 40}));
...
void Schematic::drawGrid(QPainter* painter) {
...

  {
    ...
    painter->drawLine(origin.x() - 3, origin.y(), origin.x() + 4,
                      origin.y()); // horizontal stick
    painter->drawLine(origin.x(), origin.y() - 3, origin.x(),
                      origin.y() + 4); // vertical stick
  }
  ...
  // grid-step and grow it until its "size in scale" gets larger than the
  ...
  for (double x = gridTopLeft.x(); x <= gridBottomRight.x();
       x += horizontalStep) {
    for (double y = gridTopLeft.y(); y <= gridBottomRight.y();
         y += verticalStep) {
...
Schematic::Selection Schematic::currentSelection() const {
  ...
  for (auto* pw : *a_Wires) {
    ...
    if (pw->hasLabel() &&
        pw->label()->isSelected) { // check position of wire label
  ...
  for (auto* pn : *a_Nodes) {
    if (std::all_of(pn->begin(), pn->end(),
                    [](auto* e) { return e->isSelected; })) {
      selection.nodes.push_back(pn);
    }

    if (pn->hasLabel() &&
        pn->label()->isSelected) { // check position of node label
    ...
    for (auto* pp : a_SymbolPaints) {
      if (pp->Name == ".ID ") {
        ...
        for (Number = 1, it = GNames.begin(); it != GNames.end(); ++it) {
          id->subParameters.push_back(std::make_unique<SubParameter>(
              true, *it + "=" + GDefs[Number - 1],
...
void Schematic::contentsWheelEvent(QWheelEvent* Event) {
  ...
  if ((Event->modifiers() & Qt::ShiftModifier) || horizontalWheelAngleDelta) {
    int delta = (horizontalWheelAngleDelta ? horizontalWheelAngleDelta
                                           : verticalWheelAngleDelta) /
                2;
    if (delta > 0) {
      scrollLeft(delta);
    } else {
  ...
  // Zoom in or out
    ...
    const QPoint pointer{static_cast<int>(Event->position().x()),
...
void Schematic::scrollUp(int step) {
  ...
  // If the "should-be-displayed" point is located higher than model upper
  // bound, then extend the model
...
void Schematic::scrollDown(int step) {
  ...
  // Render model in its new size and position point in the top left corner of
  // viewport
...
void Schematic::scrollRight(int step) {
  ...
  // If the "should-be-displayed" point is to the right of the model right
  // bound, then extend the model
...
void Schematic::contentsDropEvent(QDropEvent* Event) {
  ...
  QMouseEvent e(QEvent::MouseButtonPress, ev_pos, mapToGlobal(ev_pos),
...
bool Schematic::checkDplAndDatNames() {
  QFileInfo Info(a_DocName);
  if (!a_DocName.isEmpty() && a_DataSet.size() > 4 &&
      a_DataDisplay.size() > 4) {
    QString base     = Info.completeBaseName();
    ...
    if (base != base_dat || base != base_dpl) {
      ...
      auto r = QMessageBox::information(this, QObject::tr("Open document"), msg,
