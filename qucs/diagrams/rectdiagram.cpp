...
// ------------------------------------------------------------
void RectDiagram::calcCoordinate(const double* xD, const double* yD,
  ...
  if (pa->log) {
    yr = sqrt(yr * yr + yi * yi);
    if (yr <= 0.0) {
      *py = -1e5; // "negative infinity"
    } else {
      *py = float(log10(yr / fabs(pa->low)) / log10(pa->up / pa->low) *
                  double(y2));
    }
  } else {
...
MappedPoint RectDiagram::pointToValue(const QPointF& point) {
  MappedPoint result{};
  // Update the value transform. TODO: This only needs to be called when the
  // limits change.
