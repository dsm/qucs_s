...
L_SPICE::L_SPICE() {
  Description =
      QObject::tr("SPICE L:\nMultiple line ngspice or Xyce L specifications "
                  "allowed using \"+\" continuation lines.\nLeave continuation "
                  "lines blank when NOT in use.  ");
  Simulator = spicecompat::simSpice;

  Arcs.append(
      new qucs::Arc(-18, -6, 12, 12, 0, 16 * 180, QPen(Qt::darkRed, 3)));
...
QString L_SPICE::spice_netlist(
  ...
  if (L_Line_2.length() > 0) {
    s += QStringLiteral("\n%1").arg(L_Line_2);
  }
  if (L_Line_3.length() > 0) {
    s += QStringLiteral("\n%1").arg(L_Line_3);
  }
  if (L_Line_4.length() > 0) {
    s += QStringLiteral("\n%1").arg(L_Line_4);
  }
  if (L_Line_5.length() > 0) {
    s += QStringLiteral("\n%1").arg(L_Line_5);
  }
  s += "\n";
