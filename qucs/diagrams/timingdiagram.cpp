...
void TimingDiagram::paintDiagram(QPainter* painter) {
  ...
  if (y1 > 0) { // paint scroll bar ?
    // draw scroll bar
    painter->fillRect(yAxis.numGraphs, 2, zAxis.numGraphs, 14,
                      QColor(192, 192, 192));
...
int TimingDiagram::calcDiagram() {
  ...
  while (g->isEmpty()) {
    if (!ig.hasNext()) {
      break; // no more graphs, exit loop
    }
  ...
  if (!firstGraph->isEmpty()) {
    ...
    if (invisibleCount <= 0) {
      xAxis.limit_min = 0.0; // longer than needed
    } else {
  ...
  for (Graph* g : Graphs) {
    ...
    if (g->Var.right(2) != ".X") { // not digital variable ?
      ...
      for (; z > 0; z--) {
        if (x + TimeStepWidth >= x2) {
          break;
        }
        Lines.append(
            new qucs::Line(x + 2, y - 1, x + TimeStepWidth - 2, y - 1, Pen));
        Lines.append(new qucs::Line(x + 2, y - tHeight + 5,
                                    x + TimeStepWidth - 2, y - tHeight + 5,
                                    Pen));
    ...
    if (strlen(pcx) < 2) { // vector or single bit ?
      ...
      yLast = 0;
      if (z > 0) {
        yLast += 2; // vertical line before first value ?
      }
      ...
      for (; z > 0; z--) {
        ...
        if ((*pcx & 254) == '0') {
          Lines.append(
              new qucs::Line(x, y - yNow, x + TimeStepWidth, y - yNow, Pen));
        } else {
          Texts.append(new Text(x + (TimeStepWidth >> 1) - 3, y, QString(pcx)));
          Lines.append(
              new qucs::Line(x + 3, y - 1, x + TimeStepWidth - 3, y - 1, Pen));
          Lines.append(new qucs::Line(x + 3, y - tHeight + 5,
                                      x + TimeStepWidth - 3, y - tHeight + 5,
                                      Pen));
          Lines.append(new qucs::Line(x, y - yNow, x + 3, y - 1, Pen));
          Lines.append(
              new qucs::Line(x, y - yNow, x + 3, y - tHeight + 5, Pen));
          Lines.append(new qucs::Line(x + TimeStepWidth - 3, y - 1,
                                      x + TimeStepWidth, y - yNow, Pen));
          Lines.append(new qucs::Line(x + TimeStepWidth - 3, y - tHeight + 5,
                                      x + TimeStepWidth, y - yNow, Pen));
        }
...
funcEnd:
  if (invisibleCount > 0) { // could all values be displayed ?
    ...
    if (x < MIN_SCROLLBAR_SIZE + 2) { // not enough space for scrollbar ?
      ...
      Lines.append(
          new qucs::Line(xAxis.numGraphs, -17, x2, -17, QPen(Qt::red, 0)));
      Lines.append(new qucs::Line(xAxis.numGraphs, 0, xAxis.numGraphs, -17,
                                  QPen(Qt::red, 0)));
      return 1;
    }
    ...
    if (zAxis.numGraphs < MIN_SCROLLBAR_SIZE) {
      yAxis.numGraphs -=
          (MIN_SCROLLBAR_SIZE - zAxis.numGraphs + 1) * z / NumAll;
...
bool TimingDiagram::scrollTo(int initial, int dx, int) {
  ...
  xAxis.limit_min +=
      double(dx) / double(x2 - xAxis.numGraphs - 39) * zAxis.limit_max;
