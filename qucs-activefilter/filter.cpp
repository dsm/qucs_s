...
Filter::Filter(Filter::FilterFunc ffunc_, Filter::FType type_,
               FilterParam par) {
  ffunc = ffunc_;
  ftype = type_;
  ...
  if ((ftype == Filter::HighPass) || (ftype == Filter::LowPass)) {
    ...
    if ((ftype == Filter::BandPass) ||
      ...
      double Fs1 = Fu + TW;
      double Fs1lp =
          fabsf(Fs1 - (F0 * F0) / Fs1); // stopband freq. of LPF prototype
      double Fs2   = Fl - TW;
...
void Filter::createSchematic(QString& s) {
  ...
  case Filter::HighPass:
    createHighPassSchematic(s);
    break;
  case Filter::LowPass:
    createLowPassSchematic(s);
    break;
  case Filter::BandPass:
    createBandPassSchematic(s);
    break;
  case Filter::BandStop:
    createBandStopSchematic(s);
    break;
  default:
    break;
  }
...
bool Filter::calcFilter() {
  ...
  case Filter::InvChebyshev:
    res = calcInvChebyshev();
    break;
  case Filter::Bessel:
    res = calcBessel();
    break;
  case Filter::Legendre:
    res = calcLegendre();
    break;
  case Filter::User:
    res = calcUserTrFunc();
  ...
  if (((ffunc == Filter::Cauer) || (ffunc == Filter::InvChebyshev)) &&
      (Zeros.isEmpty())) {
    return false;
  }
...
void Filter::createPartList(QStringList& lst) {
  ...
  lst << QStringLiteral("%1%2%3%4%5%6%7%8%9")
             .arg("Stage#", 6)
             .arg("C1", 12)
             .arg("C2", 12)
             .arg("R1(kOhm)", 10)
...
void Filter::createPolesZerosList(QStringList& lst) {
  ...
  if (!Zeros.isEmpty()) {
    ...
    for (std::complex<float> zero : Zeros) {
      lst << QString::number(zero.real()) + " + j*" +
                 QString::number(zero.imag());
    }
  }
  ...
  for (std::complex<float> pole : Poles) {
    lst << QString::number(pole.real()) + " + j*" +
               QString::number(pole.imag());
  }
  lst << "";
}

void Filter::createFirstOrderComponentsHPF(QString& s, RC_elements stage,
                                           int dx) {
  QString suf;
  ...
  s += QStringLiteral("<R R%1 1 %2 340 15 -26 0 1 \"%3k\" 1 \"26.85\" 0 "
                      "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
           .arg(Nr + 2)
           .arg(220 + dx)
           .arg(stage.R2, 0, 'f', 3);
  s += QStringLiteral("<R R%1 1 %2 240 -75 -26 1 1 \"%3k\" 1 \"26.85\" 0 "
                      "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
           .arg(Nr + 1)
           .arg(170 + dx)
           .arg(stage.R1, 0, 'f', 3);
  s += QStringLiteral("<R R%1 1 %2 260 -26 15 1 2 \"%3k\" 1 \"26.85\" 0 "
                      "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
           .arg(Nr + 3)
           .arg(310 + dx)
           .arg(stage.R3, 0, 'f', 3);
  s += QStringLiteral(
           "<C C%1 1 %2 190 -26 -45 1 0 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
           .arg(Nc + 1)
           .arg(100 + dx)
           .arg(C1, 0, 'f', 3)
           .arg(suf);
...
void Filter::createFirstOrderComponentsLPF(QString& s, RC_elements stage,
                                           int dx) {
  QString suf;
  double C1 = autoscaleCapacitor(stage.C1, suf);
  s += QStringLiteral("<OpAmp OP%1 1 %2 160 -26 42 0 0 \"1e6\" 1 \"15 V\" 0>\n")
  ...
  s += QStringLiteral("<R R%1 1 %2 340 15 -26 0 1 \"%3k\" 1 \"26.85\" 0 "
           ...
           .arg(220 + dx)
           .arg(stage.R2, 0, 'f', 3);
  s += QStringLiteral("<R R%1 1 %2 190 -75 -52 1 0 \"%3k\" 1 \"26.85\" 0 "
                      "\"0.0\" 0 \"0.0\" 0 \"26.85\" 0 \"european\" 0>\n")
           .arg(Nr + 1)
           .arg(100 + dx)
           .arg(stage.R1, 0, 'f', 3);
  s += QStringLiteral("<R R%1 1 %2 260 -26 15 1 2 \"%3k\" 1 \"26.85\" 0 "
           ...
           .arg(stage.R3, 0, 'f', 3);
  s += QStringLiteral(
           "<C C%1 1 %2 240 26 -45 1 1 \"%3%4\" 1 \"\" 0 \"neutral\" 0>\n")
           .arg(Nc + 1)
           .arg(170 + dx)
           .arg(C1, 0, 'f', 3)
           .arg(suf);
...
bool Filter::checkRCL() {
  for (auto& sec : Sections) {
    if (std::isnan(sec.R1) || std::isnan(sec.R2) || std::isnan(sec.R3) ||
        std::isnan(sec.R4) || std::isnan(sec.C1) || std::isnan(sec.C2)) {
      return false;
    }
  }
  return true;
...
bool Filter::calcBessel() {
  ...
  for (int i = 0; i < order; i++) {
    Poles.append(std::complex<float>(BesselPoles[order - 1][2 * i],
                                     BesselPoles[order - 1][2 * i + 1]));
  }
...
bool Filter::calcLegendre() {
  ...
  for (int i = 0; i < order; i++) {
    Poles.append(std::complex<float>(LegendrePoles[order - 1][2 * i],
                                     LegendrePoles[order - 1][2 * i + 1]));
  }
