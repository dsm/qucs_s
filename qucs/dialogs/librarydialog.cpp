...
LibraryDialog::LibraryDialog(QWidget* parent) : QDialog(parent) {
  ...
  hCheck->addStretch();
  connect(checkDescr, SIGNAL(stateChanged(int)), this,
          SLOT(slotCheckDescrChanged(int)));
...
void LibraryDialog::slotCreateNext() {
  if (NameEdit->text().isEmpty()) {
    QMessageBox::critical(this, tr("Error"),
                          tr("Please insert a library name!"));
    return;
  }
  ...
  if (count < 1) {
    QMessageBox::critical(this, tr("Error"),
                          tr("Please choose at least one subcircuit!"));
    return;
  }
  ...
  if(LibFile.exists()) {
    QMessageBox::critical(this, tr("Error"), tr("A system library with this name
...
void LibraryDialog::slotSave() {
  ...
  for (int i = 0; i < SelectedNames.count(); i++) {
    ...
    Schematic* Doc =
        new Schematic(0, QucsSettings.QucsWorkDir.filePath(SelectedNames[i]));
    ErrText->insertPlainText(
        tr("Loading subcircuit \"%1\".\n").arg(SelectedNames[i]));
    if (!Doc->loadDocument()) { // load document if possible
      delete Doc;
      ErrText->appendPlainText(
          tr("Error: Cannot load subcircuit \"%1\".").arg(SelectedNames[i]));
      break;
    }
    ...
    if (!kern->checkSchematic(err_lst)) {
      ErrText->insertPlainText(
          QStringLiteral(
              "Component %1 contains SPICE-incompatible components.\n"
    ...
    if (!checkAnalogLib->isChecked()) {
      ...
      if (ret) {
        ...
        FileList.clear();
        if (!IFiles.isEmpty()) {
          Stream << "  <VerilogModelIncludes \"" << IFiles.join("\" \"")
                 << "\">\n";
        }
        Success = error > 0 ? false : true;
      } else {
