...
Mutual2::Mutual2() {
  ...
  Arcs.append(new qucs::Arc(-16, -58, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(new qucs::Arc(-16, -46, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(new qucs::Arc(-16, -34, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(new qucs::Arc(-16, 46, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(new qucs::Arc(-16, 34, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(new qucs::Arc(-16, 22, 12, 12, 16 * 270, 16 * 180,
                            QPen(Qt::darkBlue, 2)));
  Arcs.append(
  ...
  stmp = "1";
  w = smallmetrics.boundingRect(stmp).width(); // compute width to right-align
  Texts.append(new Text(-13 - w, -61, stmp));
  stmp = "2";
  w = smallmetrics.boundingRect(stmp).width(); // compute width to right-align
  Texts.append(new Text(-13 - w, 18, stmp));
  Texts.append(
      new Text(13, -22, "3")); // left-aligned, no need to compute width

  Lines.append(
      new qucs::Line(0, -57, 0, 57, QPen(Qt::darkBlue, 1, Qt::DashLine)));
  ...
  Props.append(
      new Property("L1", "1 mH", false, QObject::tr("inductance of coil 1")));
  Props.append(
      new Property("L2", "1 mH", false, QObject::tr("inductance of coil 2")));
  Props.append(
      new Property("L3", "1 mH", false, QObject::tr("inductance of coil 3")));
  Props.append(
      new Property("k12", "0.9", false,
                   QObject::tr("coupling factor between coil 1 and 2")));
  Props.append(
      new Property("k13", "0.9", false,
                   QObject::tr("coupling factor between coil 1 and 3")));
  Props.append(
      new Property("k23", "0.9", false,
