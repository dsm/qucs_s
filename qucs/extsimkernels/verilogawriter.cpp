...
/*!
 * \brief vacompat::convert_functions convert Qucs mathematical function or
 * constant to Verilog-A equivalent \param tokens[in/out] QStringList contains
 * equation tokens
 */
void vacompat::convert_functions(QStringList& tokens) {
  QStringList conv_list; // Put here functions need to be converted
  conv_list << "q" << "`P_Q" << "kB" << "`P_K" << "pi" << "`M_PI";
...
QString vacompat::normalize_current(QString& plus, QString& minus,
                                    bool left_side) {
  QString s;
  if (plus == "gnd") {
    if (left_side) {
...
/*!
 * \brief VerilogAwriter::createVA_module Build Verilog-A module from the
 * subcircuit. \param stream[out] QTextStream where Verilog-A module should be
 * written. \param sch[in] Schematic that should be converted to Verilog-A
 * module \return true on success; false otherwise
 */
