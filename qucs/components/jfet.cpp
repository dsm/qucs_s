...
JFET::JFET() {
  ...
  Props.append(
      new Property("Vt0", "-2.0 V", true, QObject::tr("threshold voltage")));
  ...
  Props.append(
      new Property("Cgs", "0.0", false,
                   QObject::tr("zero-bias gate-source junction capacitance")));
  Props.append(
      new Property("Cgd", "0.0", false,
                   QObject::tr("zero-bias gate-drain junction capacitance")));
  Props.append(
      new Property("Pb", "1.0", false, QObject::tr("gate-junction potential")));
  Props.append(new Property(
      "Fc", "0.5", false,
...
QString JFET::spice_netlist(
  ...
  if (dialect == spicecompat::SPICEXyce) {
    spice_incompat << "Type" << "Area" << "Temp" << "Ffe" << "N" << "Isr"
                   << "Nr" << "M" << "Xti" << "Betatce" << "Vt0tc"
                   << "UseGLobTemp";
    // spice-incompatible parameters
    spice_tr << "Vt0" << "VtO"; // parameters that need conversion of names
  } else {
    spice_incompat << "Type" << "Area" << "Temp" << "Ffe" << "N" << "Isr"
                   << "Nr" << "M" << "Xti" << "Betatce" << "UseGlobTemp";
    // spice-incompatible parameters
