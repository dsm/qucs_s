...
// -----------------------------------------------------------------------
QString* StepImpedance_Filter::createSchematic(tFilter* Filter,
  ...
  if (isMicrostrip) {
    TL_Filter::calcMicrostrip(Substrate, Substrate->minWidth, Filter->Frequency,
                              er_eff_min, Zhigh);
    TL_Filter::calcMicrostrip(Substrate, Substrate->maxWidth, Filter->Frequency,
                              er_eff_max, Zlow);
  ...
  x = 60;
  *s += "<Components>\n";
  *s += QStringLiteral("<Pac P1 1 %1 330 18 -26 0 1 \"1\" 1 \"%2 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
  ...
  *s += QStringLiteral("<GND * 1 %1 360 0 0 0 0>\n").arg(x);
  ...
  for (i = 0; i < Filter->Order; i++) {
    ...
    if (i & 1) {
      ...
      if (i > 0) {
        len -=
            0.5 * Zlow * Zlow * getNormValue(i - 1, Filter) / Filter->Impedance;
      }
      if (isMicrostrip) {
        if ((i + 1) < Filter->Order) {
          len -= 0.5 * Zlow * Zlow * getNormValue(i + 1, Filter) /
                 Filter->Impedance;
        }
      }
      len /= Zhigh;
    ...
    if (isMicrostrip) {
      *s += QStringLiteral(
                "<MLIN MS1 1 %1 180 -26 15 0 0 \"Sub1\" 1 \"%2\" 1 \"%3\" 1 "
                "\"Hammerstad\" 0 \"Kirschning\" 0 \"26.85\" 0>\n")
                .arg(x)
                .arg(num2str(width))
                .arg(num2str(len));
    } else {
      *s += QStringLiteral("<TLIN Line1 1 %1 180 -26 20 0 0 \"%2\" 1 \"%3\" 1 "
                           "\"0 dB\" 0 \"26.85\" 0>\n")
                .arg(x)
                .arg(Z0)
                .arg(num2str(len));
  ...
  x += 80;
  *s += QStringLiteral("<Pac P2 1 %1 330 18 -26 0 1 \"2\" 1 \"%2 Ohm\" 1 \"0 "
                       "dBm\" 0 \"1 GHz\" 0>\n")
            .arg(x)
            .arg(Filter->Impedance);
  *s += QStringLiteral("<GND * 1 %1 360 0 0 0 0>\n").arg(x);

  *s += QStringLiteral("<.SP SP1 1 70 460 0 67 0 0 \"lin\" 1 \"%2Hz\" 1 "
                       "\"%3Hz\" 1 \"300\" 1 \"no\" 0 \"1\" 0 \"2\" 0>\n")
            .arg(num2str(0.1 * Filter->Frequency))
            .arg(num2str(10.0 * Filter->Frequency));
  if (isMicrostrip) {
    *s += QStringLiteral("<SUBST Sub1 1 300 500 -30 24 0 0 \"%1\" 1 \"%2m\" 1 "
                         "\"%3m\" 1 \"%4\" 1 \"%5\" 1 \"%6\" 1>\n")
              .arg(Substrate->er)
  ...
  *s += QStringLiteral("<Eqn Eqn1 1 450 560 -28 15 0 0 \"S21_dB=dB(S[2,1])\" 1 "
  ...
  *s += QStringLiteral(
      "<Text 420 460 12 #000000 0 \"stepped-impedance lowpass filter \\n ");
  switch (Filter->Type) {
  case TYPE_BESSEL:
