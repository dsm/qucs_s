...
Diagram::Diagram(int _cx, int _cy) {
  ...
  xAxis.min = xAxis.low = yAxis.min = yAxis.low = zAxis.min = zAxis.low = 0.0;
  xAxis.max = xAxis.up = yAxis.max = yAxis.up = zAxis.max = zAxis.up = 1.0;
...
void Diagram::paintDiagram(QPainter* painter) {
  ...
  for (qucs::Arc* arc : Arcs) {
    ...
    painter->drawArc(QRectF{arc->x, -arc->y, arc->w, arc->h}, arc->angle,
                     arc->arclen);
  }
  ...
  if (xAxis.Label.isEmpty()) {
    ...
    for (Graph* pg : Graphs) {
      ...
      if (Name[0] != 'C') { // locus curve ?
        ...
        Texts.append(
            new Text(x - w, y, "real(" + pg->Var + ")", pg->Color, 12.0));
      }
    }
  } else {
    y -= LineSpacing;
    ...
    for (Graph* pg : Graphs) {
      ...
      if (pg->cPointsY) {
        ...
        if (Name[0] != 'C') { // location curve ?
          ...
          Texts.append(new Text(x, y - w, "imag(" + var_name + ")", pg->Color,
                                12.0, 0.0, 1.0));
        }
        ...
        Texts.append(new Text(x, y - w, pg->Var + INVALID_STR, pg->Color, 12.0,
                              0.0, 1.0));
      }
      x -= LineSpacing;
    }
    ...
    for (Graph* pg : Graphs) {
      ...
      if (pg->cPointsY) {
        ...
        if (Name[0] != 'C') { // location curve ?
          ...
          Texts.append(
              new Text(x, y + w, var_name, pg->Color, 12.0, 0.0, -1.0));
        } else {
          w = metrics.boundingRect("imag(" + var_name + ")").width() >> 1;
          if (w > wmax) {
            wmax = w;
...
bool Diagram::insideDiagram(float x, float y) const {
  float R =
      float(x2) / 2.0 + 1.0; // +1 seems better (graph sometimes little outside)
  x -= R;
  y -= R;
  return ((x * x + y * y) <= R * R);
}
...
void Diagram::loadGraphData(const QString& defaultDataSet) {
  ...
  for (Graph* pg : Graphs) {
    ...
    if (pg->loadDatFile(defaultDataSet) !=
        1) { // load data, determine max/min values
      No++;
    }
    getAxisLimits(pg);
  }
...
void Diagram::recalcGraphData() {
  ...
  if ((Name == "Polar") || (Name == "Smith")) { // one axis only
    if (yAxis.min > zAxis.min) {
      yAxis.min = zAxis.min;
    }
    if (yAxis.max < zAxis.max) {
      yAxis.max = zAxis.max;
    }
  }
...
int Graph::loadDatFile(const QString& fileName) {
  ...
  if (pos <= 0) {
    ...
    QString ss3 =
        Info.path() + QDir::separator() + svar.left(pos) + ".dat" + tail;
    qDebug() << ss3;
  ...
  if (g->lastLoaded.isValid()) {
    if (g->lastLoaded.toMSecsSinceEpoch() >=
        Info.lastModified().toMSecsSinceEpoch()) { // Millisecond resulution is
                                                   // needed for tuning
  ...
  *pPos = '>';
  ...
  if (!isIndep) {
    ...
    while (!tmp.isEmpty()) {
      if (hasExplIndep) {
        g->mutable_axes().push_back(new DataX(ExplIndep));
      } else {
        g->mutable_axes().push_back(
            new DataX(tmp)); // name of independent variable
      }
      pos++;
      tmp = Line.section(' ', pos, pos);
    }
  ...
  if (Variable.right(3) != ".X ") { // not "digital"
    ...
    for (int z = counting; z > 0; z--) {
      pEnd = nullptr;
      while ((*pPos) && (*pPos <= ' ')) {
        pPos++; // find start of next number
      }
    ...
    for (int z = counting; z > 0; z--) {
      ...
      while ((*pPos) && (*pPos <= ' ')) {
        pPos++; // find start of next bit vector
      }
      if (*pPos == 0) {
        delete[] g->cPointsY;
        g->cPointsY = nullptr;
...
*/
int Graph::loadIndepVarData(const QString& Variable, char* FileString,
                            DataX* pD) {
  bool isIndep = false;
  QString Line, tmp;
  ...
  *pPos = '>';
  ...
  if (!isIndep) { // dependent variable can also be used...
    if (Line.indexOf(' ') >= 0) {
      return -1; // ...if only one dependency
    }
    Line = "<indep " + Line + " ";
  ...
  for (int z = 0; z < n; z++) {
    ...
    if (*pEnd > ' ') { // drop imaginary part because
      while (*pEnd > ' ') {
        pEnd++; // Complex number on X-axis has no sense
      }
    }
...
bool Diagram::sameDependencies(Graph const* g1, Graph const* g2) const {
  ...
  if (g1->numAxes() != g2->numAxes()) {
    return false;
  }

  for (unsigned i = 0; i < g1->numAxes(); ++i) {
    if (g1->axisName(i) != g2->axisName(i)) {
      return false;
    }
  }
...
// ------------------------------------------------------------
int Diagram::checkColumnWidth(const QString& Str, const QFontMetrics& metrics,
                              int colWidth, int x, int y) {
  ...
  if (w > colWidth) {
    ...
    if ((x + colWidth) >= x2) { // enough space for text ?
      ...
      Lines.append(
          new qucs::Line(x2 - 6, y - 4, x2 + 7, y - 4, QPen(Qt::red, 2)));
...
QString Diagram::save() {
  QString s =
      "<" + Name + " " + QString::number(cx) + " " + QString::number(cy) + " ";
  ...
  s += QString::number(yAxis.Units) + " " + QString::number(zAxis.Units);
...
void Diagram::createSmithChart(Axis* Axis, int Mode) {
  ...
  for (m = 1; m < GridY; m++) {
    ...
    n_sin = sin(n_sin);
    im    = (1.0 - n_cos) / n_sin *
         pow(Axis->up, 0.7);        // up^0.7 is beauty correction
    ...
    if (Below) {
      Arcs.append(new struct qucs::Arc(x, dx2, y, y, 16 * 360 - beta - theta,
                                       theta, GridPen));
    }
  }
  ...
  for (m = 1; m < GridX; m++) {
    ...
    if (fabs(fabs(im) - 1.0) > 0.2) { // if too near to |r|=1, it looks ugly
      Arcs.append(
          new struct qucs::Arc(x, (x2 + y) >> 1, y, y, beta, theta, GridPen));
    }
    ...
    if (Axis->up > 1.0) { // draw arcs on the rigth-handed side ?
      ...
      if (im >= 1.0) {
        ...
        Arcs.append(
            new struct qucs::Arc(x, (x2 + y) >> 1, y, y, phi, len, GridPen));
      }
    }
  }
  ...
  if (Axis->up > 1.0) { // draw circle with |r|=1 ?
    ...
    y = (x2 + R1) >> 1;
    Arcs.append(
        new struct qucs::Arc(x, y, R1, R1, beta, theta, QPen(Qt::black, 0)));
    ...
    if (Below) {
      ...
      y = y2 - 4 - QucsSettings.font.pointSize();
    }
...
// --------------------------------------------------------------
void Diagram::calcPolarAxisScale(Axis* Axis, double& numGrids, double& GridStep,
                                 double& zD) {
  if (Axis->autoScale) { // auto-scale or user defined limits ?
    ...
    Base =
        Axis->max / numGrids / pow(10.0, Expo); // get first significant digit
...
void Diagram::createPolarDiagram(Axis* Axis, int Mode) {
  ...
  if (xAxis.GridOn) {
    ...
    for (i = int(numGrids); i > 1; i--) { // create all grid circles
      ...
      QString lbl;
      if (engineeringNotation) {
        lbl = misc::num2str(GridNum);
      } else {
        lbl = misc::StringNiceNum(GridNum);
      }
      ...
      Arcs.append(new struct qucs::Arc((x2 - z) >> 1, (y2 + z) >> 1, z, z, tmp,
                                       len - phi, GridPen));
      zD += zDstep;
    }
  } else { // of  "if(GridOn)"
  ...
  Arcs.append(new struct qucs::Arc(0, y2, x2, y2, tmp, 16 * 360 - phi,
                                   QPen(Qt::black, 0)));
...
/*!
 ...
 \return value: "true" if axis runs from largest to smallest value
  ...
  \param[out]             GridNum  - number where the first numbered grid is
 placed \param[out]             GridStep - distance from one grid to the next
  \param[out]             zD     - screen coordinate where the first grid is
 placed \param[out]             zDstep - distance on screen from one grid to the
 next
*/
bool Diagram::calcAxisScale(Axis* Axis, double& GridNum, double& zD,
  ...
  if (Axis->autoScale) {
    ...
    GridNum = zD / GridStep;
    if ((1.0 - GridNum) < 1e-10) {
      GridNum = 0.0; // fix rounding errors
    }
    if (Axis->up <= 0.0) {
      if (GridNum < 0.3) {
        Axis->up += zD;
    ...
    GridNum = zD / GridStep;
    if ((1.0 - GridNum) < 1e-10) {
      zD = GridNum = 0.0; // fix rounding errors
    }
    if (Axis->low <= 0.0) {
      if (GridNum > 0.7) {
...
*/
bool Diagram::calcAxisLogScale(Axis* Axis, int& z, double& zD, double& zDstep,
                               double& corr, int len) {
  bool mirror = false;
...
bool Diagram::calcYAxis(Axis* Axis, int x0) {
  ...
  if (Axis->log) {
    if (Axis->autoScale) {
      if (Axis->max <= 0.0 or Axis->min <= 0.0) {
        return false; // invalid
      }
    } else if (Axis->limit_min <= 0.0 or Axis->limit_max <= 0.0) {
      return false; // invalid
    }
    ...
    while ((z <= y2) && (z >= 0)) { // create all grid lines
      ...
      if ((zD < 1.5 * zDstep) || (z == 0)) {
        ...
        if (engineeringNotation) {
          tmp = misc::num2str(yVal, 2);
        } else {
          tmp = misc::StringNiceNum(yVal);
        }
    ...
    double Expo;
    if (Axis->up == 0.0) {
      Expo = log10(fabs(Axis->up - Axis->low));
    } else {
      Expo = log10(fabs(Axis->up));
    }
    ...
    while ((z <= y2) && (z >= 0)) { // create all grid lines
      ...
      Lines.append(
          new qucs::Line(x0 - 5, z, x0 + 5, z, QPen(Qt::black, 0))); // y marks
      zD += zDstep;
      z = int(zD);
    }
  } // of "if(ylog) ... else ..."
...
void Diagram::calcCoordinateP(const double* x, const double* y, const double* z,
                              Graph::iterator& p, Axis const* A) const {
  float f1, f2;
