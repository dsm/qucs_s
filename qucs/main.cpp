...
bool loadSettings() {
  ...
  QucsSettings.DefaultSimulator =
      _settings::Get().item<int>("DefaultSimulator");
  QucsSettings.firstRun = _settings::Get().item<bool>("firstRun");

  /*** Temporarily continue to use QucsSettings to make sure all settings
   * convert okay and remain compatible ***/
  ...
  if (settings.contains("Qucsator")) {
    ...
    if (QucsSettings.Qucsconv.isEmpty()) {
      QucsSettings.Qucsconv = QStandardPaths::findExecutable(
          "qucsconv_rf", {QucsSettings.QucsatorDir});
    }
  } else {
  ...
  QucsSettings.AdmsXmlBinDir.setPath(
      _settings::Get().item<QString>("AdmsXmlBinDir"));
  QucsSettings.AscoBinDir.setPath(_settings::Get().item<QString>("AscoBinDir"));
  QucsSettings.NgspiceExecutable =
      _settings::Get().item<QString>("NgspiceExecutable");
  QucsSettings.XyceExecutable =
      _settings::Get().item<QString>("XyceExecutable");
  QucsSettings.XyceParExecutable =
      _settings::Get().item<QString>("XyceParExecutable");
  QucsSettings.SpiceOpusExecutable =
      _settings::Get().item<QString>("SpiceOpusExecutable");
  QucsSettings.NProcs = _settings::Get().item<int>("Nprocs");

  // TODO: Currently the default settings cannot include other settings during
  // initialisation. This is a problem for this setting as it needs to include
  // the QucsWorkDir setting. Therefore, set the default to an empty string and
  // populate it here by brute force.
  QucsSettings.S4Qworkdir = _settings::Get().item<QString>("S4Q_workdir");
  if (QucsSettings.S4Qworkdir == "") {
    QucsSettings.S4Qworkdir = QDir::toNativeSeparators(
        QucsSettings.QucsWorkDir.absolutePath() + "/spice4qucs");
  }

  QucsSettings.OctaveExecutable =
      _settings::Get().item<QString>("OctaveExecutable");
  QucsSettings.OpenVAFExecutable =
      _settings::Get().item<QString>("OpenVAFExecutable");

  QucsSettings.RFLayoutExecutable =
      _settings::Get().item<QString>("RFLayoutExecutable");
  QucsSettings.ResolveSpicePrefix =
      _settings::Get().item<bool>("ResolveSpicePrefix");

  QucsSettings.qucsWorkspaceDir.setPath(
      _settings::Get().item<QString>("QucsHomeDir"));
  QucsSettings.QucsWorkDir = QucsSettings.qucsWorkspaceDir;
  QucsSettings.tempFilesDir.setPath(
      QStandardPaths::writableLocation(QStandardPaths::CacheLocation));

  QucsSettings.IgnoreFutureVersion =
      _settings::Get().item<bool>("IgnoreVersion");
  QucsSettings.GraphAntiAliasing =
      _settings::Get().item<bool>("GraphAntiAliasing");
  QucsSettings.TextAntiAliasing =
      _settings::Get().item<bool>("TextAntiAliasing");
  QucsSettings.fullTraceName  = _settings::Get().item<bool>("fullTraceName");
  QucsSettings.RecentProjects = _settings::Get()
                                    .item<QString>("RecentProjects")
                                    .split("*", Qt::SkipEmptyParts);
  QucsSettings.RecentDocs = _settings::Get()
                                .item<QString>("RecentDocs")
                                .split("*", Qt::SkipEmptyParts);
...
bool saveApplSettings() {
  ...
  // store LargeFontSize as a string, so it will be also human-readable in the
  // settings file (will be a @Variant() otherwise)
  qs.setItem<QString>("LargeFontSize",
                      QString::number(QucsSettings.largeFontSize));
  ...
  qs.setItem<QString>("AdmsXmlBinDir",
                      QucsSettings.AdmsXmlBinDir.canonicalPath());
  ...
  qs.setItem<QString>("QucsHomeDir",
                      QucsSettings.qucsWorkspaceDir.canonicalPath());
 ...
 */
void qucsMessageOutput(QtMsgType type, const QMessageLogContext& context,
                       const QString& msg) {
...
int doNetlist(QString schematicFileName, QString netlistFileName,
  ...
  for (QStringList::Iterator it = Collect.begin(); it != Collect.end(); ++it) {
    ...
    if ((*it).right(4) != ".lst" && (*it).right(5) != ".vhdl" &&
        (*it).right(4) != ".vhd" && (*it).right(2) != ".v") {
      *netlistStream << *it << '\n';
    }
  }
  ...
  *netlistStream << '\n';
  ...
  if (netlist2Console) {
    std::cout << std::endl
              << netlistString->toLatin1().constData() << std::endl;
  }
...
int doNgspiceNetlist(QString schematicFileName, QString netlistFileName,
                     bool netlist2Console) {
  QucsSettings.DefaultSimulator = spicecompat::simNgspice;
  Module::registerModules();
...
int doCdlNetlist(QString schematicFileName, QString netlistFileName,
                 bool netlist2Console, bool resolveSpicePrefix) {
  QucsSettings.DefaultSimulator = spicecompat::simNgspice;
  Module::registerModules();
    ...
    if (cdlFile->open(QFile::WriteOnly)) {
      ...
      QString finalMsg =
          QStringLiteral(
              ...
              "The value for S4Q_workdir (default:/spice4qucs) needs to be "
              "writeable!")
              .arg(msg);
      QMessageBox::critical(nullptr, QStringLiteral("Problem with SaveNetlist"),
                            finalMsg, QMessageBox::Ok);
  ...
  CdlNetlistWriter cdlWriter(*netlistStream, schematic.get(),
                             resolveSpicePrefix);
  if (!cdlWriter.write()) {
    QMessageBox::critical(nullptr, QStringLiteral("Save CDL netlist"),
...
int doXyceNetlist(QString schematicFileName, QString netlistFileName,
                  bool netlist2Console) {
  QucsSettings.DefaultSimulator = spicecompat::simXyce;
  Module::registerModules();
  QScopedPointer<Schematic> schematic(openSchematic(schematicFileName));
...
int doPrint(QString schematicFileName, QString printFile, QString page, int dpi,
            QString color, QString orientation) {
  QucsSettings.DefaultSimulator = spicecompat::simQucsator;
  ...
  if (printFile.endsWith(".pdf")) {
    ...
    Printer->noGuiPrint(schematic.get(), printFile, page, dpi, color,
                        orientation);
  } else {
    ...
    for (Module* Mod : Comps) {
      if (Mod->info) {
        ...
        for (struct qucs::Arc* a : Arcs) {
          // we need an open item here; QGraphisEllipseItem draws a filled
          // ellipse and doesn't do the job here...
          QPainterPath* path = new QPainterPath();
          // the components do not contain the angles in degrees but in 1/16th
        ...
        //   introduce artifacts
        QSize sImage =
            rScene.size().toSize(); // rounding seems not to be an issue
        ...
        if (QucsSettings.GraphAntiAliasing) {
          hints |= QPainter::Antialiasing;
        }
        // Ask to antialias text if requested
        if (QucsSettings.TextAntiAliasing) {
...
void createDocData() {
  ...
  for (const QString& category : cats) {
    ...
    for (Module* Mod : Comps) {
      ...
      compData << "Identifier; ``" + c->Model +
                      "``"; // backticks for reST verbatim
      ...
      out << compData.join("\n");
      file.close();
      fprintf(stdout, "[%s] %s %s \n", category.toLatin1().data(),
              c->Model.toLatin1().data(), file.fileName().toLatin1().data());

      QStringList compProps;
      compProps
          << "# Note: auto-generated file (changes will be lost on update)";
      compProps << QStringLiteral("# %1; %2; %3; %4")
                       .arg("Name", "Value", "Display", "Description");
      for (Property* prop : c->Props) {
        compProps << QStringLiteral("%1; \"%2\"; %3; \"%4\"")
                         .arg(prop->Name, prop->Value,
                              prop->display ? "yes" : "no",
                              prop->Description.replace(
                                  "\"", "\"\"")); // escape quote in quote
      }
      ...
      if (!fileProps.open(QFile::WriteOnly | QFile::Text)) {
        return;
      }
      ...
      fprintf(stdout, "[%s] %s %s \n", category.toLatin1().data(),
              ...
              fileProps.fileName().toLatin1().data());
    } // module
  } // category
...
int main(int argc, char* argv[]) {
  ...
  // default
  QString QucsWorkdirPath =
      QDir::homePath() + QDir::toNativeSeparators("/QucsWorkspace");
  ...
  QucsSettings.BinDir  = QucsApplicationPath.contains("bin")
                             ? (QucsApplicationPath + QDir::separator())
                             : QucsDir.absoluteFilePath("bin/");
  QucsSettings.LangDir = QucsDir.canonicalPath() + "/share/" QUCS_NAME "/lang/";

  QucsSettings.LibDir =
      QucsDir.canonicalPath() + "/share/" QUCS_NAME "/library/";
  QucsSettings.SpiceLibDir =
      QucsDir.canonicalPath() + "/share/" QUCS_NAME "/spicelibrary/";
  QucsSettings.OctaveDir =
      QucsDir.canonicalPath() + "/share/" QUCS_NAME "/octave/";
  QucsSettings.ExamplesDir =
      QucsDir.canonicalPath() + "/share/" QUCS_NAME "/examples/";
...
#if defined(_WIN32) || defined(__MINGW32__)
    admsExec =
        QDir::toNativeSeparators(QucsSettings.BinDir + "/" + "admsXml.exe");
#else
  ...
  const QString applicationVersion(
      QString::fromUtf8("qucs s%1 (%2)").arg(PACKAGE_VERSION).arg(GIT));
#else
  const QString applicationVersion(
  ...
  parser.addOptions({
      ...
      {{"n", "netlist"},
       QCoreApplication::translate("main",
                                   "convert Qucs schematic into netlist")},
      {{"p", "print"},
       QCoreApplication::translate(
           "main", "print Qucs schematic to file (eps needs inkscape)")},
      {"page",
       QCoreApplication::translate("main", "set print page size (default A4)"),
       "A4|A3|B4|B5", "A4"},
      {"dpi", QCoreApplication::translate("main", "set dpi value (default 96)"),
       "NUMBER", "96"},
      {"color",
       QCoreApplication::translate("main", "set color mode (default RGB)"),
       "RGB|BW", "RGB"},
      {"orin",
       QCoreApplication::translate("main",
                                   "set orientation (default portraid)"),
       "portraid|landscape", "portraid"},
      {"i", QCoreApplication::translate("main", "use file as input schematic"),
       "FILENAME"},
      {"o", QCoreApplication::translate("main", "use file as output netlist"),
       "FILENAME"},
      {"ngspice",
       QCoreApplication::translate("main", "create Ngspice netlist")},
      {"cdl", QCoreApplication::translate("main", "create CDL netlist")},
      {"xyce", QCoreApplication::translate("main", "Xyce netlist")},
      {"run",
       QCoreApplication::translate("main", "execute Ngspice/Xyce immediately")},
      {"icons",
       QCoreApplication::translate(
           "main", "create component icons under ./bitmaps_generated")},
      {"doc", QCoreApplication::translate(
                  "main", "dump data for documentation:\n"
      ...
      {"list-entries",
       QCoreApplication::translate(
           "main", "list component entry formats for schematic and netlist")},
      {{"c", "netlist2Console"},
       QCoreApplication::translate("main", "write netlist to console")},
      {{"x", "spiceprefix"},
       QCoreApplication::translate("main",
                                   "resolve spice prefix during netlist CDL")},
  });
  ...
  if (parser.isSet("help")) {
    ...
    QRegularExpression optIndent(QString::fromUtf8(
        "--page|--dpi|--color|--orin|--ngspice|--xyce|--run|--cdl"));
    int idx;
    int from = 0;
  ...
  if (netlist_flag and print_flag) {
    ...
    return -1;
  } else if (((ngspice_flag || xyce_flag || cdl_flag) && print_flag) ||
             (run_flag && print_flag)) {
    fprintf(stderr,
            "Error: --print and Ngspice/CDL/Xyce cannot be used together\n");
    return -1;
