...
void Logical_Inv::createSymbol() {
  ...
  if (Props.back()->Value.at(0) == 'D') { // DIN symbol
    ...
    Arcs.append(
        new qucs::Arc(-30, -20, 40, 30, 0, 16 * 90, QPen(Qt::darkBlue, 2)));
    Arcs.append(
        new qucs::Arc(-30, -10, 40, 30, 0, -16 * 90, QPen(Qt::darkBlue, 2)));
...
Element* Logical_Inv::info(QString& Name, char*& BitmapFile, bool getNewOne) {
  ...
  if (getNewOne) {
    return new Logical_Inv();
  }
  return 0;
}

QString Logical_Inv::spice_netlist(
  ...
  s += " " + tmp_model + "\n";
  s += QStringLiteral(".model %1 d_inverter(rise_delay=%2 fall_delay=%2 "
                      "input_load=5e-13)\n")
           .arg(tmp_model)
           .arg(td);
  return s;
