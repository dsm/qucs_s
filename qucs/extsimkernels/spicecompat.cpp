...
QString spicecompat::normalize_value(QString Value) {
  ...
  if (s.startsWith('\'') && s.endsWith('\'')) {
    return Value; // Expression detected
  }
...
QString spicecompat::convert_function(QString tok, bool isXyce) {
  const QStringList conv_list_ngspice = {
      "q",    "1.6021765e-19", "kB",   "1.38065e-23", "pi",    "3.1415926539",
      "step", "stp",           "sign", "sgn",         "log10", "log"};
  ...
  if (isXyce) {
    conv_list = conv_list_xyce;
  } else {
    conv_list = conv_list_ngspice;
  }
...
/*!
 * \brief spicecompat::splitEqn Split Equation into tokens. Token delimenters
 * are mathamatical operation signs:  = ^ + - * /  and parentheses () \param[in]
 * eqn Equation to split \param[out] tokens String list in which to put tokens.
 */
void spicecompat::splitEqn(QString& eqn, QStringList& tokens) {
  ...
  const QRegularExpression fpn(
      "[0-9]+\\.[0-9]+[eE]"); // first part of float number
 ...
 * \return Return true if equation contain node voltages and/or current probe
 * variables
 */
bool spicecompat::containNodes(QStringList& tokens, QStringList& vars) {
  ...
  for (const QString& tok : tokens) {
    if (var_pattern.match(tok).hasMatch()) {
      return true;
    }
    if (disto_var.match(tok).hasMatch()) {
      return true;
    }
    if (system_vars.contains(tok)) {
      return true;
    }
 ...
 *        Spice-notation (i.e. Node.Vt --> V(Node) ) and determine used
 * simulation. This method modifies the input list of tokens. \param[in/out]
 * tokens \param[out] sim Used simulation. "ac" --- AC simulation; "disto" ---
 * DISTO simulation; "dc" --- DC simulation; "tran" --- Transient simulation;
 *             "all" --- All simulations;
 */
void spicecompat::convertNodeNames(QStringList& tokens, QString& sim) {
  ...
  for (QStringList::iterator it = tokens.begin(); it != tokens.end(); it++) {
    ...
    if (var_pattern.match(*it).hasMatch()) {
      if (!disto_var.match(*it).hasMatch()) {
        if ((it->endsWith(".v")) || (it->endsWith(".i"))) {
          sim = "ac";
        }
        if ((it->endsWith(".Vt")) || (it->endsWith(".It"))) {
          sim = "tran";
        }
        if ((it->endsWith(".V")) || (it->endsWith(".I"))) {
          sim = "dc";
...
QString spicecompat::convert_relative_filename(QString filename)
{
    ...
    QString s = QucsSettings.QucsWorkDir.absolutePath() + QDir::separator() +
filename; inf.setFile(s); return inf.exists() ? s : filename;
}
*/

int spicecompat::getPins(const QString& file, const QString& compname,
  ...
  const QRegularExpression subckt_header(
      "^\\s*\\.(S|s)(U|u)(B|b)(C|c)(K|k)(T|t)\\s.*");
  ...
  while (!stream.atEnd()) {
    ...
    if (header_start) {
      ...
      if (lin.startsWith("+")) {
        lin.remove(0, 1);
        QStringList pins =
            lin.split(QRegularExpression("[ \\t]"), Qt::SkipEmptyParts);
    ...
    if (subckt_header.match(lin).hasMatch()) {
      ...
      QString name =
          lin.section(sep, 1, 1, QString::SectionSkipEmpty).toLower();
      ...
      for (const auto& s1 : lst2) {
        QString pp = s1;
        if (pp.toLower() == "params:") {
          header_start = false;
        }
        if (!s1.contains('=') && (pp.toLower() != "params:")) {
          pin_names.append(s1);
        }
      }
      r = pin_names.count();
...
/*!
 * \brief spicecompat::convert_sweep_type Convert sweep mode (i.e. linear,
 * logarithmic, etc.) from Qucs notation to SPICE notation \param sweep Sweep
 * designation in Qucs notation \return Sweep designation in SPICE notation
 */
...
bool spicecompat::check_nodename(QString& node) {
  static QStringList nutmeg_keywords = {"gt",  "lt",  "ge", "ne", "le",
                                        "and", "not", "or", "eq"};
