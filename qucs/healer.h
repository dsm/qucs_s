...
template <typename T1, typename T2> struct UnorderedPair {
  T1 first;
  T2 second;
...
public:
  ...
  GenericPort(Port* port, Component* comp)
      : m_port{port}, m_comp{comp}, m_portType{PortType::Component} {}

  GenericPort(Wire* wire, WirePort portId)
      : m_wire{wire}, m_portType{portId == WirePort::One ? PortType::WireOne
                                                         : PortType::WireTwo} {}
...
struct SchematicMutator {
  ...
  virtual void connectWithWire(const QPoint& /*unused*/,
                               const QPoint& /*unused*/) {}
