...
void showWireModeHint(QucsApp* app, const qucs_s::wire::Planner planner) {
  ...
  case qucs_s::wire::Planner::PlanType::Straight:
    app->statusBar()->showMessage(
        QucsApp::tr("Wiring mode: free. RMB to switch to orthogonal."));
    break;
  // Here is a hidden knownledge: ThreeStepYX goes right before Straight in
  // PlanType enumeration.
  case qucs_s::wire::Planner::PlanType::ThreeStepYX:
    app->statusBar()->showMessage(
        QucsApp::tr("Wiring mode: orthogonal. RMB to switch to free."));
    break;
  default:
    app->statusBar()->showMessage(
        QucsApp::tr("Wiring mode: orthogonal. RMB to cycle through variants."));
  }
}
...
MouseActions::MouseActions(QucsApp* App_) {
  ...
  focusMEvent =
      new QMouseEvent(QEvent::MouseButtonPress, QPointF(0, 0), QPointF(0, 0),
...
static void paintAim(Schematic* Doc, int aimX, int aimY) {
  ...
  //   2. Shrink it a bit. The resulting rectangle is the *bounding* for the
  //   cross
  ...
  const QRect aimBounds{Doc->viewportToModel(viewportAimBounds.topLeft()),
...
void MouseActions::MMoveWire1(Schematic* Doc, QMouseEvent* Event) {
  ...
  inModel = Doc->contentsToModel(
      QPoint{Doc->contentsX() + Doc->viewport()->width() - 1 - 2,
...
void MouseActions::MMovePaste(Schematic* Doc, QMouseEvent* Event) {
  ...
  if (movingElements.size() == 1) {
    ...
    const auto get_br = [](const Element* e) { return e->boundingRect(); };

    auto br = std::transform_reduce(
        ++movingElements.begin(), movingElements.end(),
        get_br(movingElements.front()),
        [](const QRect& a, const QRect& b) { return a.united(b); }, get_br);
  ...
  for (auto* pe : movingElements) {
    ...
    if (auto* l = dynamic_cast<WireLabel*>(pe);
        l != nullptr && l->owner() == nullptr) {
...
void MouseActions::MMoveDelete(Schematic* Doc, QMouseEvent* Event) {
  ...
  // cannot draw on the viewport, it is displaced by the size of dock and
  // toolbar
  Doc->PostPaintEvent(_Line, MAx3 - 15, MAy3 - 15, MAx3 + 15, MAy3 + 15, 0, 0,
                      false);
  Doc->PostPaintEvent(_Line, MAx3 - 15, MAy3 + 15, MAx3 + 15, MAy3 - 15, 0, 0,
                      false);
}
...
/**
 * @brief MouseActions::MMoveLabel Paints a label above the mouse cursor for
 * "set wire label".
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveMarker Paints a triangle above the mouse for "set
 * marker on graph"
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveSetLimits Sets the cursor to a magnifying glass
 * with a wave.
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveMirrorX Paints rounded "mirror about y axis" mouse
 * cursor
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveMirrorX Paints rounded "mirror about x axis" mouse
 * cursor
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveActivate Paints a crossed box mouse cursor to
...
/**
 * @brief MouseActions::MMoveOnGrid Paints a grid beside the mouse cursor, put
 * "on grid" mode.
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveMoveTextB Paints mouse symbol for "move component
 * text" mode.
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveMoveText Paint rectangle around component text
 * being mouse moved
 * @param Doc
 * @param Event
 */
...
/**
 * @brief MouseActions::MMoveZoomIn Paints symbol beside the mouse to show the
 * "Zoom in" modus.
 * @param Doc
 * @param Event
 */
...
void MouseActions::rightPressMenu(Schematic* Doc, QMouseEvent* Event, float fX,
                                  float fY) {
  MAx1         = int(fX);
  MAy1         = int(fY);
  ...
  while (true) {
    if (focusElement) {
      ...
      QObject::connect(editProp, SIGNAL(triggered(bool)), QucsMain,
                       SLOT(slotEditElement()));
    ...
    if (focusElement) {
      if (focusElement->Type == isDiagram) {
        ...
        if (diagram->Name == "Rect" &&
            (!diagram->xAxis.autoScale || !diagram->yAxis.autoScale ||
             !diagram->zAxis.autoScale)) {
          ComponentMenu->addAction(QucsMain->resetDiagramLimits);
        }

        // TODO: This should probably be in qucs_init::initActions.
        QAction* actExport =
            new QAction(QObject::tr("Export as image"), QucsMain);
        QObject::connect(actExport, SIGNAL(triggered(bool)), QucsMain,
  ...
  if (focusElement) {
    if (focusElement->Type == isMarker) {
      ...
      QObject::connect(actPwrMatching, SIGNAL(triggered(bool)), QucsMain,
                       SLOT(slotPowerMatching()));
      ComponentMenu->addAction(actPwrMatching);
      if (((Marker*)focusElement)->pGraph->Var.left(2) == "S[") {
        QAction* act2PortMatching =
            new QAction(QObject::tr("2-port matching"), QucsMain);
        QObject::connect(act2PortMatching, SIGNAL(triggered(bool)), QucsMain,
...
// -----------------------------------------------------------
void MouseActions::MPressLabel(Schematic* Doc, QMouseEvent*, float fX,
                               float fY) {
...
// -----------------------------------------------------------
void MouseActions::MPressSelect(Schematic* Doc, QMouseEvent* Event, float fX,
                                float fY) {
  ...
  if (focusElement) {
    ...
    case isNode:
      if (QucsSettings.NodeWiring) {
        ...
        QucsMain->MouseReleaseAction =
            nullptr; // if function is called from elsewhere
        QucsMain->MouseDoubleClickAction = nullptr;
  ...
  if (focusElement == nullptr) {
    ...
    if (!Ctrl) {
      if (!focusElement
               ->isSelected) { // Don't move selected elements if clicked
...
// -----------------------------------------------------------
void MouseActions::MPressDelete(Schematic* Doc, QMouseEvent*, float fX,
                                float fY) {
...
// -----------------------------------------------------------
void MouseActions::MPressActivate(Schematic* Doc, QMouseEvent*, float fX,
                                  float fY) {
  MAx1 = int(fX);
  MAy1 = int(fY);
...
void MouseActions::MPressMirrorX(Schematic* Doc, QMouseEvent*, float fX,
                                 float fY) {
...
void MouseActions::MPressMirrorY(Schematic* Doc, QMouseEvent*, float fX,
                                 float fY) {
  Element* e = Doc->selectElement(int(fX), int(fY), false);
...
void MouseActions::MPressRotate(Schematic* Doc, QMouseEvent*, float fX,
                                float fY) {
  Element* e = Doc->selectElement(int(fX), int(fY), false);
...
void MouseActions::MPressElement(Schematic* Doc, QMouseEvent* Event, float,
                                 float) {
  if (selElem == 0) {
    return;
  }
  ...
  if (selElem->Type & isComponent) {
    ...
    case Qt::LeftButton:
      ...
      if (Module::vaComponents.contains(entryName)) {
        ...
        Comp =
            dynamic_cast<vacomponent*>(Comp)->newOne(filename); // va component
...
/**
 * @brief MouseActions::MPressWire1 Is called if starting point of wire is
 * pressed
 * @param Doc
 * @param fX
 * @param fY
 */
void MouseActions::MPressWire1(Schematic* Doc, QMouseEvent*, float fX,
                               float fY) {
  MAx1 = 0; // paint wire corner first up, then left/right
  MAx3 = int(fX);
  MAy3 = int(fY);
 ...
 * @param fY
 */
void MouseActions::MPressWire2(Schematic* Doc, QMouseEvent* Event, float fX,
                               float fY) {
  switch (Event->button()) {
  case Qt::LeftButton: {
...
void MouseActions::MPressMarker(Schematic* Doc, QMouseEvent*, float fX,
                                float fY) {
  MAx1       = int(fX);
  MAy1       = int(fY);
...
/**
 * @brief MouseActions::MPressSetLimits Sets the start point of the diagram
 * limits.
 * @param Doc
 * @param Event
 */
void MouseActions::MPressSetLimits(Schematic* Doc, QMouseEvent*, float fX,
                                   float fY) {
  // fX, fY are the scaled / adjusted coordinates, equivalent to
  // DOC_X_POS(Event->x()). MAx1, MAy1 are needed to set the start of the
  // selection box.
  MAx1 = int(fX);
  MAy1 = int(fY);

  // this (and many other collections) to be std::vector.
  // Check to see if the mouse is within a diagram using the oddly named
  // "getSelected".
  for (Diagram* diagram : *Doc->a_Diagrams) {
    // BUG: Obtaining the diagram type by name is marked as a bug elsewhere (to
    ...
    if (diagram->getSelected(fX, fY) && diagram->Name == "Rect") {
      ...
      // cx and cy are the adjusted points of the diagram's bottom left hand
      // corner.
...
// -----------------------------------------------------------
void MouseActions::MPressOnGrid(Schematic* Doc, QMouseEvent*, float fX,
                                float fY) {
  Element* pe = Doc->selectElement(fX, fY, false);
  if (pe) {
...
// -----------------------------------------------------------
void MouseActions::MPressMoveText(Schematic* Doc, QMouseEvent*, float fX,
                                  float fY) {
  MAx1         = int(fX);
  MAy1         = int(fY);
...
// -----------------------------------------------------------
void MouseActions::MPressZoomIn(Schematic* Doc, QMouseEvent*, float fX,
                                float fY) {
...
void MouseActions::MReleaseSelect2(Schematic* Doc, QMouseEvent* Event) {
  ...
  Doc->selectElements(QRect{MAx1, MAy1, MAx2, MAy2}.normalized(), IsCtrl,
                      !IsShift);
...
void MouseActions::MReleaseSetLimits(Schematic* Doc, QMouseEvent* Event) {
  ...
  for (Diagram* diagram : *Doc->a_Diagrams) {
    ...
    // Only process the selection if it ends in a diagram, and is the same
    // diagram as start.
    if (diagram->getSelected(MAx2, MAy2) && diagram == pActiveDiagram) {
      ...
      if (select.width() < MIN_SELECT_SIZE ||
          select.height() < MIN_SELECT_SIZE) {
        break;
      }
...
void MouseActions::MReleasePaste(Schematic* Doc, QMouseEvent* Event) {
  ...
  case Qt::LeftButton:
    ...
    for (auto* pe : movingElements) {
      ...
      case isDiagram:
        ...
        ((Diagram*)pe)
            ->loadGraphData(Info.absolutePath() + QDir::separator() +
                            Doc->getDataSet());
        Doc->enlargeView(pe);
        break;
      case isPainting: {
    ...
    rot = movingRotated;
    while (rot--) {
      std::ranges::for_each(movingElements, [](Element* e) { e->rotate(); });
    }
  ...
  case Qt::RightButton: { // right button rotates the elements
    ...
    if (movingElements.size() == 1) {
      ...
      std::ranges::for_each(movingElements,
                            [rot_c](Element* e) { e->rotate(rot_c); });
    }
    paintElementsScheme(Doc);
    // save rotation
    movingRotated++;
...
void MouseActions::editElement(Schematic* Doc, QMouseEvent* Event) {
  ...
  case isDigitalComponent:
    ...
    if (c->Model == "CORE") {
    ...
    } else if ((c->Model == ".CUSTOMSIM") || (c->Model == ".XYCESCR") ||
               (c->Model == "INCLSCR")) {
      ...
      TextBoxDialog* od =
          new TextBoxDialog("Edit .spiceinit configuration", c, Doc);
      if (od->exec() != 1) {
        break; // dialog is WDestructiveClose
      }
    } else {
  ...
  case isDiagram:
    ...
    if (dia->Name.at(0) == 'T' // check only on double click
        && Event->type() ==
               QMouseEvent::MouseButtonDblClick) { // don't open dialog on
                                                   // scrollbar
      if (dia->Name == "Time") {
        if (dia->cy < int(fY)) {
...
/**
 * @brief MouseActions::MDoubleClickWire2  Double click terminates wire
 * insertion.
 * @param Doc
 * @param Event
 */
void MouseActions::MDoubleClickWire2(Schematic* Doc, QMouseEvent* Event) {
  auto inModel = Doc->contentsToModel(Event->pos());
  MPressWire2(Doc, Event, static_cast<float>(inModel.x()),
              static_cast<float>(inModel.y()));
...
void MouseActions::MPressTune(Schematic* Doc, QMouseEvent* Event, float fX,
                              float fY) {
  Q_UNUSED(Event);
  int No       = 0;
  MAx1         = int(fX);
  ...
  if (focusElement && App->TuningMode) {
    ...
    case isComponentText: // property text of component ?
      ...
      // current property
      if (No > 0) {
        No--; // No counts also the on/screen component Name, so subtract 1 to
              // get the actual property number
        pp = pc->Props.at(No);
      }
      if (!pp) {
