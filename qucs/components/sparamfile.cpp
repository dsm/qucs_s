...
SParamFile::SParamFile() {
  ...
  Props.append(
      new Property("Data", "rectangular", false,
                   QObject::tr("data type") + " [rectangular, polar]"));
  Props.append(
      new Property("Interpolator", "linear", false,
  ...
  Props.append(
      new Property("Ports", "1", false, QObject::tr("number of ports")));
...
void SParamFile::createSymbol() {
  ...
  while (i < Num) { // add ports lines and numbers
    ...
    Texts.append(
        new Text(-25 - w, y - fHeight - 2, stmp)); // text right-aligned
...
QString SParamFile::spice_netlist(
  ...
  if (dialect == spicecompat::SPICEXyce) {
    ...
    for (int i = 0; i < Np; i++) {
      QString p_in =
          spicecompat::normalize_node_name(Ports.at(i)->Connection->Name);
      QString p_com =
          spicecompat::normalize_node_name(Ports.at(Np)->Connection->Name);
    ...
    for (Port* p1 : Ports) {
      s += " " +
           spicecompat::normalize_node_name(p1->Connection->Name); // node names
    }
    s += " Sub_" + Model + "_" + Name + "\n";
  }
  return s;
