...
QString Schematic::createClipboardFile() {
  ...
  if (z == 0) {
    return ""; // return empty if no selection
  }
...
bool Schematic::pasteFromClipboard(QTextStream* stream,
  ...
  // If no image in clipboard, proceed with normal text-based clipboard
  // processing
  QString Line;
  Line = stream->readLine();
  ...
  if (a_symbolMode) {
    while (!stream->atEnd()) {
      ...
      if (Line == "<Components>") {
        ...
        if (!loadPaintings(stream, (std::list<Painting*>*)pe)) {
          return false;
        }
      } else {
        QMessageBox::critical(
  ...
  while (!stream->atEnd()) {
    Line = stream->readLine();
    if (Line == "<Components>") {
      if (!loadComponents(stream, (std::list<Component*>*)pe)) {
        return false;
      }
    } else if (Line == "<Wires>") {
      if (!loadWires(stream, pe)) {
        return false;
      }
    } else if (Line == "<Diagrams>") {
      if (!loadDiagrams(stream, (std::list<Diagram*>*)pe)) {
        return false;
      }
    } else if (Line == "<Paintings>") {
      if (!loadPaintings(stream, (std::list<Painting*>*)pe)) {
...
int Schematic::savePropsJSON() {
  QFileInfo info(a_DocName);
  QString jsonfile =
      info.absolutePath() + QDir::separator() + info.baseName() + "_props.json";
  QString vafilename =
      info.absolutePath() + QDir::separator() + info.baseName() + ".va";
  QString osdifile =
      info.absolutePath() + QDir::separator() + info.baseName() + ".osdi";
  ...
  if (!QFile::exists(osdifile)) {
    ...
    for (; name != prop_name.end(); name++, val++) {
      stream << QStringLiteral("    { \"name\" : \"%1\", \"value\" : \"%2\", "
                               "\"display\" : \"false\", \"desc\" : \"-\"},\n")
                    .arg(*name, *val);
    }
    stream << "  ],\n\n";
    ...
    *osdi_log_ = (void*)osdi_log_skip;
    ...
    OsdiDescriptor* descriptors =
        reinterpret_cast<OsdiDescriptor*>(osdilib.resolve("OSDI_DESCRIPTORS"));
    auto descriptor = descriptors[0];
    void* handler   = nullptr;

    // OsdiSimParas and OsdiInitInfo have to be initialized before setup_model
    // and setup_instance
    ...
    OsdiSimParas sim_params = {.names     = sim_params_names_vec.data(),
                               .vals      = sim_params_vals_vec.data(),
                               .names_str = sim_params_str_vec.data(),
                               .vals_str  = nullptr};
    OsdiInitInfo sim_info   = {.flags = 0, .num_errors = 0, .errors = nullptr};
    ...
    descriptor.setup_instance(handler, instance, model, 300,
                              descriptor.num_terminals, &sim_params, &sim_info);
...
int Schematic::saveSymbolJSON() {
  QFileInfo info(a_DocName);
  QString jsonfile =
      info.absolutePath() + QDir::separator() + info.baseName() + "_sym.json";
...
int Schematic::saveDocument() {
  ...
  if (a_symbolMode) {
    ...
    stream << "  <View=" << a_ViewX1 << "," << a_ViewY1 << "," << a_ViewX2
           << "," << a_ViewY2 << ",";
    stream << a_Scale << "," << contentsX() << "," << contentsY() << ">\n";
  }
  stream << "  <Grid=" << a_GridX << "," << a_GridY << "," << a_GridOn << ">\n";
  ...
  if (fileSuffix() == "sym") {
    if (fileSuffix(a_DataDisplay) == "va") {
      ...
      if (QucsSettings.DefaultSimulator == spicecompat::simNgspice) {
        ...
        Arguments << QDir::toNativeSeparators(vaFile) << "-I"
                  << QDir::toNativeSeparators(include.absolutePath()) << "-e"
                  << QDir::toNativeSeparators(include.absoluteFilePath(
                         "qucsMODULEguiJSONsymbol.xml"))
                  << "-A" << "dyload";

        //      QProcessEnvironment env =
        //      QProcessEnvironment::systemEnvironment(); env.insert("PATH",
        //      env.value("PATH") );
        ...
        if (!file.exists()) {
          QMessageBox::critical(
              ...
              tr("Program admsXml not found: %1\n\n"
                 "Set the admsXml location on the application settings.")
                  .arg(admsXml));
          return -1;
        }
...
bool Schematic::loadProperties(QTextStream* stream) {
  ...
  while (!stream->atEnd()) {
    ...
    if (cstr == "View") {
      a_ViewX1 = nstr.section(',', 0, 0).toInt(&ok);
      if (ok) {
        a_ViewY1 = nstr.section(',', 1, 1).toInt(&ok);
        if (ok) {
          a_ViewX2 = nstr.section(',', 2, 2).toInt(&ok);
          if (ok) {
            a_ViewY2 = nstr.section(',', 3, 3).toInt(&ok);
            if (ok) {
              a_Scale = nstr.section(',', 4, 4).toDouble(&ok);
              if (ok) {
                a_tmpViewX1 = nstr.section(',', 5, 5).toInt(&ok);
                if (ok) {
      ...
      a_GridX = nstr.section(',', 0, 0).toInt(&ok);
      if (ok) {
        a_GridY = nstr.section(',', 1, 1).toInt(&ok);
        if (ok) {
      ...
      misc::convert2Unicode(a_Frame_Text0 = nstr);
    } else if (cstr == "FrameText1") {
      misc::convert2Unicode(a_Frame_Text1 = nstr);
    } else if (cstr == "FrameText2") {
      misc::convert2Unicode(a_Frame_Text2 = nstr);
    } else if (cstr == "FrameText3") {
      misc::convert2Unicode(a_Frame_Text3 = nstr);
    } else {
      QMessageBox::critical(nullptr, QObject::tr("Error"),
                            QObject::tr("Format Error:\nUnknown property: ") +
                                cstr);
...
bool Schematic::loadComponents(QTextStream* stream,
  ...
  while (!stream->atEnd()) {
    ...
    if (List) { // "paste" ?
      int z;
      for (z = c->Name.length() - 1; z >= 0;
           z--) { // cut off number of component name
        if (!c->Name.at(z).isDigit()) {
          break;
        }
      }
...
void Schematic::simpleInsertWire(Wire* pw) {
  ...
  if (pw->P1() == pw->P2()) {
    pn->acquireLabel(
        pw->releaseLabel()); // wire with length zero are just node labels
...
bool Schematic::loadDiagrams(QTextStream* stream, std::list<Diagram*>* List) {
  ...
  while (!stream->atEnd()) {
    ...
    if (cstr == "<Rect") {
      ...
      d = new SmithDiagram(0, 0, false);
    } else if (cstr == "<PS") {
      d = new PSDiagram();
    } else if (cstr == "<SP") {
      d = new PSDiagram(0, 0, false);
    } else if (cstr == "<Rect3D") {
      d = new Rect3DDiagram();
    } else if (cstr == "<Curve") {
      d = new CurveDiagram();
...
bool Schematic::loadPaintings(QTextStream* stream, std::list<Painting*>* List) {
  ...
  while (!stream->atEnd()) {
    ...
    if (cstr == "Line") {
      ...
      p = new EllipseArc();
    } else if (cstr == ".PortSym") {
      p = new PortSymbol();
    } else if (cstr == ".ID") {
      p = new ID_Text();
    } else if (cstr == "Text") {
      p = new GraphicText();
    } else if (cstr == "Rectangle") {
      p = new qucs::Rectangle();
    } else if (cstr == "Arrow") {
      p = new Arrow();
    } else if (cstr == "Ellipse") {
      p = new qucs::Ellipse();
    } else if (cstr == "ImagePainting") {
      p = new ImagePainting();
    } else {
      QMessageBox::critical(nullptr, QObject::tr("Error"),
                            QObject::tr("Format Error:\nUnknown painting!"));
...
bool Schematic::loadDocument() {
  ...
  if (!misc::checkVersion(Line)) { // wrong version number ?
    ...
    result = QMessageBox::warning(nullptr, QObject::tr("Warning"),
  ...
  while (!stream.atEnd()) {
    ...
    if (Line == "<Symbol>") {
      ...
      if (!loadDiagrams(&stream, &a_DocDiags)) {
        file.close();
        return false;
      }
    } else if (Line == "<Paintings>") {
      if (!loadPaintings(&stream, &a_DocPaints)) {
        file.close();
        return false;
      }
    } else {
...
void Schematic::propagateNode(QStringList& Collect, int& countInit,
                              Node* start_node) {
  bool setName = false;
  std::list<Node*> Cons;
  ...
  for (auto it = Cons.begin(); it != Cons.end(); it++) {
    ...
    for (auto* connected_element : *node) {
      ...
      if (setName) {
        if (a_isAnalog) {
          createNodeSet(Collect, countInit, wire, start_node);
        }
        setName = false;
      }
    }
 ...
 * \param countInit is the reference to a counter for nodesets (initial
 * conditions) \param Collect is the reference to a list of collected nodesets
...
bool Schematic::throughAllComps(QTextStream* stream, int& countInit,
                                QStringList& Collect, QPlainTextEdit* ErrText,
                                int NumPorts) {
  bool r;
  QString s;
  ...
  for (auto* pc : a_DocComps) {
    ...
    if (a_isAnalog) {
      if ((pc->Type & isAnalogComponent) == 0) {
        ErrText->appendPlainText(
            QObject::tr("ERROR: Component \"%1\" has no analog model.")
                .arg(pc->Name));
        return false;
      }
    } else {
      if ((pc->Type & isDigitalComponent) == 0) {
        ErrText->appendPlainText(
            QObject::tr("ERROR: Component \"%1\" has no digital model.")
                .arg(pc->Name));
        return false;
      }
    }
    ...
    if (pc->Model == "Sub") {
      ...
      {
        delete d;
        /// \todo implement error/warning message dispatcher for GUI and CLI
        /// modes.
        QString message =
            QObject::tr("ERROR: Cannot load subcircuit \"%1\".").arg(s);
    ...
    if (LibComp* lib = dynamic_cast</*const*/ LibComp*>(pc)) {
      ...
      if (!r) {
        ErrText->appendPlainText(
            QObject::tr("ERROR: \"%1\": Cannot load library component \"%2\" "
                        "from \"%3\"")
    ...
    if (pc->Model == "SPICE") {
      ...
      if (s.isEmpty()) {
        ErrText->appendPlainText(
            QObject::tr("ERROR: No file name in SPICE component \"%1\".")
                .arg(pc->Name));
        return false;
      }
      QString f           = pc->getSubcircuitFile();
    ...
    if (pc->Model == "VHDL" || pc->Model == "Verilog") {
      ...
      if (s.isEmpty()) {
        ErrText->appendPlainText(
            QObject::tr("ERROR: No file name in %1 component \"%2\".")
                .arg(pc->Model)
                .arg(pc->Name));
        return false;
      }
...
void Schematic::createSubNetlistPlain(QTextStream* stream,
                                      QPlainTextEdit* ErrText, int NumPorts) {
  int i, z;
  QString s;
  ...
  if (a_creatingLib) {
    ...
    if (!ofile.open(QIODevice::WriteOnly)) {
      ErrText->appendPlainText(
          tr("ERROR: Cannot create library file \"%s\".").arg(f));
      return;
    }
    ...
    if (pc->Model.at(0) == '.') { // no simulations in subcircuits
      ...
      if (!a_isAnalog) {
        if (a_isVerilog) {
          ...
          (*it_name) +=
              " " + ((*it_type).isEmpty() ? VHDL_SIGNAL_TYPE : (*it_type));
        }
      }
    }
  }
        ...
        for (auto* pi : a_SymbolPaints) {
          if (pi->Name == ".ID ") {
            ...
            for (const auto& sub_param : pid->subParameters) {
              ...
              QString v =
                  misc::Verilog_Param(sub_param->name.section('=', 1, 1));
...
bool Schematic::createSubNetlist(QTextStream* stream, int& countInit,
  ...
  // Emit subcircuit components
  createSubNetlistPlain(stream, ErrText, NumPorts);
  if (QucsSettings.DefaultSimulator != spicecompat::simQucsator && a_isAnalog) {
    AbstractSpiceKernel* kern = new AbstractSpiceKernel(this);
    QStringList err_lst;
    if (!kern->checkSchematic(err_lst)) {
      QString s = QStringLiteral(
                      "Subcircuit %1 contains SPICE-incompatible components.\n"
...
int Schematic::prepareNetlist(QTextStream& stream, QStringList& Collect,
  ...
  if (a_showBias > 0) {
    a_showBias = -1; // do not show DC bias anymore
  }
  ...
  for (Component* pc : a_DocComps) {
    ...
    if (pc->Model.at(0) == '.') {
      ...
      if ((allTypes & isComponent) == isComponent) {
        ErrText->appendPlainText(QObject::tr(
    ...
    if (a_isAnalog) {
      ...
      if (pc->Model == ".Digi" && pc->isActive) { // simulation component ?
        if (NumPorts > 0) {                       // truth table simulation ?
          ...
          if (a_isVerilog) {
            if (!misc::Verilog_Time(Time, pc->Name)) {
              return Time;
            }
          } else {
            if (!misc::VHDL_Time(Time, pc->Name)) {
              return Time; // wrong time format
            }
          }
        }
      }
