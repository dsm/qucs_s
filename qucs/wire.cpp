...
Wire::Wire(Node* n1, Node* n2) : Wire(n1->x(), n1->y(), n2->x(), n2->y()) {
  connectPort1(n1);
  connectPort2(n2);
}
...
bool Wire::getSelected(int x_, int y_) {
  return qucs_s::geom::is_near_line(QPoint{x_, y_}, QPoint{x1, y1},
                                    QPoint{x2, y2}, 5);
}
...
QPoint coordinatesFromDistance(double distanceFromPort1, const QPoint& port1,
                               const QPoint& port2) {
...
// Adapter to use when loading a label from a file. Label root coordinates are
// stored as distance from port 1 of the wire. Here it's translated to absolute
// coordinates and passed further where real work is done.
void Wire::setName(int distFromPort1, int text_x, int text_y,
  ...
  const auto root =
      coordinatesFromDistance(distFromPort1, QPoint{x1, y1}, QPoint{x2, y2});
  setName(name, value, root.x(), root.y(), text_x, text_y);
}

void Wire::setName(const QString& Name_, const QString& Value_, int root_x,
                   int root_y, int x_, int y_) {
...
QString Wire::save() {
  ...
  if (hasLabel()) {
    s += " \"" + label()->Name + "\" ";
    s +=
        QString::number(label()->x1) + " " + QString::number(label()->y1) + " ";
    s += QString::number(static_cast<int>(
        qucs_s::geom::distance(QPoint{x1, y1}, label()->root())));
    s += " \"" + label()->initValue + "\">";
  } else {
    s += R"( "" 0 0 0 "">)";
  }
...
bool Wire::load(const QString& _s) {
  ...
  if (!n.isEmpty()) {                         // is wire labeled ?
    ...
    int delta =
        s.section(' ', 7, 7).toInt(&ok); // delta for x/y root coordinate
    if (!ok) {
      return false;
    }
...
bool Wire::setP1(const QPoint& new_p1) {
  ...
  if (hasLabel()) {
    ...
    const auto ratio = qucs_s::geom::distance(old_p1, label()->root()) /
                       qucs_s::geom::distance(old_p1, p2);
    const auto x = static_cast<int>(
        std::round(new_p1.x() + ratio * (p2.x() - new_p1.x())));
    const auto y = static_cast<int>(
        std::round(new_p1.y() + ratio * (p2.y() - new_p1.y())));
...
bool Wire::setP2(const QPoint& new_p2) {
  ...
  if (hasLabel()) {
    ...
    const auto ratio = qucs_s::geom::distance(p1, label()->root()) /
                       qucs_s::geom::distance(p1, old_p2);
    const auto x =
        static_cast<int>(std::round(p1.x() + ratio * (new_p2.x() - p1.x())));
    const auto y =
        static_cast<int>(std::round(p1.y() + ratio * (new_p2.y() - p1.y())));
