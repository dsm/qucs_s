...
SettingsDialog::SettingsDialog(Schematic* Doc_) : QDialog(Doc_) {
  Doc = Doc_;
  ...
  connect(DataDisplayBrowseButt, SIGNAL(clicked()),
          SLOT(slotDataDisplayBrowse()));
  ...
  Check_RunScript = new QCheckBox(tr("run script after simulation"), Tab1);
  ...
  valExpr = new QRegularExpressionValidator(QRegularExpression("[1-9]\\d{0,2}"),
                                            this);
...
AuxFilesDialog::AuxFilesDialog(QWidget* parent, const QString& filter)
    : QDialog(parent) {
  fileName = QString();
