...
// --------------------------------------------------------------
// Calculates the impedance 'zl' and the relative effective permittivity
// 'er_eff' of a microstrip line.
void TL_Filter::calcMicrostrip(tSubstrate* substrate, double width, double freq,
...
// ---------------------------------------------------------------------
// Calculates the additional line length (per substrate height) of a microstrip
// open.
...
// synthesis equations doesn't exist.
void TL_Filter::sythesizeCoupledMicrostrip(double zl_e, double zl_o,
                                           double freq, tSubstrate* substrate,
  ...
  do {
    // compute line parameters
    getCoupledMicrostrip(width, gap, freq, substrate, zl_e_current,
                         zl_o_current, ce, co);
    ...
    dw = width / 100.0;
    getCoupledMicrostrip(width + dw, gap, freq, substrate, zl_e_result1,
                         zl_o_result1, ce, co);

    // compute line parameters
    dg = gap / 100.0;
    getCoupledMicrostrip(width, gap + dg, freq, substrate, zl_e_result2,
                         zl_o_result2, ce, co);
...
double TL_Filter::dispersionKirschning(double er, double Wh, double freq,
                                       double& er_eff, double& zl) {
  double a, b;
...
void TL_Filter::getCoupledMicrostrip(double ms_width, double g, double freq,
                                     tSubstrate* substrate, double& zl_e,
