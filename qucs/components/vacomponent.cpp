...
/*!
 * \brief vacomponent::newOne is used to mouse drop new items into the
 * schematic. \param filename File (JSON) containing the symbol paintins and
 * properties. \return \a Component based on the \p filename Used by
 * mouseactions to drop new items into the schematic.
 */
 ...
 */
Element* vacomponent::info(QString& Name, QString& BitmapFile, bool getNewOne,
                           QString filename) {
  ...
  /// The BitmapFile JSON entry can be modified in \see
  ...
  for (int paintIndex = 0; paintIndex < paintArray.size(); ++paintIndex) {
    ...
    if (!type.compare("line")) {
      ...
      Lines.append(new qucs::Line(
          x1, y1, x2, y2, QPen(QColor(color), thick, penMap.value(style))));
    }
    ...
    if (!type.compare("rectangle")) {
      ...
      Rects.append(new qucs::Rect(
          x, y, w, h, QPen(QColor(color), thick, penMap.value(style)),
    ...
    if (!type.compare("ellipse")) {
      ...
      Ellipses.append(new qucs::Ellips(
          x, y, w, h, QPen(QColor(color), thick, penMap.value(style)),
          QBrush(QColor(colorfill), brushMap.value(stylefill))));
    }
    ...
    if (!type.compare("ellipsearc")) {
      ...
      Arcs.append(
          new qucs::Arc(x, y, w, h, angle, arclen,
    ...
    if (!type.compare("graphictext")) {
      ...
      Texts.append(new Text(x, y, s, QColor(color), size, cos, sin));
    }
...
QString vacomponent::spice_netlist(
  ...
  for (int i = 0; i < Props.count(); i++) {
    par_str +=
        QStringLiteral("%1=%2 ").arg(Props.at(i)->Name).arg(Props.at(i)->Value);
  }
