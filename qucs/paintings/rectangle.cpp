...
Element* qucs::Rectangle::info(QString& Name, char*& BitmapFile,
                               bool getNewOne) {
...
Element* qucs::Rectangle::info_filled(QString& Name, char*& BitmapFile,
                                      bool getNewOne) {
  Name       = QObject::tr("filled Rectangle");
  BitmapFile = (char*)"filledrect";

  if (getNewOne) {
    return new qucs::Rectangle(true);
  }
  return nullptr;
}
...
QString qucs::Rectangle::save() {
  QString s = Name + QString::number(x1) + " " + QString::number(y1) + " " +
              ...
              QString::number(pen.style()) + " " + brush.color().name() + " " +
              QString::number(brush.style());
  if (filled) {
    s += " 1";
  } else {
    s += " 0";
...
QString qucs::Rectangle::saveCpp() {
  QString b = filled ? QString(", QBrush (QColor (\"%1\"), %2)")
                           .arg(brush.color().name())
                           .arg(toBrushString(brush.style()))
                     : "";
  QString s = QString("new Area (%1, %2, %3, %4, "
                      "QPen (QColor (\"%5\"), %6, %7)%8)")
                  .arg(x1)
                  .arg(y1)
                  .arg(x2 - x1)
...
QString qucs::Rectangle::saveJSON() {
  QString b = filled ? QString("\"colorfill\" : \"%1\", \"stylefill\" : \"%2\"")
                           .arg(brush.color().name())
                           .arg(toBrushString(brush.style()))
                     : "";
...
void qucs::Rectangle::MouseMoving(const QPoint& onGrid, Schematic* sch,
                                  const QPoint& cursor) {
  if (isBeingDrawn) {
    x2 = onGrid.x();
    y2 = onGrid.y();
  ...
  if (filled) {
    sch->PostPaintEvent(_Line, cursor.x() + 14, cursor.y() + 6, cursor.x() + 19,
                        cursor.y() + 1, 0, 0, true);
    sch->PostPaintEvent(_Line, cursor.x() + 26, cursor.y() + 1, cursor.x() + 17,
                        cursor.y() + 10, 0, 0, true);
    sch->PostPaintEvent(_Line, cursor.x() + 29, cursor.y() + 5, cursor.x() + 24,
                        cursor.y() + 10, 0, 0, true);
  }
}
...
bool qucs::Rectangle::Dialog(QWidget* parent) {
  ...
  auto d = std::make_unique<FillDialog>(
      QObject::tr("Edit Rectangle Properties"), true, parent);
