...
ExportDialog::ExportDialog(int w, int h, int wsel, int hsel, QString filename_,
                           bool nosel_, QWidget* parent)
    : QDialog(parent) {
  ...
  connect(editFilename, SIGNAL(textChanged(QString)), this,
          SLOT(setSvg(QString)));
  ...
  connect(cbResolution, SIGNAL(toggled(bool)), editResolutionX,
          SLOT(setDisabled(bool)));
  connect(cbResolution, SIGNAL(toggled(bool)), editResolutionY,
          SLOT(setDisabled(bool)));
  connect(cbResolution, SIGNAL(toggled(bool)), cbRatio,
          SLOT(setDisabled(bool)));
  connect(cbResolution, SIGNAL(toggled(bool)), editScale,
          SLOT(setDisabled(bool)));
  connect(cbResolution, SIGNAL(toggled(bool)), this,
          SLOT(restoreOriginalWtoH()));

  connect(editResolutionX, SIGNAL(textEdited(QString)), this,
          SLOT(calcHeight()));
  connect(editResolutionY, SIGNAL(textEdited(QString)), this,
          SLOT(calcWidth()));
...
void ExportDialog::setFileName() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Export Schematic to Image"), editFilename->text(),
      "PNG images (*.png);;"
...
bool ExportDialog::isValidFilename() {
  ...
  filetypes << "png" << "svg" << "jpeg" << "jpg" << "pdf" << "pdf_tex" << "eps"
            << "PNG" << "JPG" << "SVG" << "JPEG" << "PDF" << "PDF_TEX" << "EPS";
