...
SpiceSENS_TR_Xyce::SpiceSENS_TR_Xyce() {
  ...
  Props.append(
      new Property("RefParam", "R1:R", true,
                   QObject::tr("Reference parameter for .SENS analysis")));
  Props.append(
      new Property("Mode", "direct", true,
                   QObject::tr("Analysis mode ") + "[direct,adjoint]"));
  Props.append(
      new Property("Start", "0", true, QObject::tr("start time in seconds")));
  Props.append(
      new Property("Stop", "1 ms", true, QObject::tr("stop time in seconds")));
  Props.append(
      new Property("Step", "5u", true, QObject::tr("simulation time step")));
  Props.append(new Property("initialDC", "yes", true,
                            QObject::tr("perform an initial DC analysis") +
                                " [yes, no]"));
}
...
Element* SpiceSENS_TR_Xyce::info(QString& Name, char*& BitmapFile,
                                 bool getNewOne) {
...
QString SpiceSENS_TR_Xyce::spice_netlist(
  ...
  if (dialect == spicecompat::SPICEXyce) {
    ...
    if (Props.at(6)->Value == "yes") {
      s += "\n";
    } else {
      s += " uic\n";
    }
    if (Props.at(2)->Value == "direct") {
      s += ".options sensitivity direct=1 adjoint=0\n";
    } else {
      s += ".options sensitivity direct=0 adjoint=1\n";
    }
