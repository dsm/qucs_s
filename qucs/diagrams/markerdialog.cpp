...
MarkerDialog::MarkerDialog(Marker* pm_, QWidget* parent) : QDialog(parent) {
  ...
  if (pMarker->varPos().size() > 0) {
    XPosition->setText(
        QString::number(pMarker->varPos().at(0), 'g', pMarker->precision()));
  } else {
    XPosition->setText("0");
...
void MarkerDialog::slotAcceptValues() {
  ...
  if (IndicatorBox->currentIndex() != pMarker->indicatorMode) {
    pMarker->indicatorMode =
        static_cast<indicatorMode_t>(IndicatorBox->currentIndex());
    changed = true;
  }
  ...
  if ((xpos != pMarker->powFreq()) && (pMarker->varPos().size() > 0)) {
