...
void Rect3DDiagram::calcCoordinate(const double* xD, const double* zD,
                                   const double* yD, float* px, float* py,
                                   Axis const*) const {
  double x3D = zD[0];
  double y3D = zD[1];
  double z3D;
...
void Rect3DDiagram::calcLine(tPoint3D*& p, tPoint3D*& MemEnd, tBound* Bounds,
                             char* zBuffer) {
  int Pos_;
  int x1_ = p->x, y1_ = p->y;
...
void Rect3DDiagram::removeHiddenLines(char* zBuffer, tBound* Bounds) {
  ...
  int malloc_8xsize = 8;
  tPointZ* zMem =
      (tPointZ*)malloc(malloc_8xsize * (Size + 2) * sizeof(tPointZ));
    ...
    for (int No = g->countY / dy * (dx - 1) * (dy - 1); No > 0; No--) {
      ...
      for (i = x2 - 1; i >= 0; i--) { // all x coordinates
        if ((Bounds + i)->max > INT_MIN) {
          pc = zBuffer + i * ((y2 + 7) >> 3);
          for (j = (Bounds + i)->min; j <= (Bounds + i)->max;
               j++) { // all y coordinates
            *(pc + (j >> 3)) |= (1 << (j & 7));
          }
        }
...
void Rect3DDiagram::removeHiddenCross(int x1_, int y1_, int x2_, int y2_,
  ...
  *(Mem + 1) = *Mem;
  ...
  do {
    if (((p - 1)->done & 4) == 0) {
      Lines.append(new qucs::Line((p - 1)->x, (p - 1)->y, p->x, p->y,
                                  QPen(Qt::black, 0)));
    }
    p++;
  } while (p <= pMem);
...
// --------------------------------------------------------------
int Rect3DDiagram::calcAxis(Axis* Axis, int x, int y, double xD, double phi) {
  ...
  if (Axis->log) {
    ...
    // double upD  = Axis->up;
    if (yD > 1.5 * stepD) {
      yD = 10.0 * stepD; // always start at power of 10
    }
    if (back) {
      // upD  = Axis->low;
      phi += pi;
    ...
    for (; count > 0; count--) {
      ...
      // short grid marks
      Lines.append(
          new qucs::Line(x - gx, y - gy, x + gx, y + gy, QPen(Qt::black, 0)));
      xD += xstepD;
      yD += ystepD;
    }
  } // of "if(Axis->log) ... else ..."
...
// --------------------------------------------------------------
void Rect3DDiagram::createAxis(Axis* Axis, bool Right, int x1_, int y1_,
                               int x2_, int y2_) {
  Q_UNUSED(Right);
  ...
  if (Axis->Label.isEmpty()) {
    ...
    for (Graph* pg : Graphs) {
      ...
      Texts.append(new Text(x + int(double((z - w) >> 1) * cos_phi),
                            y + int(double((z - w) >> 1) * sin_phi), s,
                            pg->Color, 12.0, cos_phi, sin_phi));
    }
  } else {
    x += int(double(metrics.lineSpacing()) * sin_phi);
    y -= int(double(metrics.lineSpacing()) * cos_phi);
    ...
    Texts.append(new Text(x + int(double((z - w) >> 1) * cos_phi),
                          y + int(double((z - w) >> 1) * sin_phi), Axis->Label,
                          Qt::black, 12.0, cos_phi, sin_phi));
  }
}
...
int Rect3DDiagram::calcDiagram() {
  ...
  if (xAxis.log) {
    if (xAxis.autoScale) {
      if (xAxis.max * xAxis.min <= 0.0) {
        goto Frame; // invalid
      }
    } else if (xAxis.limit_min * xAxis.limit_max <= 0.0) {
      goto Frame; // invalid
    }
  ...
  if (yAxis.log) {
    if (yAxis.autoScale) {
      if (yAxis.max * yAxis.min <= 0.0) {
        goto Frame; // invalid
      }
    } else if (yAxis.limit_min * yAxis.limit_max <= 0.0) {
      goto Frame; // invalid
    }
  ...
  if (zAxis.log) {
    if (zAxis.autoScale) {
      if (zAxis.max * zAxis.min <= 0.0) {
        goto Frame; // invalid
      }
    } else if (zAxis.limit_min * zAxis.limit_max <= 0.0) {
      goto Frame; // invalid
    }
  ...
  // xy area
  Lines.append(new qucs::Line(X[o ^ 1], Y[o ^ 1], X[o ^ 3], Y[o ^ 3],
                              QPen(Qt::black, 0)));
  Lines.append(new qucs::Line(X[o ^ 2], Y[o ^ 2], X[o ^ 3], Y[o ^ 3],
                              QPen(Qt::black, 0)));

  // yz area
  Lines.append(new qucs::Line(X[o ^ 2], Y[o ^ 2], X[o ^ 6], Y[o ^ 6],
                              QPen(Qt::black, 0)));
  Lines.append(new qucs::Line(X[o ^ 4], Y[o ^ 4], X[o ^ 6], Y[o ^ 6],
                              QPen(Qt::black, 0)));

  // xz area
  Lines.append(new qucs::Line(X[o ^ 1], Y[o ^ 1], X[o ^ 5], Y[o ^ 5],
                              QPen(Qt::black, 0)));
  Lines.append(new qucs::Line(X[o ^ 4], Y[o ^ 4], X[o ^ 5], Y[o ^ 5],
                              QPen(Qt::black, 0)));

  // =====  create axis  =============================================
  if (X[o ^ 1] < X[o ^ 2]) {
    w = 2; // use which z axis ?
  } else {
    w = 1;
  }
  ...
  if (hideLines) {
    ...
    removeHiddenCross(X[o ^ 1], Y[o ^ 1], X[o], Y[o], zBuffer,
                      Bounds); // x axis
    removeHiddenCross(X[o ^ 2], Y[o ^ 2], X[o], Y[o], zBuffer,
                      Bounds); // y axis
    removeHiddenCross(X[o ^ 4], Y[o ^ 4], X[o], Y[o], zBuffer,
                      Bounds); // z axis
    ...
    Lines.append(
        new qucs::Line(X[o], Y[o], X[o ^ 1], Y[o ^ 1], QPen(Qt::black, 0)));
    Lines.append(
        new qucs::Line(X[o], Y[o], X[o ^ 2], Y[o ^ 2], QPen(Qt::black, 0)));
    Lines.append(
        new qucs::Line(X[o], Y[o], X[o ^ 4], Y[o ^ 4], QPen(Qt::black, 0)));
  }
