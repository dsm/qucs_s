...
Capacitor::Capacitor() {
  ...
  Props.append(
      new Property("C", "1 nF", true, QObject::tr("capacitance in Farad")));
  Props.append(new Property(
      "V", "", false, QObject::tr("initial voltage for transient simulation")));
  Props.append(
      new Property("Symbol", "neutral", false,
...
QString Capacitor::va_code() {
  ...
  if (Vpm.startsWith("(-")) {
    Vpm.remove(1, 1); // Make capacitor unipolar, remove starting minus
  }
...
void Capacitor::createSymbol() {
  if (Props.back()->Value.at(0) == 'n') {
    ...
    Arcs.append(new qucs::Arc(4, -12, 20, 24, 16 * 122, 16 * 116,
                              QPen(Qt::darkBlue, 3)));
  }
