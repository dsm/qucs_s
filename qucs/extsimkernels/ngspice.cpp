...
Ngspice::Ngspice(Schematic* schematic, QObject* parent)
    : AbstractSpiceKernel(schematic, parent), a_spinit_name() {
  if (QFileInfo(QucsSettings.NgspiceExecutable)
          .isRelative()) { // this check is related to MacOS
    a_simulator_cmd =
        QFileInfo(QucsSettings.BinDir + QucsSettings.NgspiceExecutable)
            .absoluteFilePath();
  } else {
    a_simulator_cmd =
        QFileInfo(QucsSettings.NgspiceExecutable).absoluteFilePath();
  }
 ...
 */
void Ngspice::createNetlist(QTextStream& stream, QStringList& simulations,
  ...
  stream << collectSpiceLibs(
      a_schematic); // collect libraries on the top of netlist
  if (!prepareSpiceNetlist(stream)) {
    return; // Unable to perform spice simulation
  }
  ...
  if (QucsMain != nullptr) { // if not run from CLI
    if (!QucsMain->ProjName.isEmpty()) {
      ...
      osdi_ext << "*.osdi";
      QStringList osdi_files =
          QucsSettings.QucsWorkDir.entryList(osdi_ext, QDir::Files);
      for (const auto& file : osdi_files) {
        QString abs_file =
            QucsSettings.QucsWorkDir.absolutePath() + QDir::separator() + file;
  ...
  for (Component* pc : a_schematic->a_DocComps) {
    ...
    for (Component* pc1 : a_schematic->a_DocComps) {
      if (pc1->isActive != COMP_IS_ACTIVE) {
        continue;
      }
      if (pc1->Model == "Eqn") {
        spiceNetlist.append(
            (reinterpret_cast<Equation*>(pc1))->getNgspiceScript());
      }
    }
      ...
      if (sim_typ == ".SW") {
        ...
        if (SwpSim == sim_name) {
          ...
          if (!sim_name.startsWith("dc")) {
            spiceNetlist.append(
                getParentSWPscript(pc1, sim_name, true, hasDblSWP));
    ...
    if (sim_typ == ".AC") {
      ...
      QRegularExpression ac_rx("^\\s*ac\\s.*",
                               QRegularExpression::CaseInsensitiveOption);
      QRegularExpression sp_rx("^\\s*sp\\s.*",
                               QRegularExpression::CaseInsensitiveOption);
      QRegularExpression noise_rx("^\\s*noise\\s.*",
                                  QRegularExpression::CaseInsensitiveOption);
      QRegularExpression disto_rx("^\\s*disto\\s.*",
                                  QRegularExpression::CaseInsensitiveOption);
      QRegularExpression fft_rx("^\\s*fft\\s.*",
                                QRegularExpression::CaseInsensitiveOption);
      QRegularExpression four_rx("^\\s*(four|fourier)\\s.*",
                                 QRegularExpression::CaseInsensitiveOption);
      QRegularExpression dc_rx("^\\s*dc\\s.*",
                               QRegularExpression::CaseInsensitiveOption);
      QRegularExpression op_rx("^\\s*op\\s*",
                               QRegularExpression::CaseInsensitiveOption);
      QRegularExpression tran_rx("^\\s*tran\\s.*",
                                 QRegularExpression::CaseInsensitiveOption);
      QRegularExpression sens_ac_rx("^\\s*sens\\s.*ac\\s.*",
                                    QRegularExpression::CaseInsensitiveOption);
      QRegularExpression sens_dc_rx("^\\s*sens\\s.*",
                                    QRegularExpression::CaseInsensitiveOption);
      QRegularExpression pz_rx("^\\s*pz\\s.*",
                               QRegularExpression::CaseInsensitiveOption);

      QStringList lines = pc->getSpiceNetlist().split('\n');
      for (const QString& line : lines) {
        if (ac_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (sp_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (noise_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (disto_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (fft_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (four_rx.match(line).hasMatch()) {
          fourSims++;
        } else if (dc_rx.match(line).hasMatch()) {
          dcSims++;
        } else if (op_rx.match(line).hasMatch()) {
          dcSims++;
        } else if (tran_rx.match(line).hasMatch()) {
          timeSims++;
        } else if (sens_ac_rx.match(line).hasMatch()) {
          freqSims++;
        } else if (sens_dc_rx.match(line).hasMatch()) {
          dcSims++;
        } else if (pz_rx.match(line).hasMatch()) {
          pzSims++;
        }
      }
      ...
      if (hasParSWP) { // Set necessary plot number to output Noise spectrum
        ...
        spiceNetlist.append(
            QStringLiteral("let noise_%1 = 2*%1+1\n").arg(cnt_var));
        spiceNetlist.append(
            QStringLiteral("setplot noise$&noise_%1\n").arg(cnt_var));
    ...
    if (sim_typ == ".DC") {
      ...
      outputs.append(out);
    } else if ((sim_typ != ".PZ") && (sim_typ != ".SENS") &&
               (sim_typ != ".SENS_AC")) {
      ...
      if (!nods.isEmpty()) {
        ...
        if (hasParSWP && hasDblSWP) {
          filename =
              QStringLiteral("%1.%2._swp_swp.plot").arg(basenam).arg(sim_name);
        } else if (hasParSWP) {
          filename =
              QStringLiteral("%1.%2._swp.plot").arg(basenam).arg(sim_name);
        } else {
          filename = QStringLiteral("%1.%2.plot").arg(basenam).arg(sim_name);
        }
        filename.replace(' ',
                         '_'); // Ngspice cannot understand spaces in filename
        spiceNetlist.append(
      ...
      if (sim_typ == ".SW") {
        ...
        if (SwpSim == sim_name) {
          if (!sim_name.startsWith("dc")) {
            ...
            spiceNetlist.append(
                getParentSWPscript(pc1, sim_name, false, hasDblSWP));
          }
        }
      }
    }
 ...
 * \return
 */
QString Ngspice::getParentSWPscript(Component* pc_swp, QString sim, bool before,
                                    bool& hasDblSwp) {
  hasDblSwp   = false;
...
/*!
 * \brief Ngspice::slotSimulate Create netlist and execute Ngspice simulator.
 * Netlist is saved at $HOME/.qucs/spice4qucs/spice4qucs.cir
 */
void Ngspice::slotSimulate() {
  ...
  if (!checkSchematic(incompat)) {
    ...
    a_output.append(
        "There were SPICE-incompatible components. Simulator cannot proceed.");
  ...
  if (!checkGround()) {
    a_output.append(
        "No Ground found. Please add at least one ground!\n"
  ...
  if (!checkSimulations()) {
    a_output.append(
        "No simulation found. Please add at least one simulation!\n"
        "Navigate to the \"simulations\" group in the components panel (left)"
        " and drag simulation to the schematic sheet. Then define its "
        "parameters.\n");
    checker_error = true;
  }
  ...
  if (!checkDCSimulation()) {
    a_output.append(
        "Only DC simulation found in the schematic. It has no effect!"
  ...
  if (!checkNodeNames(incompat)) {
    QString s = incompat.join("; ");
    a_output.append("There were Nutmeg-incompatible node names. Simulator "
                    "cannot proceed.\n");
  ...
  QString tmp_path =
      QDir::toNativeSeparators(a_workdir + QDir::separator() + netfile);
  ...
  qDebug() << a_workdir;
  QString cmd = QStringLiteral("\"%1\" %2 %3")
                    .arg(a_simulator_cmd, a_simulator_parameters, netfile);
...
/*!
 * \brief Ngspice::checkNodeNames Check schematic node names on reserved Nutmeg
 * keywords. \param incompat \return
 */
 ...
 *        contains math.functions definitions for Ngspice. It's need to let to
 * simulate circuit if it is not found. \param mathf_inc[out] The filename of
 * include file \return True if found. False otherwise
 */
bool Ngspice::findMathFuncInc(QString& mathf_inc) {
  QDir qucs_root(QucsSettings.BinDir);
  qucs_root.cdUp();
  mathf_inc = QStringLiteral("%1/share/" QUCS_NAME
                             "/xspice_cmlib/include/ngspice_mathfunc.inc")
                  .arg(qucs_root.absolutePath());
  return QFile::exists(mathf_inc);
}

/*!
 * \brief Ngspice::slotProcessOutput Process Ngspice output and report
...
/*!
 * \brief Ngspice::SaveNetlist Create netlist and save it to file without
 * execution of simulator. \param[in] filename Absolute path to netlist
...
void Ngspice::SaveNetlist(QString filename, bool netlist2Console) {
  ...
  if (netlist2Console) {
    ...
    if (netlistFile->open(QFile::WriteOnly)) {
      ...
      QString final_msg =
          QStringLiteral(
              "%1\n This could be an error in the QSettings settings "
              "file\n(usually in ~/.config/qucs/qucs_s.conf)\nThe value for "
              "S4Q_workdir (default:/spice4qucs) needs to be writeable!\nFor a "
              "Simulation Simulation will raise error! (most likely "
              "S4Q_workdir does not exists)")
              .arg(msg);
      QMessageBox::critical(nullptr, tr("Problem with SaveNetlist"), final_msg,
                            QMessageBox::Ok);
      return;
    }
  }
...
void Ngspice::setSimulatorCmd(QString cmd) {
  if (cmd.contains(QRegularExpression("spiceopus(....|)$"))) {
    // spiceopus needs English locale to produce correct decimal point (dot
    // symbol)
...
void Ngspice::createSpiceinit(const QString& initial_spiceinit) {
  ...
  if (initial_spiceinit.isEmpty() && compat_str.isEmpty()) {
    return;
  }
