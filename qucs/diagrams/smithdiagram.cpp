...
/*!
  \class SmithDiagram
  \brief The SmithDiagram class implements the Impedance and Admittance Smith
  diagram
*/
...
SmithDiagram::SmithDiagram(int _cx, int _cy, bool ImpMode) : Diagram(_cx, _cy) {
  ...
  if (ImpMode) {
    Name = "Smith"; // with impedance circles
  } else {
    Name = "ySmith"; // with admittance circles
  }

  Arcs.append(
      new struct qucs::Arc(0, y2, x2, y2, 0, 16 * 360, QPen(Qt::black, 0)));
...
void SmithDiagram::calcCoordinate(const double*, const double* yD,
...
// ------------------------------------------------------------
Element* SmithDiagram::info_y(QString& Name, char*& BitmapFile,
                              bool getNewOne) {
  Name       = QObject::tr("Admittance Smith");
  BitmapFile = (char*)"ysmith";

  if (getNewOne) {
    return new SmithDiagram(0, 0, false);
  }
  return 0;
}
...
QString SmithDiagram::extraMarkerText(Marker const* m) const {
  ...
  if (Var.startsWith("S")) { // uuh, ooh hack.
    return "\n" + Var.replace('S', 'Z') + ": " +
           misc::complexRect(Zr, Zi, Precision);
  } else {
