...
QucsApp::QucsApp(bool netlist2Console) : a_netlist2Console(netlist2Console) {
  ...
  QucsFileFilter =
      tr("Schematic") + " (*.sch);;" + tr("Symbol only") + " (*.sym);;" +
      tr("Data Display") + " (*.dpl);;" + tr("Qucs Documents") +
      " (*.sch *.dpl);;" + tr("VHDL Sources") + " (*.vhdl *.vhd);;" +
      tr("Verilog Sources") + " (*.v);;" + tr("Verilog-A Sources") +
      " (*.va);;" + tr("Octave Scripts") + " (*.m *.oct);;" +
      tr("Spice Files") + QStringLiteral(" (") +
      QucsSettings.spiceExtensions.join(" ") + QStringLiteral(");;") +
      tr("Any File") + " (*)";
  ...
  if (QucsSettings.firstRun) { // try to find Ngspice
    ...
    QString ngspice_exe1 =
        QStandardPaths::findExecutable(ngspice_exe_name, {"/opt/homebrew/bin"});
#else
    QString ngspice_exe1 = QStandardPaths::findExecutable(
        ngspice_exe_name, {QucsBinDir.absolutePath()});
#endif
    ...
    /* search own path */
    QString qucsator_exe1 = QStandardPaths::findExecutable(
        "qucsator_rf", {QucsBinDir.absolutePath()});
    /* search system path */
    ...
    if (ngspice_found || qucsator_found) {
      ...
      QMessageBox::information(
          this, tr("Qucs"),
          tr("No simulators found automatically. Please specify simulators"
    ...
    if (!QucsSettings.Qucsator.contains("qucsator_rf")) {
      QucsSettings.Qucsator = QStandardPaths::findExecutable(
          "qucsator_rf", {QucsBinDir.absolutePath()});

      if (!QucsSettings.Qucsator.isEmpty()) {
        QMessageBox::information(this, "Qucs",
                                 tr("QucsatorRF found at: ") +
...
void QucsApp::initView() {
...
#ifndef __APPLE__
  setWindowIcon(
      QPixmap(QStringLiteral(":/bitmaps/hicolor/scalable/apps/qucs.svg")));
#else
  ...
  connect(DocumentTab, SIGNAL(currentChanged(int)), SLOT(slotChangeView()));

  // Give every tab a close button, and connect the button's signal to
  // slotFileClose
  DocumentTab->setTabsClosable(true);
  connect(DocumentTab, SIGNAL(tabCloseRequested(int)),
          SLOT(slotFileClose(int)));
  ...
  TabView->setTabToolTip(TabView->indexOf(ProjGroup),
                         tr("content of project directory"));

  connect(Projects, SIGNAL(doubleClicked(const QModelIndex&)), this,
          SLOT(slotListProjOpen(const QModelIndex&)));
  ...
  TabView->setTabToolTip(TabView->indexOf(Content),
                         tr("content of current project"));
  ...
  simulatorsCombobox->setSizePolicy(QSizePolicy::Preferred,
                                    QSizePolicy::Preferred);
  connect(simulatorsCombobox, SIGNAL(activated(int)),
          SLOT(slotChangeSimulator(int)));
  ...
  TabView->setTabToolTip(TabView->indexOf(CompGroup),
                         tr("components and diagrams"));

  connect(CompChoose, SIGNAL(activated(int)), SLOT(slotSetCompView(int)));
  connect(CompComps, SIGNAL(itemActivated(QListWidgetItem*)),
          SLOT(slotSelectComponent(QListWidgetItem*)));
  connect(CompComps, SIGNAL(itemPressed(QListWidgetItem*)),
          SLOT(slotSelectComponent(QListWidgetItem*)));
  connect(CompSearch, SIGNAL(textEdited(const QString&)),
          SLOT(slotSearchComponent(const QString&)));
  ...
  connect(LibCompSearch, SIGNAL(textEdited(const QString&)),
          SLOT(slotSearchLibComponent(const QString&)));
  ...
  TabView->setTabToolTip(TabView->indexOf(CompGroup),
                         tr("system and user component libraries"));
  ...
  connect(octDock, SIGNAL(visibilityChanged(bool)),
          SLOT(slotToggleOctave(bool)));
...
void QucsApp::fillLibrariesTreeView() {
  ...
  QTreeWidgetItem* newitem =
      new QTreeWidgetItem((QTreeWidget*)0, QStringList("System Libraries"));
  ...
  QString UserLibDirPath =
      QucsSettings.qucsWorkspaceDir.canonicalPath() + "/user_lib/";
  populateLibTreeFromDir(UserLibDirPath, topitems);

  // make the user libraries section header
  newitem =
      new QTreeWidgetItem((QTreeWidget*)0, QStringList("Project Libraries"));
  newitem->setChildIndicatorPolicy(QTreeWidgetItem::DontShowIndicator);
  newitem->setFont(0, sectionFont);
  topitems.append(newitem);
  if (!ProjName.isEmpty()) {
...
bool QucsApp::populateLibTreeFromDir(const QString& LibDirPath,
                                     QList<QTreeWidgetItem*>& topitems,
                                     bool relpath) {
  QDir LibDir(LibDirPath);
  QStringList LibFiles =
      LibDir.entryList(QStringList("*.lib"), QDir::Files, QDir::Name);
  ...
  for (auto it = LibFiles.begin(); it != LibFiles.end(); it++) {
    ...
    int result = parseComponentLibrary(libPath, parsedlibrary,
                                       QUCS_COMP_LIB_FULL, relpath);
    QStringList nameAndFileName;
    nameAndFileName.append(parsedlibrary.name);
    nameAndFileName.append(LibDirPath + *it);

    QTreeWidgetItem* newlibitem =
        new QTreeWidgetItem((QTreeWidget*)nullptr, nameAndFileName);
...
int QucsApp::fillComboBox(bool setAll) {
  ...
  // CompChoose->setMaxVisibleItems (13); // Increase this if you add items
  // below.
...
void QucsApp::fillSimulatorsComboBox() {
  ...
  // simulatorsCombobox->addItem(spicecompat::getDefaultSimulatorName(spicecompat::simNotSpecified),
  // 0);

  if (misc::simulatorExists(QucsSettings.NgspiceExecutable)) {
    QucsSettings.NgspiceExecutable =
        misc::unwrapExePath(QucsSettings.NgspiceExecutable);
    simulatorsCombobox->addItem(
        spicecompat::getDefaultSimulatorName(spicecompat::simNgspice), 1);
  }
  if (misc::simulatorExists(QucsSettings.XyceExecutable)) {
    QucsSettings.XyceExecutable =
        misc::unwrapExePath(QucsSettings.XyceExecutable);
    simulatorsCombobox->addItem(
        spicecompat::getDefaultSimulatorName(spicecompat::simXyce), 2);
  }
  if (misc::simulatorExists(QucsSettings.SpiceOpusExecutable)) {
    QucsSettings.SpiceOpusExecutable =
        misc::unwrapExePath(QucsSettings.SpiceOpusExecutable);
    simulatorsCombobox->addItem(
        spicecompat::getDefaultSimulatorName(spicecompat::simSpiceOpus), 4);
  }
  if (misc::simulatorExists(QucsSettings.Qucsator)) {
    QucsSettings.Qucsator = misc::unwrapExePath(QucsSettings.Qucsator);
    simulatorsCombobox->addItem(
        spicecompat::getDefaultSimulatorName(spicecompat::simQucsator), 8);
  }
  ...
  if (anySimulatorsFound) {
    ...
    QMessageBox::critical(
        ...
        tr("No simulation backend found! Simulaiton not possible.\n"
           ...
           "1. You are using portable version and have moved installation "
           "directory.\n"
           "2. The simulators were removed from system or the paths configured "
           "wrong.\n\n"
           "Please configure simulator paths in Simulation->Simulator settings "
           "menu.\n"));
  }
...
void QucsApp::slotSetCompView(int index) {
  ...
  if (item == QObject::tr("verilog-a user devices")) {
    ...
    while (i.hasNext()) {
      ...
      QString vaBitmap;
      Component* c =
          (Component*)vacomponent::info(Name, vaBitmap, false, i.value());
      if (c) {
        delete c;
      }
...
void QucsApp::slotSearchComponent(const QString& searchText) {
  ...
  if (searchText.isEmpty()) {
    ...
    CompChoose->setCurrentIndex(
        0); // make sure the "Search results" category is selected
    editText->setHidden(true); // disable text edit of component property
    ...
    int catIdx       = 0;
    for (const QString& it : cats) {
      // this will go also over the "verilog-a user devices" category, if
      // present
...
void QucsApp::slotSearchClear() {
  ...
  if (CompChoose->itemText(0) == tr("Search results")) {
    ...
    // the added "Search results" panel text will be removed by
    // slotSetCompView()
  }
}
...
void QucsApp::slotSelectComponent(QListWidgetItem* item) {
  ...
  if (CompChoose->itemText(0) == tr("Search results")) {
    if (Infos || InfosVA) {
      ...
      CompChoose->setCurrentIndex(
          catIdx + 1);   // +1 due to the added "Search Results" item
...
#undef APPEND_MENU
  connect(Content, SIGNAL(customContextMenuRequested(const QPoint&)),
          SLOT(slotShowContentMenu(const QPoint&)));
}
...
void QucsApp::slotCMenuCopy() {
  ...
  bool ok;
  QString s =
      QInputDialog::getText(this, tr("Copy file"), tr("Enter new name:"),
  ...
  if (ok && !s.isEmpty()) {
    ...
    if (QFile::exists(dir.filePath(s))) { // check New Name exists
      QMessageBox::critical(
          this, tr("error"),
          tr("Cannot copy file to identical name: %1").arg(filename));
      return;
    }

    if (!QFile::copy(dir.filePath(filename), dir.filePath(s))) {
      QMessageBox::critical(this, tr("Error"),
                            tr("Cannot copy schematic: %1").arg(filename));
      return;
    }
...
void QucsApp::slotCMenuRename() {
  ...
  bool ok;
  QString s =
      QInputDialog::getText(this, tr("Rename file"), tr("Enter new filename:"),
                            QLineEdit::Normal, base, &ok);
  ...
  if (ok && !s.isEmpty()) {
    ...
    if (!dir.rename(filename, s)) {
      QMessageBox::critical(this, tr("Error"),
                            tr("Cannot rename file: %1").arg(filename));
      return;
    }
...
void QucsApp::slotCMenuDelete() {
  ...
  if (filesToDelete.size() > 1) {
    message = tr("This will delete %1 files permanently! Continue?")
                  .arg(filesToDelete.size());
  } else {
    message = tr("This will delete the file permanently! Continue?");
  }

  int response = QMessageBox::warning(this, tr("Warning"), message,
  ...
  if (!allDeleted) {
    QString errorMsg =
        tr("Could not delete the following files:\n") + failedFiles.join("\n");
...
void QucsApp::openProject(const QString& Path) {
  // this will also remove the path from recent projects if the directory
  // doesn't exist.
  ...
  if (!closeAllFiles()) {
    return; // close files and ask for saving them
  }
  Schematic* d = new Schematic(this, "");
  int i        = addDocumentTab(d);
  DocumentTab->setCurrentIndex(i);
...
void QucsApp::slotMenuProjClose() {
  ...
  slotResetWarnings();
  setWindowTitle(windowTitle);
  QucsSettings.QucsWorkDir.setPath(
      QucsSettings.qucsWorkspaceDir.absolutePath());
  octave->adjustDirectory();
...
bool QucsApp::deleteProject(const QString& Path) {
  ...
  if (QMessageBox::warning(this, tr("Warning"),
                           tr("This will destroy all the project files "
...
void QucsApp::slotFileOpen() {
  ...
  QString s = QFileDialog::getOpenFileName(
      ...
      lastDirOpenSave.isEmpty() ? QString(QDir::homePath()) : lastDirOpenSave,
      QucsFileFilter);
...
bool QucsApp::saveAs() {
  ...
  while (true) {
    ...
    s = QFileDialog::getSaveFileName(this, tr("Enter a Document Name"), s,
                                     Filter, &selfilter);
    if (s.isEmpty()) {
      return false;
    }
...
void QucsApp::slotFileSaveAll() {
  ...
  while ((Doc = getDoc(No++)) != nullptr) {
    ...
    if (saveFile(
            Doc)) { // Hack! TODO: Maybe it's better to let slotFileChanged()
...
void QucsApp::slotHelpTutorial() {
  ...
  QUrl url = QUrl::fromLocalFile(QDir::cleanPath(
      QucsSettings.DocDir + "/tutorial/" + QObject::sender()->objectName()));
...
void QucsApp::slotHelpTechnical() {
  // pass the QUrl representation of a local file
  QUrl url = QUrl::fromLocalFile(QDir::cleanPath(
      QucsSettings.DocDir + "/technical/" + QObject::sender()->objectName()));
  QDesktopServices::openUrl(url);
}
...
void QucsApp::slotHelpReport() {
  // pass the QUrl representation of a local file
  QUrl url = QUrl::fromLocalFile(QDir::cleanPath(
      QucsSettings.DocDir + "/report/" + QObject::sender()->objectName()));
  QDesktopServices::openUrl(url);
}
...
void QucsApp::slotChangeView() {
  ...
  //    QDir parentDir = QucsSettings.QucsWorkDir;
  //    parentDir.cdUp();
  //    setWindowTitle(tr("Project: ") + ProjName + " (" +
  //    parentDir.absolutePath() + ") "
  //                   + windowTitle);
  //  } else {
  //    setWindowTitle(Info.fileName() + " (" + Info.filePath() +") - " +
  //    windowTitle);
  //  }
...
void QucsApp::slotRefreshSchPath() {
  ...
  statusBar()->showMessage(tr("The schematic search path has been refreshed."),
                           2000);
}
...
void QucsApp::updatePortNumber(QucsDoc* currDoc, int No) {
  ...
  while ((w = DocumentTab->widget(No++)) != nullptr) {
    ...
    std::ranges::copy_if((*Doc->a_Components), std::back_inserter(to_recreate),
                         needToRecreate);
    std::ranges::for_each(to_recreate,
                          [Doc](Component* c) { Doc->recreateComponent(c); });
  }
}
...
int QucsApp::addDocumentTab(QFrame* widget, const QString& title) {
  int index =
      DocumentTab->addTab(widget, title.isEmpty() ? tr("untitled") : title);
#if __APPLE__
...
#ifdef __APPLE__
  ((QLabel*)DocumentTab->tabBar()->tabButton(index, QTabBar::RightSide))
      ->setText(changed ? "\u26AB" : " ");
#else
  if (changed) {
...
void QucsApp::slotTune(bool checked) {
  if (checked) {
    QWidget* w =
        DocumentTab
            ->currentWidget(); // remember from which Tab the tuner was started
    if (isTextDocument(w)) {
      // Probably digital Simulation
      QMessageBox::warning(
          this, "Not implemented",
          "Currently tuning is not supported for this document type",
          QMessageBox::Ok);
    ...
    if (!found) {
      QMessageBox::warning(this, tr("Error"),
                           tr("No simulations found. Tuning not possible."
                              " Please add at least one simulation."));
      exit = true;
    }
    if (digi_found) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Tuning not possible for digital simulation. "
                              "Only analog simulation supported."));
    }
    if (d->a_Diagrams->empty() && !d->getSimOpenDpl()) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Tuning has no effect without diagrams. "
    ...
    // instance of tuner
    TuningMode = true;
    tunerDia   = new TunerDialog(
        w, this); // The object can be instantiated here since when checked ==
                  // false the memory will be freed
    // inform the Tuner Dialog when a component is deleted
    connect(d, SIGNAL(signalComponentDeleted(Component*)), tunerDia,
            SLOT(slotComponentDeleted(Component*)));

    slotHideEdit(); // disable text edit of component property
    simulateToolbar->setEnabled(
        false); // disable workToolbar to preserve TuneMouseAction

    MousePressAction = &MouseActions::MPressTune;
    MouseReleaseAction =
        nullptr; // While Tune is active release is not needed. This puts Press
                 // Action back to normal select
    ...
    tunerDia
        ->close(); // According to QWidget documentation
                   // (http://doc.qt.io/qt-4.8/qwidget.html#close), the object
                   // is removed since it has the Qt::WA_DeleteOnClose flag
    TuningMode = false;
  }
}
...
QWidget* QucsApp::getSchematicWidget(QucsDoc* Doc) {
  ...
  QString sch_name =
      sch_inf.absolutePath() + QDir::separator() + Doc->getDataDisplay();
  QucsDoc* d =
      findDoc(sch_name, &z); // check if schematic is already open in a Tab
  ...
  if (d) {
    ...
    if (Info.suffix() == "sch" || Info.suffix() == "dpl" ||
      ...
      i = DocumentTab->addTab((Schematic*)d, QPixmap(":/bitmaps/empty.xpm"),
                              Info.fileName());
    } else {
      d = new TextDoc(this, Info.absoluteFilePath());
      i = DocumentTab->addTab((TextDoc*)d, QPixmap(":/bitmaps/empty.xpm"),
                              Info.fileName());
    }
    DocumentTab->setCurrentIndex(
        i); // temporarily switch to the newly created Tab
    ...
    if (d->load()) {
      ...
      // load() above has already shown a QMessageBox about not being able to
      // load the file
      delete d;
      DocumentTab->setCurrentIndex(No);
    }
...
void QucsApp::slotSimulate(QWidget* w) {
  ...
  connect(sim, SIGNAL(displayDataPage(QString&, QString&)), this,
          SLOT(slotChangePage(QString&, QString&)));

  if (TuningMode == true) {
    connect(sim, SIGNAL(progressBarChanged(int)), tunerDia,
  ...
  if (!sim->startProcess()) {
    if (TuningMode == true) {
      sim->show(); // The message window is hidden when the tuning mode is
                   // active, but in case of error such window pops up
    }
    return;
  }
...
void QucsApp::slotAfterSimulation(int Status, SimMessage* sim) {
  ...
  // Kill the simulation process, otherwise we have 200+++ sims in the
  // background
  if (TuningMode) {
    sim->slotClose();
    tunerDia->SimulationEnded();
...
void QucsApp::slotChangePage(const QString& DocName,
                             const QString& DataDisplay) {
  ...
  if (d) {
    ...
    if (file.open(QIODevice::ReadOnly)) { // try to load document
      ...
      if (file.open(
              QIODevice::ReadWrite)) { // if document doesn't exist, create
 ...
 */
void QucsApp::openFileFromProjectView(const QFileInfo& Info,
                                      const QString& note) {
  ...
  if (extName == "sch" || extName == "dpl" || extName == "vhdl" ||
      extName == "vhd" || extName == "v" || extName == "va" || extName == "m" ||
      extName == "oct" || extName == "net") {
  ...
  if (extName == "s1p" || extName == "s2p" || extName == "s3p" ||
      extName == "s4p") {
    QStringList args;
    args << absolutePath;
...
void QucsApp::slotSelectSubcircuit(const QModelIndex& idx) {
  ...
  QString tab_titl = "";
  if (idx_pag >= 0) {
    tab_titl = DocumentTab->tabText(idx_pag);
  }
  if (filename == tab_titl) {
    return; // Forbid to paste subcircuit into itself.
  }
...
void QucsApp::slotSymbolEdit() {
  ...
  if (isTextDocument(w)) {
    ...
    if (!TDoc->getDocName().endsWith(".va")) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Symbol editing supported only for schematics "
                              "and Verilog-A documents!"));
      return;
    } else {
      QMessageBox::warning(this, tr("Warning"),
                           tr("Attaching symbols to Verilog-A sources is "
                              "deprecated and not recommended "
                              "for new designs. Use SPICE generic device "
...
void QucsApp::slotPowerMatching() {
  ...
  if (view->focusElement->Type != isMarker) {
    return;
  }
  ...
  Dia->setTwoPortMatch(false); // will also cause the corresponding impedance
                               // LineEdit to be updated
...
void QucsApp::slotEditElement() {
  if (view->focusMEvent) {
    view->editElement((Schematic*)DocumentTab->currentWidget(),
                      view->focusMEvent);
  }
}
...
void QucsApp::updateSchNameHash(void)
{
    ...
    for (const QString& qucspath : qucsPathList) {
        ...
        QFileInfoList schfilesList = thispath.entryInfoList( nameFilter,
QDir::Files );
        // put each one in the hash table with the unique key the base name of
        // the file, note this will overwrite the value if the key already
exists for (const QFileInfo& schfile : schfilesList) {
            schNameHash[schfile.completeBaseName()] =
schfile.absoluteFilePath();
        }
    }
...
void QucsApp::updateSpiceNameHash()
{
    ...
    for (const QString& qucspath : qucsPathList) {
        QDir thispath(qucspath);
        // get all the schematic files in the directory
        QFileInfoList spicefilesList = thispath.entryInfoList(
QucsSettings.spiceExtensions, QDir::Files );
        // put each one in the hash table with the unique key the base name of
        // the file, note this will overwrite the value if the key already
exists for (const QFileInfo& spicefile : spicefilesList) {
            schNameHash[spicefile.completeBaseName()] =
spicefile.absoluteFilePath();
        }
    }
...
QucsSettings.spiceExtensions, QDir::Files );
    ...
    for (const QFileInfo& spicefile : spicefilesList) {
        spiceNameHash[spicefile.completeBaseName()] =
spicefile.absoluteFilePath();
    }
}
*/
...
void QucsApp::slotSimulateWithSpice() {
  if (!isTextDocument(DocumentTab->currentWidget())) {
    Schematic* schematic(
        dynamic_cast<Schematic*>(DocumentTab->currentWidget()));
    if (TuningMode) {
    ...
    connect(SimDlg, SIGNAL(simulated(ExternSimDialog*)), this,
            SLOT(slotAfterSpiceSimulation(ExternSimDialog*)));
    ...
    /*disconnect(SimDlg, SIGNAL(simulated()), this,
    SLOT(slotAfterSpiceSimulation())); disconnect(SimDlg, SIGNAL(warnings()),
    this, SLOT(slotShowWarnings())); disconnect(SimDlg, SIGNAL(success()), this,
    SLOT(slotResetWarnings()));*/
    /*if (SimDlg->wasSimulated && schematic->getSimOpenDpl())
        if (schematic->getShowBias() < 1)
    slotChangePage(schematic->getDocName(), schematic->getDataDisplay()); delete
    SimDlg;*/
  } else {
    QMessageBox::warning(this, tr("Simulate schematic"),
  ...
  if (!isTextDocument(DocumentTab->currentWidget())) {
    ...
    if (a_netlist2Console) {
      QString netlistString;
      {
        ...
        if (!cdlWriter.write()) {
          QMessageBox::critical(this, tr("Save CDL netlist"),
...
void QucsApp::slotAfterSpiceSimulation(ExternSimDialog* SimDlg) {
  ...
  disconnect(SimDlg, SIGNAL(simulated(ExternSimDialog*)), this,
             SLOT(slotAfterSpiceSimulation(ExternSimDialog*)));
  ...
  if (sch->getShowBias() > 0 || QucsMain->TuningMode) {
    SimDlg->close();
  }
}
...
void QucsApp::slotBuildVAModule() {
  if (!isTextDocument(DocumentTab->currentWidget())) {
    ...
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save Verilog-A module"),
        inf.path() + QDir::separator() + "testmodule.va", "Verilog-A (*.va)");
    if (filename.isEmpty()) {
      return;
    }
...
void QucsApp::slotShowModel() {
  DisplayDialog* dlg =
      new DisplayDialog(this, Symbol->ModelString, Symbol->SpiceString);
  dlg->exec();
  delete dlg;
}
...
bool QucsSortFilterProxyModel::lessThan(const QModelIndex& left,
                                        const QModelIndex& right) const {
  // if sorting by file names column
  if (sortColumn() == 0) {
    QucsFileSystemModel* model =
        qobject_cast<QucsFileSystemModel*>(sourceModel());
...
ContextMenuTabWidget::ContextMenuTabWidget(QucsApp* parent)
    : QTabWidget(parent) {
  App = parent;
...
#define APPEND_MENU(action, slot, text)                                        \
  {                                                                            \
    ...
    APPEND_MENU(ActionCxMenuCloseOthers, slotCxMenuCloseOthers,
                "Close all but this")
    APPEND_MENU(ActionCxMenuCloseOthers, slotCxMenuCloseLeft,
                "Close all to the left")
    APPEND_MENU(ActionCxMenuCloseOthers, slotCxMenuCloseRight,
    ...
    APPEND_MENU(ActionCxMenuOpenFolder, slotCxMenuOpenFolder,
