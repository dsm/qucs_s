...
Arrow::Arrow()
    : headStyle(ArrowHeadStyle::empty), headHeight(20.0), headWidth(8.0),
...
void Arrow::paint(QPainter* painter) {
  ...
  case ArrowHeadStyle::filled:
    ...
    painter->drawConvexPolygon(
        QPolygon(QList{headLeftWing, arrowTip, headRightWing}));
    break;
  default:
    assert(false);
  }
...
void Arrow::paintScheme(Schematic* p) {
  ...
  // Wings
  p->PostPaintEvent(_Line, x2, y2, headLeftWing.x(), headLeftWing.y(), 0, 0,
                    false);
  p->PostPaintEvent(_Line, x2, y2, headRightWing.x(), headRightWing.y(), 0, 0,
                    false);
}
...
QString Arrow::save() {
  ...
  s += QString::number(static_cast<int>(headHeight)) + " " +
       QString::number(static_cast<int>(headWidth)) + " ";
...
void Arrow::updateHead() noexcept {
  ...
  headLeftWing = QPoint{x2 - static_cast<int>(headWingLength * cos(w)),
                        y2 - static_cast<int>(headWingLength * sin(w))};

  w = arrow_angle - headAngle;

  headRightWing = QPoint{x2 - static_cast<int>(headWingLength * cos(w)),
                         y2 - static_cast<int>(headWingLength * sin(w))};
}

// fx/fy are the precise coordinates, gx/gy are the coordinates set on grid.
// x/y are coordinates without scaling.
void Arrow::MouseMoving(const QPoint& onGrid, Schematic* sch,
                        const QPoint& cursor) {
  if (isBeingDrawn) {
    x2 = onGrid.x();
    y2 = onGrid.y();
  ...
  sch->PostPaintEvent(_Line, cursor.x() + 25, cursor.y(), cursor.x() + 13,
                      cursor.y() + 12, 0, 0, true);
  sch->PostPaintEvent(_Line, cursor.x() + 18, cursor.y() + 2, cursor.x() + 25,
                      cursor.y(), 0, 0, true);
  sch->PostPaintEvent(_Line, cursor.x() + 23, cursor.y() + 7, cursor.x() + 25,
                      cursor.y(), 0, 0, true);
}
...
bool Arrow::getSelected(const QPoint& click, int tolerance) {
  ...
  return qucs_s::geom::is_near_line(click, QPoint(x1, y1), arrowTip,
                                    tolerance) ||
