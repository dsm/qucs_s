...
ChangeDialog::ChangeDialog(Schematic* Doc_) : QDialog(Doc_) {
  Doc = Doc_;
...
void ChangeDialog::slotButtReplace() {
  ...
  Expr = QRegularExpression(
      QRegularExpression::wildcardToRegularExpression(CompNameEdit->text()));
#else
  Expr = QRegularExpression(
      misc::wildcardToRegularExpression(CompNameEdit->text(), false));
#endif
  if (!Expr.isValid()) {
  ...
  QLabel* Dia_Label =
      new QLabel(tr("Change properties of\n") + tr("these components ?"), Dia);
      ...
      if (match.hasMatch()) {
        for (const auto& pp : pc->Props) {
          if (pp->Name == PropNameEdit->currentText()) {
            ...
            if (i1 < 0) {
              break; // no multiple-choice property
            }
  ...
  while (i.hasNext()) {
    ...
    for (Component* pc : *Doc->a_Components) {
      ...
      for (auto pp : pc->Props) {
        ...
        if ((tmp > 0) || (tmp < -6)) {
          tx_Dist = 0; // remember text position
        }
