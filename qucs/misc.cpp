...
QString misc::complexRect(double real, double imag, int Precision) {
  QString Text;
  if (fabs(imag) < 1e-250) {
    Text = QString::number(real, 'g', Precision);
  } else {
...
QString misc::complexDeg(double real, double imag, int Precision) {
  ...
  if (fabs(imag) < 1e-250) {
    ...
    Text = QString::number(sqrt(real * real + imag * imag), 'g', Precision) +
           " / ";
    Text += QString::number(180.0 / pi * atan2(imag, real), 'g', Precision) +
            QString::fromUtf8("°");
  }
  return Text;
}
...
QString misc::StringNum(double num, char form, int Precision) {
  ...
  if (p) {
    ...
    if (*p == '0') {
      a++;
      p++;
    } // remove leading zeros of exponent
    if (a > 0) {
      do {
        *(p - a) = *p;
...
QString misc::StringNiceNum(double num) {
  char Format[6] = "%.8e";
  if (fabs(num) < 1e-250) {
    return QStringLiteral("0"); // avoid many problems
  }
  if (fabs(log10(fabs(num))) < 3.0) {
    Format[3] = 'g';
  }
  ...
  if (p) {
    ...
    if (p) {
      ...
      if (*p != '.') {
        p++; // no digit after decimal point ?
      }
      while ((*(p++) = *(pe++)) != 0)
        ; // overwrite zero characters
    }
  }
...
// #########################################################################
void misc::str2num(const QString& s_, double& Number, QString& Unit,
                   double& Factor) {
...
bool misc::VHDL_Time(QString& t, const QString& Name) {
  ...
  for (;;) {
    ...
    t = QString::fromUtf8("§") +
        QObject::tr("Error: Wrong time format in \"%1\". Use positive number "
                    "with units")
            .arg(Name) +
...
QString misc::wildcardToRegularExpression(const QString& wc_str,
...
void misc::draw_richtext(QPainter* painter, int x, int y, const QString& text,
                         QRectF* br) {
  ...
  const double font_size       = font_size_in_pixels ? smaller_font.pixelSize()
                                                     : smaller_font.pointSizeF();
  ...
  while (text.length() > i) {
    if ((text[i].toLatin1() == '_' || text[i].toLatin1() == '^')) {
      ...
      if (text[i] == '{') {
        i++;
        while (!text[i + len].isNull() && text[i + len].toLatin1() != '}') {
          len++;
        }
      }
      ...
      painter->drawText(
          current_text_x,
          current_text_y + (is_sub ? subscript_offset : superscript_offset), 1,
          1, Qt::TextDontClip, text.mid(i, len ? len : 1), &fragment_br);
      painter->restore();
      ...
      QRect fragment_br;
      painter->drawText(current_text_x, current_text_y, 1, 1, Qt::TextDontClip,
                        text.mid(i, len), &fragment_br);
