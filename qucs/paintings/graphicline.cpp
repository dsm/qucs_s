...
GraphicLine::GraphicLine(int ax, int ay, int bx, int by, QPen pen_)
    : pen(pen_) {
  Name       = "Line ";
  isSelected = false;
...
void GraphicLine::MouseMoving(const QPoint& onGrid, Schematic* sch,
                              const QPoint& cursor) {
  if (isBeingDrawn) {
    x2 = onGrid.x();
    y2 = onGrid.y();
  ...
  sch->PostPaintEvent(_Line, cursor.x() + 27, cursor.y(), cursor.x() + 15,
                      cursor.y() + 12, 0, 0,
                      true); // paint new cursor scursor.y()mbol
  sch->PostPaintEvent(_Line, cursor.x() + 25, cursor.y() - 2, cursor.x() + 29,
                      cursor.y() + 2, 0, 0, true);
  sch->PostPaintEvent(_Line, cursor.x() + 13, cursor.y() + 10, cursor.x() + 17,
                      cursor.y() + 14, 0, 0, true);
}
...
bool GraphicLine::getSelected(const QPoint& click, int tolerance) {
  return qucs_s::geom::is_near_line(click, QPoint(x1, y1), QPoint(x2, y2),
                                    tolerance);
}
