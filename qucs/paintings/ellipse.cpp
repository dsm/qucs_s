...
Element* qucs::Ellipse::info_filled(QString& Name, char*& BitmapFile,
                                    bool getNewOne) {
  Name       = QObject::tr("filled Ellipse");
  BitmapFile = (char*)"filledellipse";

  if (getNewOne) {
    return new qucs::Ellipse(true);
  }
  return nullptr;
}
...
void qucs::Ellipse::MouseMoving(const QPoint& onGrid, Schematic* sch,
                                const QPoint& cursor) {
  if (isBeingDrawn) {
    x2 = onGrid.x();
    y2 = onGrid.y();
  ...
  // paint cursor symbol
  sch->PostPaintEvent(_Ellipse, cursor.x() + 13, cursor.y(), 18, 12, 0, 0,
                      true);
  if (filled) {
    sch->PostPaintEvent(_Line, cursor.x() + 14, cursor.y() + 7, cursor.x() + 20,
                        cursor.y() + 1, 0, 0, true);
    sch->PostPaintEvent(_Line, cursor.x() + 25, cursor.y() + 2, cursor.x() + 18,
                        cursor.y() + 9, 0, 0, true);
    sch->PostPaintEvent(_Line, cursor.x() + 29, cursor.y() + 4, cursor.x() + 23,
...
bool qucs::Ellipse::Dialog(QWidget* parent) {
  ...
  auto d = std::make_unique<FillDialog>(QObject::tr("Edit Ellipse Properties"),
                                        true, parent);
