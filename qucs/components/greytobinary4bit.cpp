...
Element* greytobinary4bit::info(QString& Name, char*& BitmapFile,
                                bool getNewOne) {
  Name       = QObject::tr("4Bit Gray2Bin");
  BitmapFile = (char*)"greytobinary4bit";

  if (getNewOne) {
    return new greytobinary4bit();
  }
  return 0;
}
...
QString greytobinary4bit::vhdlCode(int) {
  ...
  s = "\n  " + Name + ":process (" + G0 + ", " + G1 + ", " + G2 + ", " + G3 +
      ")\n" + "  begin\n" + "    " + B3 + " <= " + G3 + td + "    " + B2 +
      " <= " + G3 + " xor " + G2 + td + "    " + B1 + " <= " + G3 + " xor " +
      G2 + " xor " + G1 + td + "    " + B0 + " <= " + G3 + " xor " + G2 +
      " xor " + G1 + " xor " + G0 + td + "  end process;\n";
  return s;
}
