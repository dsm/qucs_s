...
OptimizeDialog::OptimizeDialog(Optimize_Sim* c_, Schematic* d_) : QDialog(d_) {
  Comp    = c_;
  Doc     = d_;
  changed = false;
  numPrec = 3;
  ...
  VarTable->setHorizontalHeaderLabels(
      QStringList() << tr("Name") << tr("active") << tr("initial") << tr("min")
                    << tr("max") << tr("Type"));
  VarTable->setSortingEnabled(false);
  VarTable->setSelectionBehavior(QAbstractItemView::SelectRows);
  VarTable->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  VarTable->horizontalHeader()->setSectionsClickable(
  ...
  connect(VarTable->horizontalHeader(),
          SIGNAL(customContextMenuRequested(const QPoint&)), this,
          SLOT(slotSetPrecision(const QPoint&)));
  ...
  connect(VarTypeCombo, SIGNAL(activated(int)), SLOT(slotChangeVarType(int)));
  ...
  QPushButton* CreateEqn_Butt =
      new QPushButton(tr("Copy current values to equation"));
  ...
  GoalTable->setHorizontalHeaderLabels(QStringList() << tr("Name") << tr("Type")
                                                     << tr("Value"));
  ...
  connect(GoalTypeCombo, SIGNAL(activated(int)), SLOT(slotChangeGoalType(int)));
  ...
  if (!p0->Value.isEmpty()) {
    ...
    if (i != -1) { // current simulation is in the available simulations list
                   // (normal case)
  ...
  for (; pp != Comp->Props.end(); ++pp) {
    if ((*pp)->Name == "Var") {
      ...
      // active
      item = new QTableWidgetItem((ValueSplit.at(1) == "yes") ? tr("yes")
                                                              : tr("no"));
      item->setFlags(item->flags() & ~Qt::ItemIsEditable);
      VarTable->setItem(row, 1, item);
      // initial
      item = new QTableWidgetItem(
          QString::number(ValueSplit.at(2).toDouble(), 'g', numPrec));
      item->setFlags(item->flags() & ~Qt::ItemIsEditable);
...
void OptimizeDialog::slotEditVariable() {
  ...
  VarTypeCombo->setCurrentIndex(
      VarTypeCombo->findText(VarTable->item(row, 5)->text()));
...
void OptimizeDialog::slotAddVariable() {
  ...
  VarTable->setItem(row, 0, item);
  item =
      new QTableWidgetItem(VarActiveCheck->isChecked() ? tr("yes") : tr("no"));
  item->setFlags(item->flags() & ~Qt::ItemIsEditable);
  VarTable->setItem(row, 1, item);
...
void OptimizeDialog::slotDeleteVariable() {
  ...
  int nextRow =
      (selectedrow == VarTable->rowCount()) ? selectedrow - 1 : selectedrow;
...
void OptimizeDialog::slotEditGoal() {
  ...
  GoalTypeCombo->setCurrentIndex(
      GoalTypeCombo->findText(GoalTable->item(row, 1)->text()));
...
void OptimizeDialog::slotDeleteGoal() {
  ...
  int nextRow =
      (selectedrow == GoalTable->rowCount()) ? selectedrow - 1 : selectedrow;
...
void OptimizeDialog::slotApply() {
  ...
  if (NameEdit->text().isEmpty()) {
    NameEdit->setText(Comp->Name);
  } else if (NameEdit->text() != Comp->Name) {
  ...
  Prop = QString::number(MethodCombo->currentIndex() + 1) + "|" +
         IterEdit->text() + "|" + RefreshEdit->text() + "|" +
         ParentsEdit->text() + "|" + ConstEdit->text() + "|" +
         CrossEdit->text() + "|" + SeedEdit->text() + "|" +
         CostVarEdit->text() + "|" + CostObjEdit->text() + "|" +
         CostConEdit->text();
...
void OptimizeDialog::slotCancel() {
  if (changed) {
    done(QDialog::Accepted); // changed could have been done before
  } else {
    done(QDialog::Rejected); // (by "Apply"-button)
  }
}
...
void OptimizeDialog::slotCreateEqn() {
  ...
  for (int i = 2; i < Comp->Props.size(); i++) {
    if (Comp->Props.at(i)->Name ==
        "Var") { // property is an optimization variable
...
void OptimizeDialog::slotSetPrecision(const QPoint& pos) {
  ...
  if (selectedItem) {
    bool ok;
    int i = QInputDialog::getInt(this, tr("Set precision"), tr("Precision:"), 2,
                                 1, 16, 1, &ok);
    if (!ok) {
      return;
    }
