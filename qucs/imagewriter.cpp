...
void ImageWriter::noGuiPrint(QWidget* doc, const QString& printFile,
                             const QString& color) {
  ...
  if (printFile.endsWith(".svg") || printFile.endsWith(".eps")) {
    ...
    if (!printFile.endsWith(".svg")) {
      ...
      if (result != 0) {
        QMessageBox* msg =
            new QMessageBox(QMessageBox::Critical, "Export to image",
                            "Inkscape start error!", QMessageBox::Ok);
        msg->exec();
        delete msg;
      }
...
int ImageWriter::print(QWidget* doc) {
  ...
  ExportDialog* dlg = new ExportDialog(w, h, wsel, hsel, lastExportFilename,
                                       selected.isNull(), 0);
  ...
  if (dlg->exec()) {
    ...
    if (!dlg->isOriginalSize()) {
      auto size_coef =
          static_cast<double>(dlg->Xpixels()) / static_cast<double>(w);
    ...
    if (dlg->isValidFilename()) {
      if (!dlg->isSvg()) {
        ...
        sch->print(nullptr, p, exportAll, true,
        ...
        p->fillRect(0, 0, svgwriter->size().width(), svgwriter->size().height(),
                    Qt::white);
        sch->print(nullptr, p, exportAll, true,
                   QMargins{border, border, border, border});
        ...
        if (dlg->needsInkscape()) {
          ...
          if (result != 0) {
            QMessageBox::critical(0, QObject::tr("Export to image"),
                                  QObject::tr("Inkscape start error!"),
                                  QMessageBox::Ok);
          }
      ...
      if (QFile::exists(filename)) {
        ...
        QMessageBox::information(0, QObject::tr("Export to image"),
                                 QObject::tr("Disk write error!"),
                                 QMessageBox::Ok);
        status = -1;
      }
    } else {
      QMessageBox::critical(
          0, QObject::tr("Export to image"),
