...
auto getFontDescription = [](const auto& Font) -> QString {
  ...
  QString fontDescription =
      Font.family() + comma + QString::number(Font.pointSize());
...
QucsSettingsDialog::QucsSettingsDialog(QucsApp* parent) : QDialog(parent) {
  App = parent;
  ...
  const QStringList appLanguages = {tr("system language"),
                                    tr("English") + " (en)",
                                    tr("Arabic") + " (ar)",
                                    tr("Catalan") + " (ca)",
                                    tr("Chinese") + " (zh_CN)",
  ...
  appSettingsGrid->addWidget(
      new QLabel(tr("Language (set after reload):"), appSettingsTab), 1, 0);
  ...
  appSettingsGrid->addWidget(
      new QLabel(tr("Maximum undo operations:"), appSettingsTab), 2, 0);
  undoNumEdit = new QLineEdit(appSettingsTab);
  undoNumEdit->setValidator(val200);
  appSettingsGrid->addWidget(undoNumEdit, 2, 1);

  appSettingsGrid->addWidget(new QLabel(tr("Text editor:"), appSettingsTab), 3,
                             0);
  editorEdit = new QLineEdit(appSettingsTab);
  editorEdit->setToolTip(
      tr("Set to qucs, qucsedit or the path to your favorite text editor."));
  appSettingsGrid->addWidget(editorEdit, 3, 1);

  appSettingsGrid->addWidget(
      new QLabel(tr("Start wiring when clicking open node:"), appSettingsTab),
      4, 0);
  checkWiring = new QCheckBox(appSettingsTab);
  appSettingsGrid->addWidget(checkWiring, 4, 1);

  appSettingsGrid->addWidget(
      new QLabel(tr("Load documents from future versions:")), 5, 0);
  checkLoadFromFutureVersions = new QCheckBox(appSettingsTab);
  checkLoadFromFutureVersions->setToolTip(
      tr("Try to load also documents created with newer versions of Qucs."));
  ...
  appSettingsGrid->addWidget(
      new QLabel(tr("Show trace name prefix on diagrams:")), 6, 0);
  checkFullTraceNames = new QCheckBox(appSettingsTab);
  checkFullTraceNames->setToolTip(
      tr("Show prefixes for trace names on diagrams like \"ngspice/\""));
  appSettingsGrid->addWidget(checkFullTraceNames, 6, 1);
  checkFullTraceNames->setChecked(QucsSettings.fullTraceName);

  appSettingsGrid->addWidget(
      new QLabel(tr("Flexible wires (requires restart):"), appSettingsTab), 7,
      0);
  allowFlexibleWires = new QCheckBox(appSettingsTab);
  appSettingsGrid->addWidget(allowFlexibleWires, 7, 1);

  appSettingsGrid->addWidget(
      new QLabel(tr("Lay wires anew when moving elements (requires restart):"),
                 appSettingsTab),
      8, 0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Schematic font (set after reload):"), appSettingsTab), 0,
      0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Application font (set after reload):"), appSettingsTab), 1,
      0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Text document font (set after reload):"), appSettingsTab),
      2, 0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Large font size:"), appSettingsTab), 3, 0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Document Background Color:"), appSettingsTab), 4, 0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Grid Color (set after reload):"), appSettingsTab), 5, 0);
  ...
  appAppearanceGrid->addWidget(
      new QLabel(tr("Draw diagrams with anti-aliasing feature:")), 6, 0);
  checkAntiAliasing = new QCheckBox(appSettingsTab);
  checkAntiAliasing->setToolTip(
      tr("Use anti-aliasing for graphs for a smoother appearance."));
  appAppearanceGrid->addWidget(checkAntiAliasing, 6, 1);
  checkAntiAliasing->setChecked(QucsSettings.GraphAntiAliasing);

  appAppearanceGrid->addWidget(
      new QLabel(tr("Draw text with anti-aliasing feature:")), 7, 0);
  checkTextAntiAliasing = new QCheckBox(appSettingsTab);
  checkTextAntiAliasing->setToolTip(
      tr("Use anti-aliasing for text for a smoother appearance."));
  appAppearanceGrid->addWidget(checkTextAntiAliasing, 7, 1);
  checkTextAntiAliasing->setChecked(QucsSettings.TextAntiAliasing);

  appAppearanceGrid->addWidget(
      new QLabel(tr("Default graph line thickness:"), appSettingsTab), 8, 0);
  ...
  appAppearanceGrid->addWidget(new QLabel(tr("App Style:"), appSettingsTab), 9,
                               0);
  ...
  editorGrid->addWidget(
      new QLabel(tr("Colors for Syntax Highlighting:"), editorTab), 0, 1, 0, 2);
  ...
  QLabel* note = new QLabel(tr("Register filename extensions here in order "
  ...
  fileTypesTableWidget->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  fileTypesTableWidget->horizontalHeader()->setSectionsClickable(
      false); // no action when clicking on the header
  fileTypesTableWidget->verticalHeader()->hide();
  connect(fileTypesTableWidget, SIGNAL(cellClicked(int, int)),
          SLOT(slotTableClicked(int, int)));
  ...
  while (it != QucsSettings.FileTypes.end()) {
    ...
    QTableWidgetItem* suffix =
        new QTableWidgetItem(QString((*it).section('/', 0, 0)));
    QTableWidgetItem* program =
        new QTableWidgetItem(QString((*it).section('/', 1, 1)));
  ...
  QLabel* note2 =
      new QLabel(tr("Edit the standard paths and external applications"));
  ...
  locationsGrid->addWidget(new QLabel(tr("RF Layout Path:"), locationsTab), 6,
                           0);
  ...
  connect(pathsTableWidget, SIGNAL(cellClicked(int, int)),
          SLOT(slotPathTableClicked(int, int)));
  connect(pathsTableWidget, SIGNAL(itemSelectionChanged()),
          SLOT(slotPathSelectionChanged()));
  ...
  QPushButton* AddPathSubFolButt =
      new QPushButton(tr("Add Path With SubFolders"));
  locationsGrid->addWidget(AddPathSubFolButt, 8, 2);
  connect(AddPathSubFolButt, SIGNAL(clicked()),
          SLOT(slotAddPathWithSubFolders()));
  ...
  graphLineWidthEdit->setText(
      _settings::Get().item<QString>("DefaultGraphLineWidth"));
  ...
  p.setColor(GridColorButton->backgroundRole(),
             _settings::Get().item<QColor>("GridColor"));
...
void QucsSettingsDialog::slotAddFileType() {
  QModelIndexList indexes =
      fileTypesTableWidget->selectionModel()->selection().indexes();
  if (indexes.count()) {
    fileTypesTableWidget->item(indexes.at(0).row(), 0)
        ->setText(Input_Suffix->text());
    fileTypesTableWidget->item(indexes.at(0).row(), 1)
        ->setText(Input_Program->text());
  ...
  QTableWidgetItem* newSuffix =
      new QTableWidgetItem(QString(Input_Suffix->text()));
...
void QucsSettingsDialog::slotApply() {
  ...
  if (QucsSettings.BGColor !=
      BGColorButton->palette().color(BGColorButton->backgroundRole())) {
  ...
  if (_settings::Get().item<QColor>("GridColor") !=
      GridColorButton->palette().color(GridColorButton->backgroundRole())) {
    _settings::Get().setItem<QColor>(
        "GridColor",
        GridColorButton->palette().color(GridColorButton->backgroundRole()));
    ...
    for (int tab = 0; tab < App->DocumentTab->count(); tab++) {
      ...
      if (!QucsApp::isTextDocument(widget)) {
        static_cast<Schematic*>(widget)->setGridColor(
            _settings::Get().item<QColor>("GridColor"));
      }
    }
  ...
  _settings::Get().setItem("AllowFlexibleWires",
                           allowFlexibleWires->isChecked());
  _settings::Get().setItem("AllowLayingWiresAnew",
                           allowLayingWiresAnew->isChecked());
  ...
  if (QucsSettings.IgnoreFutureVersion !=
      checkLoadFromFutureVersions->isChecked()) {
  ...
  // use toDouble() as it can interpret the string according to the current
  // locale
  ...
  if (_settings::Get().item<QString>("DefaultGraphLineWidth") !=
      graphLineWidthEdit->text()) {
    _settings::Get().setItem<QString>("DefaultGraphLineWidth",
                                      graphLineWidthEdit->text());
    changed = true;
  }
...
void QucsSettingsDialog::slotBGColorDialog() {
  QColor c = QColorDialog::getColor(
      BGColorButton->palette().color(BGColorButton->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotDefaultValues() {
  ...
  allowLayingWiresAnew->setChecked(
      _settings::Get().itemDefault<bool>("AllowLayingWiresAnew"));
...
void QucsSettingsDialog::slotColorComment() {
  QColor c = QColorDialog::getColor(
      ColorComment->palette().color(ColorComment->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorString() {
  QColor c = QColorDialog::getColor(
      ColorString->palette().color(ColorString->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorInteger() {
  QColor c = QColorDialog::getColor(
      ColorInteger->palette().color(ColorInteger->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorReal() {
  QColor c = QColorDialog::getColor(
      ColorReal->palette().color(ColorReal->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorCharacter() {
  QColor c = QColorDialog::getColor(
      ColorCharacter->palette().color(ColorCharacter->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorDataType() {
  QColor c = QColorDialog::getColor(
      ColorDataType->palette().color(ColorDataType->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorAttribute() {
  QColor c = QColorDialog::getColor(
      ColorAttribute->palette().color(ColorAttribute->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorDirective() {
  QColor c = QColorDialog::getColor(
      ColorDirective->palette().color(ColorDirective->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotColorTask() {
  QColor c = QColorDialog::getColor(
      ColorTask->palette().color(ColorTask->foregroundRole()), this);
  if (c.isValid()) {
...
void QucsSettingsDialog::slotHomeDirBrowse() {
  QString d = QFileDialog::getExistingDirectory(
      this, tr("Select the home directory"), homeEdit->text(),
...
void QucsSettingsDialog::slotAdmsXmlDirBrowse() {
  QString d = QFileDialog::getExistingDirectory(
      this, tr("Select the admsXml bin directory"), admsXmlEdit->text(),
      QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
void QucsSettingsDialog::slotAscoDirBrowse() {
  QString d = QFileDialog::getExistingDirectory(
      this, tr("Select the ASCO bin directory"), ascoEdit->text(),
      QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
void QucsSettingsDialog::slotOctaveDirBrowse() {
  QString d =
      QFileDialog::getOpenFileName(this, tr("Select the octave executable"),
...
void QucsSettingsDialog::slotOpenVAFDirBrowse() {
  QString d =
      QFileDialog::getOpenFileName(this, tr("Select the OpenVAF executable"),
...
void QucsSettingsDialog::slotRFLayoutDirBrowse() {
  QString d = QFileDialog::getOpenFileName(
      this, tr("Select the Qucs-RFLayout executable"), RFLayoutEdit->text(),
      "All files (*)");
...
void QucsSettingsDialog::slotAddPath() {
  QString d = QFileDialog::getExistingDirectory(
      this, tr("Select a directory"), QucsSettings.QucsWorkDir.canonicalPath(),
      QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
