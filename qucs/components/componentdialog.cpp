...
/*
  ...
  3. DONE: Add special property names - i.e., for log sweeps (per decade instead
  of step)
...
QStringList getOptionsFromString(const QString& description) {
  ...
  if (start != -1 && end != -1) {
    ...
    for (auto entry : list) {
      options
          << entry.trimmed(); // QString::trimmed flagged by valgrind leak check
    }
  }
...
public:
  CompoundWidget(const QString& text, ComponentDialog* dialog,
                 void (ComponentDialog::*func)(QLineEdit*) = nullptr)
      : QWidget(dialog) {
...
class ParamWidget {
public:
  ParamWidget(const QString& param, const QString& label, bool displayCheck,
              QGridLayout* layout)
  ...
  QString defaultLabel() { return mDefaultLabel; }

  void setCheck(bool checked) {
    mCheckBox->setCheckState((mHasCheck && checked) ? Qt::Checked
                                                    : Qt::Unchecked);
  }

  bool check() { return (mCheckBox->checkState() == Qt::Checked); }
...
public:
  ParamLineEdit(const QString& param, const QString& label,
                QValidator* validator, bool displayCheck, QGridLayout* layout,
  ...
  void setValue(const QString& value) override { setText(value); }

  void setOptions(const QStringList& options) override { (void)options; }

  QString value() override { return text(); }
};
...
public:
  ParamCombo(const QString& param, const QString& label, bool displayCheck,
             QGridLayout* layout, ComponentDialog* dialog,
             void (ComponentDialog::*func)(const QString&) = nullptr)
      : ParamWidget(param, label, displayCheck, layout) {
    layout->addWidget(this, layout->rowCount() - 1, 1);
  ...
  void setValue(const QString& value) { setCurrentIndex(findText(value)); }
  ...
  QString value() override { return currentText(); }
};
...
EqnHighlighter::EqnHighlighter(const QString& keywordSet, QTextDocument* parent)
  ...
  if (keywordSet == "ngspice") {
    ...
    const QString keywordPatterns[]{
        L("\\bmag\\b"),     L("\\bph\\b"),          L("\\bcph\\b"),
        L("\\bunwrap\\b"),  L("\\bj\\b"),           L("\\breal\\b"),
        L("\\bimag\\b"),    L("\\bconj\\b"),        L("\\bdb\\b"),
        L("\\blog10\\b"),   L("\\bln\\b"),          L("\\bexp\\b"),
        L("\\babs\\b"),     L("\\bsqrt\\b"),        L("\\bsin\\b"),
        L("\\bcos\\b"),     L("\\btan\\b"),         L("\\batan\\b"),
        L("\\bsinh\\b"),    L("\\bcosh\\b"),        L("\\btanh\\b"),
        L("\\batanh\\b"),   L("\\bfloor\\b"),       L("\\bceil\\b"),
        L("\\bnorm\\b"),    L("\\bmean\\b"),        L("\\bavg\\b"),
        L("\\bstddev\\b"),  L("\\bgroup_delay\\b"), L("\\bvector\\b"),
        L("\\bcvector\\b"), L("\\bunitvec\\b"),     L("\\blength\\b"),
        L("\\binteg\\b"),   L("\\bderiv\\b"),       L("\\bvecd\\b"),
        L("\\bminimum\\b"), L("\\bvecmax\\b"),      L("\\bmaximum\\b"),
        L("\\bfft\\b"),     L("\\bifft\\b"),        L("\\bsortorder\\b"),
        L("\\btimer\\b"),   L("\\bclock\\b")};
...
ComponentDialog::ComponentDialog(Component* schematicComponent,
                                 Schematic* schematic)
    : QDialog(schematic) {
  ...
  excludeList << "Diode" << "MCROSS" << "MTEE" << "R" << "C" << "Buf" << "Inv";

  // qDebug() << "Component Model text is: " << component->Model;

  restoreGeometry(
      _settings::Get().item<QByteArray>("ComponentDialog/geometry"));
  ...
  // Setup validators.
  // TODO: These don't look right and don't seem to restrict the edit boxes in
  // the way they should?
  intVal      = new QIntValidator(1, 1000000, this);
  compNameVal = new QRegularExpressionValidator(
      QRegularExpression("[A-Za-z][A-Za-z0-9_]+"), this);
  nameVal = new QRegularExpressionValidator(
      QRegularExpression("[\\w_.,\\(\\) @:\\[\\]]+"), this);
  paramVal =
  ...
  componentNameWidget = new ParamLineEdit("Name", tr("Name"), compNameVal, true,
                                          nameLayout, this, nullptr);
  ...
  isEquation =
      QStringList({"Eqn", "NutmegEq", "SpiceIC", "SpicePar", "SpiceOptions",
                   "SpiceFunc", "SpiceCSPar", "SpGlobPar", "SpiceNodeset"})
          .contains(component->Model);
  hasSweep = QStringList({".AC", ".DISTO", ".NOISE", ".SW", ".SP", ".TR"})
                 .contains(component->Model);
  hasFile =
      component->Props.count() > 0 && component->Props.at(0)->Name == "File";

  sweepProperties =
      QStringList({"Sim", "Type", "Param", "Start", "Stop", "Points"});

  // TODO: Consider creating a sweepParams hash which contains the valid sweep
  // parameters for a given simulation type. Then only create the valid widgets
  // fo sweepParams[".AC"] = QStringList({"Type", "Start", "Stop", "Points"});

  paramsHiddenBySim["Export"] =
      QStringList{"NutmegEq",  "SpiceIC",    "SpicePar",  "SpiceOptions",
                  "SpiceFunc", "SpiceCSPar", "SpGlobPar", "SpiceNodeset"};
  paramsHiddenBySim["Sim"] = QStringList{
      ".AC",        ".DISTO",    ".SP",         ".NOISE",       ".TR",
      "Eqn",        "SpiceIC",   "SpicePar",    "SpiceOptions", "SpiceFunc",
      "SpiceCSPar", "SpGlobPar", "SpiceNodeset"};
  paramsHiddenBySim["Param"] =
      QStringList{".AC", ".DISTO", ".SP", ".NOISE", ".TR"};
  ...
  if (isEquation) {
    ...
    if (hasSweep) {
      ...
      void (ComponentDialog::*func)(const QString&) =
          &ComponentDialog::updateSweepProperty;
      sweepParamWidget["Sim"] = new ParamCombo("Sim", tr("Simulation"), true,
                                               sweepPageLayout, this, func);
      sweepParamWidget["Param"] =
          new ParamLineEdit("Param", tr("Sweep Parameter"), compNameVal, true,
                            sweepPageLayout, this, func);
      sweepParamWidget["Type"] =
          new ParamCombo("Type", tr("Type"), true, sweepPageLayout, this, func);
      sweepParamWidget["Values"] = new ParamLineEdit(
          "Values", tr("Values"), paramVal, true, sweepPageLayout, this, func);
      sweepParamWidget["Start"] = new ParamLineEdit(
          "Start", tr("Start"), paramVal, true, sweepPageLayout, this, func);
      sweepParamWidget["Stop"] = new ParamLineEdit(
          "Stop", tr("Stop"), paramVal, true, sweepPageLayout, this, func);
      sweepParamWidget["Step"] = new ParamLineEdit(
          "Step", tr("Step"), paramVal, false, sweepPageLayout, this, func);
      sweepParamWidget["Points"] = new ParamLineEdit(
          "Points", tr("Number"), intVal, true, sweepPageLayout, this, func);

      // Setup the widget specialisations for each simulation type.
      sweepTypeEnabledParams["lin"] = QStringList{
          "Sim", "Type", "Param", "Start", "Stop", "Step", "Points"};
      sweepTypeEnabledParams["log"] = QStringList{
          "Sim", "Type", "Param", "Start", "Stop", "Step", "Points"};
      sweepTypeEnabledParams["list"] =
          QStringList{"Sim", "Type", "Param", "Values"};
      sweepTypeSpecialLabels[qMakePair(QString("log"), QString("Step"))] = {
          "Points per decade"};
    ...
    // This component does not have sweep settings, so add properties directly
    ...
    if (QStringList({"Diode", "_BJT", "JFET", "MOSFET", "_MOSFET", "VDMOS"})
            .contains(component->Model)) {
      QHBoxLayout* spiceButtonLayout = new QHBoxLayout;
      propertyTableLayout->addLayout(spiceButtonLayout);
      QPushButton* spiceButton =
          new QPushButton(tr("Populate parameters from SPICE file..."), this);
      connect(spiceButton, &QPushButton::released, this,
              &ComponentDialog::slotFillFromSpice);
    ...
    propertyTable->setHorizontalHeaderLabels(
        {tr("Name"), tr("Value"), tr("Show"), tr("Description")});
    ...
    connect(propertyTable, &QTableWidget::cellClicked,
            [=](int row, int column) {
              (void)column;
              propertyTable->setCurrentCell(row, 1);
            });
  }
  ...
  connect(buttonBox, &QDialogButtonBox::accepted, this,
          &ComponentDialog::slotOKButton);
  connect(buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
  connect(buttonBox->button(QDialogButtonBox::Apply), &QPushButton::clicked,
          this, &ComponentDialog::slotApplyButton);
...
ComponentDialog::~ComponentDialog() {
  ...
  // Clean up the sweep parameter multi-widget containers. The widgets are
  // deleted by the system because they are reparented to the dialog or the
  // dialog's subwidgets.
  for (auto it = sweepParamWidget.keyValueBegin();
       it != sweepParamWidget.keyValueEnd(); ++it) {
    if (it->second) {
      delete it->second;
    }
  }
...
void ComponentDialog::keyPressEvent(QKeyEvent* e) {
  // Workaround for Qt+Wayland bug. Otherwise Qt::Key_Return invokes
  // slotOKButton() before registering propertyTable changes meaning the old
  // value is retained.
...
void ComponentDialog::updateSweepWidgets(const QString& type) {
  for (auto it = sweepParamWidget.keyValueBegin();
       it != sweepParamWidget.keyValueEnd(); ++it) {
    it->second->setLabel(
        sweepTypeSpecialLabels.contains(qMakePair(type, it->first))
            ? sweepTypeSpecialLabels[qMakePair(type, it->first)]
            : it->second->defaultLabel());
    it->second->setHidden(
        paramsHiddenBySim.contains(it->first) &&
...
void ComponentDialog::updateSweepProperty(const QString& property) {
  ...
  if (property == "All") {
    for (auto property : component->Props) {
      ...
      pointsEdit->setValue(pointsEdit->value() == "" ? "100"
                                                     : pointsEdit->value());
    }
  }
  ...
  if (property == "Values" || sweepParamWidget["Type"]->value() == "list") {
    QStringList values =
        sweepParamWidget["Values"]->value().remove('[').remove(']').split(';');
  ...
  // Specialisations for updating start, stop, step, and points values.
    ...
    if (sweepParamWidget["Type"]->value() == "log") {
      if (property == "Start" || property == "Stop" || property == "Points" ||
          property == "All") {
      ...
      sweepParamWidget["Values"]->setValue("[" + startEdit->value() + ";" +
                                           stopEdit->value() + "]");
    }
  }
}
...
void ComponentDialog::updatePropertyTable(const Component* updateComponent) {
  // Add component properties to the properties table with the exception of the
  // sweep properties.
  int row = 0;
  for (const Property* property : updateComponent->Props) {
    if ((property->simulators & QucsSettings.DefaultSimulator) !=
    ...
    propertyTable->setItem(row, 0,
                           new QTableWidgetItem(property->Name, LabelCell));
    ...
    // Create a compound widget that selects a file.
      ...
      CompoundWidget* compound = new CompoundWidget(
          property->Value, this, &ComponentDialog::slotBrowseFile);
    ...
    // TODO: This for when parameters have a parameter type flag (see #974)
      ...
      CompoundWidget* compound = new CompoundWidget(
          property->Value, this, &ComponentDialog::simpleEditEqn);
      propertyTable->setCellWidget(row, 1, compound);
      propertyTable->setItem(row, 1, new QTableWidgetItem(CompoundCell));
    }
    ...
    propertyTable->item(row, 2)->setCheckState(
        property->display ? Qt::Checked : Qt::Unchecked);
    propertyTable->item(row, 2)->setFlags(Qt::ItemIsEnabled |
                                          Qt::ItemIsUserCheckable);
    propertyTable->setItem(
        row, 3, new QTableWidgetItem(property->Description, LabelCell));
...
void ComponentDialog::updateEqnEditor() {
  ...
  for (auto property : component->Props) {
    if (eqnSimCombo && property->Name == "Simulation") {
      ...
      eqnExportCheck->setCheckState(property->Value == "yes" ? Qt::Checked
                                                             : Qt::Unchecked);
    }
...
void ComponentDialog::writeEquation() {
  ...
  // Note: the description needs to be written as "Simulation name" because this
  // is used when saving the file.
  if (eqnSimCombo) {
    component->Props.append(new Property(
        "Simulation", eqnSimCombo->currentText(), true, "Simulation name"));
  }
  ...
  for (const QString& line : lines) {
    ...
    if (parts.count() == 2) {
      component->Props.append(
          new Property(parts[0].trimmed(), parts[1].trimmed(), true));
    }
  }

  if (eqnExportCheck) {
    component->Props.append(new Property(
        "Export", eqnExportCheck->checkState() == Qt::Checked ? "yes" : "no",
...
void ComponentDialog::slotApplyButton() {
  ...
  if (isEquation) {
    ...
    if (hasSweep) {
      // Note: Order is very important here. The component expects parameters in
      // a specific order depending on which sweep parameters are valid for the
      // component type.
    ...
    for (Property* property : component->Props) {
      ...
      if (property->Name == "Symbol" &&
        ...
        // TODO: If the number of items has changed because of an earlier edit,
        // then the property list needs to be updated. An example is when the
        // number of ports of an EDD is increased.
        if (!item) {
          continue;
        }
        ...
        if (item->type() == ComboBoxCell) {
          QComboBox* cellCombo =
              static_cast<QComboBox*>(propertyTable->cellWidget(row, 1));
          property->Value = cellCombo->currentText();
        }

        else if (item->type() == CompoundCell) {
          CompoundWidget* cellCompound =
              static_cast<CompoundWidget*>(propertyTable->cellWidget(row, 1));
        ...
        // qDebug() << "Writing property " << property->Name << " " <<
        // property->Value;
        property->display =
            (propertyTable->item(row, 2)->checkState() == Qt::Checked);
        row++;
      }
    }
  }
  ...
  // If this is needed, make it a function because something similar is called
  // elsewhere.
  /*
  if (changed)
  {
...
void ComponentDialog::slotBrowseFile(QLineEdit* lineEdit) {
  ...
  if (!currFileName.isEmpty()) {          // a file name is already defined
    if (currFileInfo.isRelative()) {      // but has no absolute path
      if (!schematicFileName.isEmpty()) { // if schematic has a filename
        ...
        currDir = lastDir.isEmpty() ? QucsSettings.QucsWorkDir.absolutePath()
                                    : lastDir;
      }
    } else { // current file name is absolute
      currDir = currFileInfo.exists() ? currFileInfo.absolutePath()
    ...
    if (!schematicFileName.isEmpty()) { // if schematic has a filename
      ...
      currDir =
          lastDir.isEmpty() ? QucsSettings.QucsWorkDir.absolutePath() : lastDir;
    }
  }

  QString s = QFileDialog::getOpenFileName(
      this, tr("Select a file"), currDir,
      tr("All Files") + " (*.*);;" + tr("Touchstone files") + " (*.s?p);;" +
          tr("CSV files") + " (*.csv);;" + tr("SPICE files") +
          " (*.cir *.spi);;" + tr("VHDL files") + " (*.vhdl *.vhd);;" +
          tr("Verilog files") + " (*.v)");
  ...
  if (!s.isEmpty()) {
    ...
    if (!(schematicFileName.isEmpty() && QucsMain->ProjName.isEmpty())) {
      // unsaved schematic outside project; only absolute file name
      // the schematic could be saved elsewhere and working directory may be
      // changed
...
// -------------------------------------------------------------------------
// Close the simple eqn editor dialog and copy the edited text back to the
// parent.
...
{
  qDebug() << "editing file " << component->Props.at(0)->Value << " or " <<
propertyTable->item(0, 1)->text();
  document->App->editFile(misc::properAbsFileName(component->Props.at(0)->Value,
document));
}
*/
...
QStringList ComponentDialog::getSimulationList(bool includeGeneric) {
  ...
  for (Component* c : sch->a_DocComps) {
    ...
    if (c->Model == ".SW" &&
        !c->Props.at(0)->Value.toUpper().startsWith("DC")) {
      continue;
    }
...
void ComponentDialog::slotFillFromSpice() {
  ...
  for (auto property : component->Props) {
    tempComponent.Props.append(new Property(property->Name, property->Value,
                                            property->display,
