 ...
 */
TextDoc::TextDoc(QucsApp* App_, const QString& Name_)
    : QPlainTextEdit(), QucsDoc(App_, Name_) {
  ...
  connect(this, SIGNAL(cursorPositionChanged()), SLOT(slotCursorPosChanged()));
  if (App_) {
...
bool TextDoc::baseSearch(const QString& str, bool CaseSensitive, bool wordOnly,
                         bool backward) {
  ...
  if (!finded) {
    ...
    if (!finded) {
      QMessageBox::information(
          this, tr("Find..."), tr("Cannot find target: %1").arg(str),
...
void TextDoc::search(const QString& str, bool CaseSensitive, bool wordOnly,
                     bool backward) {
  baseSearch(str, CaseSensitive, wordOnly, backward);
}

// implement replace function
void TextDoc::replace(const QString& str, const QString& str2,
                      bool needConfirmed, bool CaseSensitive, bool wordOnly,
                      bool backward) {
  ...
  if (finded) {
    i = QMessageBox::Yes;
    if (needConfirmed) {
      i = QMessageBox::information(this, tr("Replace..."),
...
/*!
 * \brief Zooms the document in and out. Note, the zoom amount is fixed by Qt
 * and the amount passed is ignored.
 */
...
void TextDoc::showNoZoom() {
  // Get a copy of this editor's existing font and apply the default font size
  // to it.
  QFont tempFont = font();
...
/*!
 * \brief TextDoc::getModuleName parse the module name ou of the text file
 * contents \return the module name
 */
