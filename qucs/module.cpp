...
Component* Module::getComponent(QString Model) {
  if (Modules.contains(Model)) {
    ...
    if (vaComponents.contains(Model)) {
      return (Component*)vacomponent::info(Name, vaBitmap, true,
                                           vaComponents[Model]);
    } else {
...
void Module::registerDynamicComponents() {
  ...
  while (i.hasNext()) {
    ...
    //     char * File;
    Component* c = (Component*)vacomponent::info(Name, vaBitmap, true,
                                                 vaComponents[i.key()]);
    Model        = c->Model;
    delete c;
...
#define REGISTER_EQUATION_1(val) REGISTER_MOD_1(QObject::tr("equations"), val)
