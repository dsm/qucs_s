...
TR_Sim::TR_Sim() {
  ...
  Props.append(bld.property(
      "Type", "lin", QObject::tr("sweep type") + " [lin, log, list, const]"));
  Props.append(
      bld.property("Start", "0", QObject::tr("start time in seconds")));
  Props.append(
      bld.property("Stop", "1 ms", QObject::tr("stop time in seconds")));
  Props.append(bld.hidden().property(
      "Points", "200", QObject::tr("number of simulation time steps")));
  ...
  Props.append(bld.property(
      "Order", "2", QObject::tr("order of integration method") + " (1-6)"));
  ...
  Props.append(bld.property(
      "relaxTSR", "no", QObject::tr("relax time step raster") + " [no, yes]"));

  bld.hidden().simulator(spicecompat::simAll);
  Props.append(bld.property(
      "initialDC", "yes",
      QObject::tr("perform initial DC (set \"no\" to activate UIC)") +
          " [yes, no]"));
...
QString TR_Sim::spice_netlist(
  ...
  QString max_step =
      spicecompat::normalize_value(getProperty("MaxStep")->Value);
  if (max_step != "0") {
    s += max_step;
  }

  if (dialect != spicecompat::SPICEXyce) { // Xyce ignores this parameter
    if (Props.at(18)->Value == "no") {
      s += " UIC";
    }
  }
  s += "\n";
