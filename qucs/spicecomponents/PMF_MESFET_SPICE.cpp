...
PMF_MESFET_SPICE::PMF_MESFET_SPICE() {
  Description =
      QObject::tr("Z(PMF) MESFET:\nMultiple line ngspice or Xyce Z model "
...
Element* PMF_MESFET_SPICE::info(QString& Name, char*& BitmapFile,
                                bool getNewOne) {
...
QString PMF_MESFET_SPICE::spice_netlist(
  ...
  if (Z.length() > 0) {
    s += QStringLiteral("%1").arg(Z);
  }
  if (Z_Line_2.length() > 0) {
    s += QStringLiteral("\n%1").arg(Z_Line_2);
  }
  if (Z_Line_3.length() > 0) {
    s += QStringLiteral("\n%1").arg(Z_Line_3);
  }
  if (Z_Line_4.length() > 0) {
    s += QStringLiteral("\n%1").arg(Z_Line_4);
  }
  if (Z_Line_5.length() > 0) {
    s += QStringLiteral("\n%1").arg(Z_Line_5);
  }
  s += "\n";
