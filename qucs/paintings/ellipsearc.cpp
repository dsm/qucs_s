...
void EllipseArc::paintScheme(Schematic* p) {
  p->PostPaintEvent(_Arc, x1, y1, x2 - x1, y2 - y1, arcStartAngle,
                    arcLengthAngle);
}
...
QString EllipseArc::save() {
  QString s = Name + QString::number(x1) + " " + QString::number(y1) + " " +
              ...
              QString::number(arcStartAngle) + " " +
              QString::number(arcLengthAngle) + " " + pen.color().name() + " " +
              QString::number(pen.width()) + " " + QString::number(pen.style());
  return s;
}

QString EllipseArc::saveCpp() {
  QString s = QString("new Arc (%1, %2, %3, %4, %5, %6, "
                      "QPen (QColor (\"%7\"), %8, %9))")
                  .arg(x1)
                  .arg(y1)
                  .arg(x2 - x1)
...
QPoint calculatePointOnEllipse(const QRect& ellipseBounds,
                               const QPoint& cursorPos) {
  auto relCursorPos = cursorPos - ellipseBounds.center();
  auto angle        = std::atan2(relCursorPos.y(), relCursorPos.x());
  return ellipseBounds.center() +
         QPoint((ellipseBounds.width() / 2.0) * std::cos(angle),
                (ellipseBounds.height() / 2.0) * std::sin(angle));
}
} // namespace internal

void EllipseArc::MouseMoving(const QPoint& onGrid, Schematic* sch,
                             const QPoint& cursor) {
  switch (drawingState) {
  ...
  case DrawingState::defining_arc_length:
    arcLengthAngle =
        internal::calculateQtAngle({cx, cy}, cursor) - arcStartAngle;
    sch->PostPaintEvent(_Arc, x1, y1, x2 - x1, y2 - y1, arcStartAngle,
                        arcLengthAngle);
    break;
  default:
    assert(false);
  ...
  // paint cursor symbol
  sch->PostPaintEvent(_Arc, cursor.x() + 13, cursor.y(), 18, 12, 16 * 45,
                      16 * 200, true);
}
...
bool EllipseArc::mirrorY() noexcept {
  ...
  if (arcStartAngle <= internal::qtHalfCircle) {
    ...
    arcStartAngle =
        internal::qtFullCircle + internal::qtHalfCircle - arcStartAngle;
  }
...
bool EllipseArc::Dialog(QWidget* parent) {
  ...
  auto d = std::make_unique<FillDialog>(QObject::tr("Edit Arc Properties"),
                                        false, parent);
