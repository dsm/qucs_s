...
QucsFilter::QucsFilter() {
  ...
  connect(ComboRealize, SIGNAL(activated(int)),
          SLOT(slotRealizationChanged(int)));
  ...
  connect(ComboEr, SIGNAL(activated(const QString&)),
          SLOT(slotTakeEr(const QString&)));
...
QString* QucsFilter::calculateFilter(struct tFilter* Filter) {
  ...
  if (Filter->Type != TYPE_CAUER) {
    ...
    case CLASS_HIGHPASS:
      ...
      break;
    case CLASS_BANDPASS:
      F = new qf::cauer(amin, amax, fc + bw / 2, fs, r, bw, qf::BANDPASS,
                        is_tee);
      break;
    case CLASS_BANDSTOP:
      F = new qf::cauer(amin, amax, fc + bw / 2, fs, r, bw, qf::BANDSTOP,
                        is_tee);
      break;
    }
    if (F) {
