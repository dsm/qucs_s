...
struct Schematic::HealingParams {
  qucs_s::HealerParameters m_healer_params = {.allowWireReshaping = false,
                                              .allowWireRelaying  = false};
  qucs_s::wire::Planner::PlanType m_wire_plan =
      qucs_s::wire::Planner::PlanType::Straight;
};

const Schematic::HealingParams mousyMutationParams{
    .m_healer_params = {.allowWireReshaping =
                            _settings::Get().item<bool>("AllowFlexibleWires"),
                        .allowWireRelaying =
                            _settings::Get().item<bool>("AllowLayingWiresAnew"),
                        .wireRelayingDepth = 2}};
...
constexpr Schematic::HealingParams noninteractiveMutationParams{
    .m_healer_params = {.allowWireReshaping = false,
                        .allowWireRelaying  = false,
                        .wireRelayingDepth  = 3}};
...
/**
    ...
    rectangle.
*/
static bool shouldBeSelected(const QRect& elementBoundingRect,
                             const QRect& selectionRect, bool entirely) {
    ...
    for (auto* n2 : *nodes) {
      ...
      if (n1->center() == n2->center()) {
        qCritical() << "Two different nodes at the same location!"
                    << "node 1:" << n1 << "at" << n1->center() << "with"
                    << n1->conn_count() << "connections" << "node 2:" << n2
                    << "at" << n2->center() << "with" << n2->conn_count()
                    << "connections";
        is_ok = false;
      }
    }
...
// into two wires.
bool noNodesOnWires(const std::list<Node*>* nodes,
                    const std::list<Wire*>* wires) {
  bool is_ok = true;
  for (auto* w : *wires) {
    for (auto* n : *nodes) {
      if (qucs_s::geom::is_between(n, w->Port1, w->Port2)) {
        qCritical() << "A node lies on a wire withou splitting it!"
                    << "node:" << n << "at" << n->center() << "wire:" << w
                    << "from" << w->Port1->center() << "to"
                    << w->Port2->center();
  ...
  for (auto* wire : *wires) {
    ...
    for (auto* conn : *wire->Port1) {
      ...
      if (wire->Port2->is_connected(other_wire)) {
        qCritical() << "Duplicate wires found!" << "Nodes" << wire->Port1 << "("
                    << wire->Port1->center() << ")" << "and" << wire->Port2
                    << "(" << wire->Port2->center() << ")" << "connected with"
                    << wire << "and" << other_wire;
        is_ok = false;
      }
    }
...
bool noZeroLenWires(const std::list<Wire*>* wires) {
  ...
  for (auto* w : *wires) {
    if (w->Port1->center() == w->Port2->center()) {
      qCritical()
          << "A wire connecting two nodes at the same location is found!"
          << "wire: " << w << "node 1:" << w->Port1 << "at"
          << w->Port1->center() << "with" << w->Port1->conn_count()
          << "connections" << "node 2:" << w->Port2 << "at"
          << w->Port2->center() << "with" << w->Port2->conn_count()
          << "connections";
      is_ok = false;
    }
  }
...
bool allWiresAreConsistent(const std::list<Wire*>* wires) {
  ...
  for (auto* w : *wires) {
    if (w->Port1 == nullptr) {
      qCritical() << "Incostistent wire is found!" << "Port1 of" << w
                  << "is nullptr";
      is_ok = false;
    }

    if (w->Port2 == nullptr) {
      qCritical() << "Incostistent wire is found!" << "Port2 of" << w
                  << "is nullptr";
      is_ok = false;
    }

    if (w->Port1 != nullptr && w->Port2 != nullptr && w->Port1 == w->Port2) {
      qCritical() << "Incostistent wire is found!" << w << "has the same node"
                  << w->Port1 << "on both ports";
      is_ok = false;
    }

    if (!w->Port1->is_connected(w)) {
      qCritical() << "Incostistent wire is found!"
                  << "w->Port1->is_connected(w) == false" << "for" << w;
      is_ok = false;
    }

    if (!w->Port2->is_connected(w)) {
      qCritical() << "Incostistent wire is found!"
                  << "w->Port2->is_connected(w) == false" << "for" << w;
      is_ok = false;
    }
  }
  return is_ok;
    ...
    for (auto* p : c->Ports) {
      if (p->Connection == nullptr) {
        qCritical() << "Incostistent component is found!"
                    << "port->Connection == nullptr" << "for" << c;
        is_ok = false;
      }

      if (!p->Connection->is_connected(c)) {
        qCritical()
            << "Incostistent component is found!" << "Port of the component"
            << c << "located at" << (QPoint{p->x, p->y} + c->center())
            << "is connected to a node" << p->Connection << "located at"
            << p->Connection->center()
...
bool geometryIsInOrder(const std::list<Component*>* components,
                       const std::list<Wire*>* wires) {
  bool is_ok = true;
  for (auto* c : *components) {
    for (auto* p : c->Ports) {
  ...
  for (auto* w : *wires) {
    if (w->P1() != w->Port1->center()) {
      qCritical()
          << "Crooked geometry! Wire end and node locations don't match."
          << "Wire end located at" << w->P1() << "is connected to node at"
          << w->Port1->center();
      is_ok = false;
    }
    ...
    if (w->P2() != w->Port2->center()) {
      qCritical()
          << "Crooked geometry! Wire end and node locations don't match."
          << "Wire end located at" << w->P2() << "is connected to node at"
          << w->Port2->center();
      is_ok = false;
    }
  }
...
Wire* merge_wires_at_node(Node* node) {
  ...
  auto* dissapearing_wire =
      dynamic_cast<Wire*>(node->other_than(extended_wire));
  ...
  auto* extend_to = dissapearing_wire->Port1 == node ? dissapearing_wire->Port2
  ...
  if (!extended_wire->isSelected) {
    extended_wire->isSelected = dissapearing_wire->isSelected;
  }
...
Wire* Schematic::splitWire(Wire* source_wire, Node* splitter_node) {
  ...
  assert(qucs_s::geom::is_between(splitter_node, source_wire->P1(),
                                  source_wire->P2()));
  ...
  if (label != nullptr) {
    if (qucs_s::geom::is_between(label->root(), new_wire->P1(),
                                 new_wire->P2())) {
      new_wire->acquireLabel(std::move(label));
    } else if (qucs_s::geom::is_between(label->root(), source_wire->P1(),
                                        source_wire->P2())) {
      source_wire->acquireLabel(std::move(label));
    } else if (!splitter_node->hasLabel() &&
               label->root() == splitter_node->center()) {
...
Element* Schematic::selectElement(float fX, float fY, bool flag, int* index) {
    ...
    if (pl) {
      if (pl->getSelected(x, y)) {
        if (flag) {
          ...
          // TODO: I don't see a need for the xor here, a simple ! on the
          // current value would be clearer and have the same effect?
          pl->isSelected ^= flag;
          return pl;
        }
        if (pe_sel) {
  ...
  const double Corr =
      5.0 / a_Scale; // size of line select and area for resizing
  // test all diagrams
  for (Diagram* pd : *a_Diagrams) {
...
void Schematic::highlightWireLabels() {
  ...
  for (auto* wire : *a_Wires) {
    if (wire->hasLabel()) {
      wire->label()->setHighlighted(false);
    }
  }

  for (Node* node : *a_Nodes) {
    if (node->hasLabel()) {
      node->label()->setHighlighted(false);
    }
  }
    ...
    if (pltestouter) {
      if (pltestouter->isSelected) {
        ...
        for (Wire* pwinner : *a_Wires) {
          pltestinner =
              pwinner->label(); // test any label associated with the wire
          if (pltestinner) {
        ...
        for (auto* pninner : *a_Nodes) {
          pltestinner =
              pninner->label(); // test any label associated with the node
          if (pltestinner) {
            if (pltestouter->Name == pltestinner->Name) {
        ...
        // highlight if at least two different wires/nodes with the same label
        // found
...
void Schematic::deselectElements(Element* e) const {
  ...
  std::for_each(selection.components.begin(), selection.components.end(),
                deselector);
  std::for_each(selection.wires.begin(), selection.wires.end(), deselector);
  std::for_each(selection.paintings.begin(), selection.paintings.end(),
                deselector);
  std::for_each(selection.diagrams.begin(), selection.diagrams.end(),
                deselector);
...
// Selects elements that lie within or intersect with the rectangle
// selectionRect
int Schematic::selectElements(const QRect& selection_rect, bool append,
                              bool entirely) const {
  int selected_count = 0;

  auto select_element = [=](Element* e, const QRect& ebr) {
    // If an element lies within selection rect, it must be selected regardless
    // of any conditions
    if (shouldBeSelected(ebr, selection_rect, entirely)) {
      e->isSelected = true;
    }
    // If an element is not within selection rectangle, but it is already
    // selected and we're not appending to a list of selected items, then the
    // element must be deselected.
  ...
  for (Wire* wire : *a_Wires) {
    ...
    if (wire->hasLabel() &&
        select_element(wire->label(), wire->label()->boundingRect())) {
      selected_count++;
    }
  }
  ...
  for (Node* node : *a_Nodes) {
    if (node->hasLabel() &&
        select_element(node->label(), node->label()->boundingRect())) {
      selected_count++;
    }
  }
...
std::size_t total_count(const Schematic::Selection& selection) {
  return selection.components.size() + selection.wires.size() +
         selection.paintings.size() + selection.diagrams.size() +
         selection.labels.size() + selection.markers.size();
}
...
std::optional<QRect> total_br(const std::vector<T*> elems) {
  ...
  return std::make_optional(std::transform_reduce(
      elems.begin() + 1, elems.end(), elems.at(0)->boundingRect(),
...
public:
  ...
  bool align(Element* e) const {
    ...
    case 4: // center horizontally
      return e->moveCenter(
          m_bounds.center().x() - e->boundingRect().center().x(), 0);
    case 5: // center vertically
      return e->moveCenter(0, m_bounds.center().y() -
                                  e->boundingRect().center().y());
    default:
      assert(false);
      return false;
    }
...
bool Schematic::aligning(int mode) {
  auto selection = currentSelection();
  if (internal::total_count(selection) < 2) {
    return false;
  }

  const internal::Aligner aligner{mode, internal::total_br(selection).value()};
  bool any_aligned = false;
  const auto align = [&any_aligned, aligner](Element* e) {
    any_aligned = aligner.align(e) || any_aligned;
  };
  ...
  std::ranges::for_each(selection.components,
                        [&any_aligned, aligner](Component* c) {
                          any_aligned = aligner.align(c) || any_aligned;
                        });
...
public:
  bool distribute(std::vector<Element*>& distributedElements) {
    ...
    // Start distributing from second element and end on the one before last
    // element
    ...
    while (current != sentinel) {
      auto new_center = x_axis ? QPoint{next(), (*current)->center().y()}
...
bool Schematic::distributeHorizontal() {
  ...
  if (internal::total_count(selection) < 3) {
    return false;
  }
...
bool symbolElementsOnGrid(Schematic* sch,
                          const std::vector<Painting*>& paintings) {
  auto port = std::ranges::find_if(paintings, [](Painting* p) {
    return dynamic_cast<PortSymbol*>(p) != nullptr;
  });
...
bool Schematic::mirrorXComponents() {
  ...
  const auto axis =
      static_cast<int>(bounds->top() + std::round(bounds->height() / 2.0));
...
bool Schematic::mirrorYComponents() {
  ...
  const auto axis =
      static_cast<int>(bounds->left() + std::round(bounds->width() / 2.0));
...
void Schematic::setComponentNumber(Component* component) {
  ...
  if (component_number_property == nullptr ||
      component_number_property->Name != "Num") {
    return;
  }
  ...
  for (auto* other_component : *a_Components) {
    if (other_component->Model != component->Model) {
      continue;
    }
  ...
  if (used_numbers.find(component_number_property->Value.toInt()) ==
      used_numbers.end()) {
    return;
  }
...
void Schematic::insertComponentNodes(Component* component, bool noOptimize) {
  ...
  while (iport.hasNext()) {
    ...
    for (auto* connected : *component_port->Connection) {
      ...
      if (wire->Port1->is_connected(component) &&
          wire->Port2->is_connected(component)) {
...
void Schematic::recreateComponent(Component* comp) {
  ...
  for (auto pIt = comp->Ports.begin();
       pIt != comp->Ports.end() && !saved_labels.empty(); pIt++) {
...
bool Schematic::activateSpecifiedComponent(int x, int y) {
  auto componentIt =
      std::ranges::find_if(*a_Components, [x, y](const Component* pc) {
...
Component* Schematic::searchSelSubcircuit() {
  ...
  for (auto* pc : currentSelection().components) {
    ...
    if (sub != nullptr) {
      return nullptr; // more than one subcircuit selected
    }
    sub = pc;
  }
  return sub;
...
// Component is not deleted, pointer remains valid after call. It is
// responsibility of the caller to handle it by deleting, reinstalling, etc.
...
int Schematic::placeNodeLabel(WireLabel* pl) {
  auto node = std::ranges::find_if(
      *a_Nodes, [pl](const Node* n) { return n->center() == pl->root(); });
  if (node == a_Nodes->end()) {
    return -1;
  }
...
std::pair<bool, Node*> Schematic::connectWithWire(const QPoint& a,
                                                  const QPoint& b) noexcept {
  return connectWithWire(a, b, true, a_wirePlanner.planType());
}

std::pair<bool, Node*>
Schematic::connectWithWire(const QPoint& a, const QPoint& b, bool optimize,
...
Wire* find_wire(const Node* a, const Node* b, It begin, It end) {
  for (; begin != end; begin++) {
    Wire* w = *begin;
    if ((w->Port1 == a && w->Port2 == b) || (w->Port1 == b && w->Port2 == a)) {
      return w;
    }
  }
  return nullptr;
...
std::pair<bool, Node*> Schematic::installWire(Wire* wire) {
  ...
  // (C,D)]
  const auto crossed_node_pairs =
      internal::by_pairs<Node*, std::vector<Node*>>(crossed_nodes);
  ...
  for (const auto& node_pair : crossed_node_pairs) {
    auto* existing_wire = internal::find_wire(node_pair.first, node_pair.second,
                                              a_Wires->begin(), a_Wires->end());
  ...
  if (!has_been_used) {
    ...
    auto* existing_wire = internal::find_wire(last_pair.first, last_pair.second,
                                              a_Wires->begin(), a_Wires->end());
  ...
  const std::pair<bool, Node*> result{has_changes,
                                      crossed_node_pairs.back().second};
  ...
  for (const auto& node_pair : crossed_node_pairs) {
    ...
    auto* a_wire = internal::find_wire(node_pair.first, node_pair.second,
                                       a_Wires->begin(), a_Wires->end());

    if (qucs_s::geom::is_between(wire_label->root(), a_wire->P1(),
                                 a_wire->P2())) {
...
public:
  ...
  void moveNode(Node* node, const QPoint& p) override { node->moveCenterTo(p); }
...
void Schematic::displayMutations() {
  qucs_s::Healer healer{a_Components, a_Wires,
                        mousyMutationParams.m_healer_params};
...
bool Schematic::heal(const HealingParams* params) {
  ...
  // Remove wires connecting nodes at same location
  {
    ...
    std::ranges::copy_if(*a_Wires, std::back_inserter(zerolen_wires),
                         [](const Wire* w) -> bool {
                           return w->Port1->center() == w->Port2->center();
                         });
    std::ranges::for_each(zerolen_wires,
                          [this](Wire* w) -> void { deleteWire(w); });
  ...
  for (auto sameloc_node_group : internal::sameloc_nodes(a_Nodes)) {
    ...
    for (auto donor = (sameloc_node_group.begin() + 1);
         donor != sameloc_node_group.end(); donor++) {
  ...
  // Fix "node above wire" anomalies
  {
    for (auto* n : *a_Nodes) {
      for (auto wit = a_Wires->begin(); wit != a_Wires->end(); wit++) {
        auto w = *wit;
        if (qucs_s::geom::is_between(n, w->Port1, w->Port2) && n != w->Port1 &&
            n != w->Port2) {
          splitWire(w, n);
          wit              = a_Wires->begin();
          thereWereChanges = true;
