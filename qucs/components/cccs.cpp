...
CCCS::CCCS() {
  ...
  Lines.append(new qucs::Line(
      11, -7, 11, 6, QPen(Qt::darkBlue, 3, Qt::SolidLine, Qt::FlatCap)));
  Polylines.append(new qucs::Polyline(
      std::vector<QPointF>{{15, 1}, {11, 6}, {7, 1}},
  ...
  // smaller arrow "wings"
  Lines.append(new qucs::Line(
      -12, 20, -17, 11, QPen(Qt::darkBlue, 2, Qt::SolidLine, Qt::FlatCap)));
  Lines.append(new qucs::Line(
      -12, 20, -7, 11, QPen(Qt::darkBlue, 2, Qt::SolidLine, Qt::FlatCap)));
...
QString CCCS::va_code() {
  ...
  s +=
      QStringLiteral("%1  <+   %2 * 1e3 * %3 ;\n").arg(Ipm2).arg(Vpm).arg(Gain);

  return s;
}
QString CCCS::spice_netlist(
    spicecompat::SpiceDialect dialect /* = spicecompat::SPICEDefault */) {
  Q_UNUSED(dialect);

  QString s = spicecompat::check_refdes(
      Name, SpiceModel); // spice CCCS consists two sources: output source
