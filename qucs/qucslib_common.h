...
enum LIB_PARSE_WHAT { QUCS_COMP_LIB_HEADER_ONLY, QUCS_COMP_LIB_FULL };
...
inline bool getCompLineIntegers(const QString& s, int* i1 = 0, int* i2 = 0,
...
inline int makeModelString(QString libPath, QString compname,
                           QString compstring, QString& modelstring,
                           QString default_sym) {
  ...
  if (symbolSection.isEmpty()) { // component definition contains no symbol, use
                                 // library default
    if (default_sym.isEmpty()) { // library does not define a default symbol
      return QUCS_COMP_LIB_CORRUPT;
    }
  ...
  for (int i = 0; i < symbolstringLines.count(); i++) {
    ...
    if (symbolstringLines[i].isEmpty()) {
      continue;
    }
    ...
    symbolstringLines[i] =
        symbolstringLines[i].mid(1, symbolstringLines[i].length() - 2);
  ...
  modelstring = "<Lib " + Prefix + " 1 0 0 " + QString::number(Text_x) + " " +
                QString::number(Text_y) + " 0 0 \"" + libPath + "\" 0 \"" +
                compname + "\" 0>";
...
inline int parseQucsComponentLibrary(QString libPath, ComponentLibrary& library,
                                     LIB_PARSE_WHAT what = QUCS_COMP_LIB_FULL,
                                     bool relpath        = false) {
  ...
  while ((Start = LibraryString.indexOf("\n<Component ", Start)) > 0) {
    ...
    int result = makeModelString(libName, component.name, component.definition,
                                 component.modelString, library.defaultSymbol);
...
/*!
 * \brief parseSPICEComponentLibrary Parse SPICE component library as set of
 * subcircuits. \param filename[in] \param library[out] \return
 */
inline int parseSPICEComponentLibrary(QString libPath,
  ...
  QString defsym_filename = inf.canonicalPath() + QDir::separator() +
                            inf.baseName() + QDir::separator() + library.name +
                            ".sym";
    ...
    if (lin.toLower().startsWith(".subckt ")) {
      ...
      if (idx > 0) {
        if (lin.at(idx - 1).isSpace()) {
          idx--;
          while (lin.at(idx).isSpace()) {
            idx--;
          }
          while (lin.at(idx).isLetterOrNumber()) {
            idx--;
          }
        } else {
          idx = lin.lastIndexOf(QRegularExpression("[ \t]"), idx);
        }
        pars = lin.mid(idx);
      } else {
        pars = "";
      ...
      comp.modelString =
          QStringLiteral("<SpLib X1 1 280 260 -29 -164 0 0 \"%1\" 0 \"%2\" 1 "
                         "\"auto\" 1 \"%3\" 1>")
              .arg(filename)
              .arg(comp.name)
              .arg(pars);
      ...
      comp.definition += QStringLiteral("%1 device from %2 library")
                             .arg(comp.name)
                             .arg(library.name);
      comp.definition += "</Description>\n";
      ...
      comp.definition += "</Spice>\n";
      comp.definition +=
          "<Model>"; // Hack! It's needed to make Qucs to use SpiceLibComp
      ...
      QString sym_filename = inf.canonicalPath() + QDir::separator() +
                             inf.baseName() + QDir::separator() + comp.name +
                             ".sym";
      ...
      comp.modelString = QStringLiteral("<SpiceModel %1 1 250 290 -29 17 0 0")
                             .arg(m_str); // .MODEL start
      int lin_cnt     = 0;
      QString visible = "1";
      for (const auto& p : mod_lines) {
        if (lin_cnt > 3) {
          comp.modelString += QStringLiteral(" \"Line_%1=%2\" %3")
                                  .arg(lin_cnt + 1)
                                  .arg(p)
                                  .arg(visible);
        } else {
          comp.modelString += QStringLiteral(" \"%1\" %2").arg(p).arg(visible);
        }
        lin_cnt++;
        visible = "0";
      }
      ...
      comp.definition += "<Description>\n";
      comp.definition +=
          QStringLiteral("%1 model from %2 library\n"
                         "This component is model-only (.MODEL).\n"
                         "No subcircuit definition!\n"
                         "Use appropriate device to attach this model.")
              .arg(comp.name)
              .arg(library.name);
      comp.definition += "</Description>\n";
      comp.definition += "<Spice>\n";
...
inline QStringList getBlacklistedLibraries(QString dir) {
  ...
  case spicecompat::simXyce:
    filename = dir + QDir::separator() + "xyce.blacklist";
    break;
  case spicecompat::simNgspice:
  case spicecompat::simSpiceOpus:
    filename = dir + QDir::separator() + "ngspice.blacklist";
    break;
  default:
    break;
  }
  ...
  if (!f_blist.open(QIODevice::ReadOnly)) {
