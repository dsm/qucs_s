...
ExternSimDialog::ExternSimDialog(Schematic* sch, bool netlist2Console,
                                 bool netlist_mode)
    : QDialog(sch), a_schematic(sch),
      ...
      a_simStatusLog(new QListWidget), a_simProgress(new QProgressBar(this)),
      a_ngspice(new Ngspice(sch, this)), a_xyce(new Xyce(sch, this)),
      a_wasSimulated(true), a_hasError(false),
  ...
  connect(a_buttonSaveNetlist, SIGNAL(clicked()), this,
          SLOT(slotSaveNetlist()));
...
void ExternSimDialog::slotSetSimulator() {
  ...
  case spicecompat::simNgspice: {
    ...
    connect(a_ngspice, SIGNAL(errors(QProcess::ProcessError)), this,
            SLOT(slotNgspiceStartError(QProcess::ProcessError)));
    QString cmd;
    if (QFileInfo(QucsSettings.NgspiceExecutable)
            .isRelative()) { // this check is related to MacOS
      cmd = QFileInfo(QucsSettings.BinDir + QucsSettings.NgspiceExecutable)
                .absoluteFilePath();
    } else {
    ...
    if (QFileInfo::exists(cmd)) {
      ...
      a_ngspice->setSimulatorCmd(
          QucsSettings.NgspiceExecutable); // rely on $PATH
    }
    a_ngspice->setSimulatorParameters(
        _settings::Get().item<QString>("NgspiceParams"));
  } break;
  case spicecompat::simXyce: {
    ...
    connect(a_xyce, SIGNAL(errors(QProcess::ProcessError)), this,
            SLOT(slotNgspiceStartError(QProcess::ProcessError)));
    a_xyce->setSimulatorParameters(
        _settings::Get().item<QString>("XyceParams"));
  } break;
...
void ExternSimDialog::slotProcessOutput() {
  ...
  if (logContainsError(out)) {
    ...
    addLogEntry(tr("Simulation finished. Now place diagram on schematic to "
                   "plot the result."),
                QIcon(":/bitmaps/svg/ok_apply.svg"));
    emit warnings();
  } else {
    if (!a_hasError) {
      addLogEntry(tr("Simulation successful. Now place diagram on schematic to "
                     "plot the result."),
                  QIcon(":/bitmaps/svg/ok_apply.svg"));
      emit success();
    }
  ...
  if (!a_hasError) {
    ...
    // QString qucs_dataset =
    // inf.canonicalPath()+QDir::separator()+inf.baseName()+"_ngspice.dat";
    QString qucs_dataset =
        inf.canonicalPath() + QDir::separator() + inf.completeBaseName() + ext;
    switch (QucsSettings.DefaultSimulator) {
    case spicecompat::simNgspice:
    case spicecompat::simSpiceOpus:
...
void ExternSimDialog::slotNgspiceStarted() {
  ...
  addLogEntry(tr("Simulation started on: ") +
...
void ExternSimDialog::slotSaveNetlist() {
  ...
  if (!a_netlist2Console) {
    filename = QFileDialog::getSaveFileName(
        ...
        inf.path() + QDir::separator() + "netlist.cir", "All files (*)");
    if (filename.isEmpty()) {
      return;
    }
  }
  ...
  if (!a_netlist2Console && !QFile::exists(filename)) {
    QMessageBox::critical(nullptr, QObject::tr("Save netlist"),
...
bool ExternSimDialog::logContainsError(const QString& out) {
  ...
  switch (QucsSettings.DefaultSimulator) {
  case spicecompat::simNgspice:
    err_patterns << "Error:" << "ERROR" << "Error " << "Syntax error:"
                 << "Expression err:" << "errors:" << "simulation(s) aborted"
...
bool ExternSimDialog::logContainsWarning(const QString& out) {
  ...
  switch (QucsSettings.DefaultSimulator) {
  case spicecompat::simNgspice:
    warn_patterns << "Warning:" << "WARNING" << "Warning " << "warning:";
    break;
  case spicecompat::simXyce:
    warn_patterns << "Warning:" << "WARNING" << "Warning " << "warning:";
    break;
