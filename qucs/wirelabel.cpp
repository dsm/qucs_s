...
WireLabel::WireLabel(const QString& _Name, int _cx, int _cy, int _x1, int _y1) {
  cx = _cx;
  cy = _cy;
  x1 = _x1;
  y1 = _y1;
...
bool WireLabel::getSelected(int x, int y) {
  ...
  return QRect{text, textSize}.marginsAdded({5, 5, 5, 5}).contains(click) ||
...
void WireLabel::paint(QPainter* p) const {
  ...
  p->setPen(QPen{isHighlighted ? Qt::darkBlue : Qt::black,
  ...
  p->drawLine(cx, cy, right ? text_br.right() : text_br.left(),
              bottom ? text_br.bottom() : text_br.top());
  p->drawLine(right ? text_br.right() : text_br.left(),
              bottom ? text_br.bottom() : text_br.top(),
...
QString WireLabel::save() {
  ...
  s += QString::number(cx) + " " + QString::number(cy) + " " +
       QString::number(cx) + " " + QString::number(cy) + " \"" + Name + "\" " +
       QString::number(x1) + " " + QString::number(y1) + " 0 \"" + initValue +
       "\">";
  return s;
}

void WireLabel::getLabelBounding(int& _xmin, int& _ymin, int& _xmax,
                                 int& _ymax) {
...
bool WireLabel::mirrorX() noexcept {
  return moveCenterTo(
      center().x(), qucs_s::geom::mirror_coordinate(center().y(), root().y()));
}

bool WireLabel::mirrorY() noexcept {
  return moveCenterTo(qucs_s::geom::mirror_coordinate(center().x(), root().x()),
                      center().y());
}

QRect WireLabel::boundingRect() const noexcept {
  return QRect{QPoint{cx, cy}, QPoint{x1, y1}}.normalized().united(
      QRect{{x1, y1}, textSize}.normalized());
}
