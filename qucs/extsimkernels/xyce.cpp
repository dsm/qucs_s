...
Xyce::Xyce(Schematic* schematic, QObject* parent)
    : AbstractSpiceKernel(schematic, parent), a_Noisesim(false),
      a_simulationsQueue(), a_netlistQueue() {
...
void Xyce::determineUsedSimulations(QStringList* sim_lst) {
  ...
  for (Component* pc : a_schematic->a_DocComps) {
    if (pc->isSimulation && pc->isActive == COMP_IS_ACTIVE) {
      ...
      if (sim_typ == ".AC") {
        a_simulationsQueue.append("ac");
      }
      if (sim_typ == ".NOISE") {
        a_simulationsQueue.append("noise");
      }
      if (sim_typ == ".TR") {
        a_simulationsQueue.append("tran");
      }
      if (sim_typ == ".HB") {
        a_simulationsQueue.append("hb");
      }
      if (sim_typ == ".SP") {
        a_simulationsQueue.append("sp");
      }
      if (sim_typ == ".SENS_XYCE") {
        a_simulationsQueue.append("sens");
      }
      if (sim_typ == ".SENS_TR_XYCE") {
        a_simulationsQueue.append("sens_tr");
      }
      if (sim_typ == ".XYCESCR") {
        a_simulationsQueue.append(pc->Name); // May be >= XYCE scripts
      }
 ...
 * \param[in] simulations The list of simulations that need to included in
 * netlist. \param[out] vars The list of output variables and node names.
 * \param[out] outputs The list of spice output raw text files.
 */
void Xyce::createNetlist(QTextStream& stream, QStringList& simulations,
  ...
  if (a_DC_OP_only) {
    ...
    stream
        << QStringLiteral(
               ".PRINT dc format=noindex file=spice4qucs.cir.dc_op_xyce %1\n")
               .arg(nods);
  ...
  for (Component* pc : a_schematic->a_DocComps) { // Xyce can run
    if (pc->isSimulation &&
        pc->isActive == COMP_IS_ACTIVE) { // only one simulations per time.
      QString sim_typ = pc->Model;        // Multiple simulations are forbidden.
      QString s       = pc->getSpiceNetlist(spicecompat::SPICEXyce);
      if ((sim_typ == ".AC") && (sim == "ac")) {
        stream << s;
      }
      if ((sim_typ == ".NOISE") && (sim == "noise")) {
        stream << s;
      }
      if ((sim_typ == ".SENS_XYCE") && (sim == "sens")) {
        stream << s;
      }
      if ((sim_typ == ".SENS_TR_XYCE") && (sim == "sens_tr")) {
        stream << s;
      }
      ...
      if ((sim_typ == ".HB") && (sim == "hb")) {
        stream << s;
      }
      if (sim_typ == ".SW") {
        QString SwpSim = pc->Props.at(0)->Value;
        if (SwpSim.startsWith("DC") && (sim == "dc")) {
          stream << s;
          ...
          for (Component* pc1 : a_schematic->a_DocComps) {
            if ((pc1->Name == SwpSim) &&
                (pc1->Props.at(0)->Value.startsWith("DC"))) {
              stream << s;
              hasParSweep = true;
            }
          }
  ...
  QString filename;
  if (hasParSweep) {
    filename = QStringLiteral("%1.%2._swp.plot").arg(basenam).arg(sim);
  } else {
    filename = QStringLiteral("%1.%2.plot").arg(basenam).arg(sim);
  }
  filename.remove(
      QRegularExpression("\\s")); // XYCE don't support spaces and quotes
  QString write_str;
  if (sim == "hb") {
    // write_str = QStringLiteral(".PRINT  %1 file=%2
    ...
    filename += "_std";
    write_str =
        QStringLiteral(".PRINT noise file=%1 inoise onoise\n").arg(filename);
    ...
    if (hasParSweep) {
      for (const auto& v : spar_vars) { // Bug in Xyce; cannot print Z-par if
        ...
        if (!v.startsWith("z(")) {
          write_str += QStringLiteral("%1 ").arg(v);
        }
      }
    } else {
...
void Xyce::slotSimulate() {
  ...
  for (const QString& sim : a_simulationsQueue) {
    ...
    sim_lst.append(sim);
    QString tmp_path =
        QDir::toNativeSeparators(a_workdir + "/spice4qucs." + sim + ".cir");
...
void Xyce::slotProcessOutput() {
  ...
  if (s.contains("Percent complete:")) {
    int percent =
        round(s.section(' ', 3, 3, QString::SectionSkipEmpty).toFloat());
