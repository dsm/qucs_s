...
template <> struct std::less<QPoint> {
...
public:
  MovePort(GenericPort* port, const QPoint& coords)
      : m_port{port}, m_coords{coords} {}
  void execute(SchematicMutator* mutator) override {
    mutator->movePort(m_port, m_coords);
  }
...
public:
  ReattachLabel(WireLabel* label, Node* new_host)
      : m_label{label}, m_dest{new_host} {}
  void execute(SchematicMutator* mutator) override {
    mutator->putLabel(m_label, m_dest);
  }
...
public:
  ConnectWithWire(const QPoint& a, const QPoint& b)
      : m_point_A{a}, m_point_B{b} {}
  void execute(SchematicMutator* mutator) override {
    mutator->connectWithWire(m_point_A, m_point_B);
  }
...
Node* GenericPort::replaceNodeWith(Node* new_node) {
  switch (m_portType) {
  case PortType::Component: {
    if (new_node == m_port->Connection) {
      return new_node;
    }

    auto* old_node = m_port->Connection;
    if (old_node) {
      old_node->disconnect(m_comp);
    }
  ...
  case PortType::WireTwo: {
    ...
    // Ensure that only this port is connected to the new node
    if (m_wire->Port1 == new_node) {
      m_wire->Port1 = nullptr;
    }
    return old_node;
  }
  }
...
bool hasMismatchedPorts(const Node* node,
                        const vector<shared_ptr<GenericPort>>& ports) {
  return std::ranges::any_of(ports, [node](const auto& port) {
...
public:
  ...
  bool onlyWirePortsAt(const QPoint& loc) const {
    return std::none_of(
        m_port_locations.lower_bound(loc), m_port_locations.upper_bound(loc),
        [](const auto& loc_port) { return loc_port.second->isOfComponent(); });
  }

  bool isOK() const {
    return m_unique_locations.size() == 1 &&
           m_node->center() == *m_unique_locations.begin();
  }
  ...
  bool onlyTwoPortClusters() const { return m_unique_locations.size() == 2; }

  const std::multimap<QPoint, GenericPort*>& portLocations() const {
    return m_port_locations;
  }

  const std::set<QPoint>& uniqueLocations() const { return m_unique_locations; }
};
...
bool isSpecialCase(const JointStateAssessor& jsa) {
  if (!jsa.onlyTwoPortClusters()) {
    return false;
  }

  auto single_wire_port_loc =
      std::ranges::find_if(jsa.uniqueLocations(), [&jsa](auto& loc) {
        return jsa.portLocations().count(loc) == 1 && jsa.onlyWirePortsAt(loc);
      });

  if (single_wire_port_loc == jsa.uniqueLocations().end()) {
    return false;
  }

  const auto other_loc = std::ranges::find_if(
      jsa.uniqueLocations(), [single_wire_port_loc](auto loc) {
        return loc != *single_wire_port_loc;
      });
  assert(other_loc != jsa.uniqueLocations().end());

  if (jsa.onlyWirePortsAt(*other_loc)) {
    return false;
  }

  Wire* single_wire = jsa.portLocations()
                          .lower_bound(*single_wire_port_loc)
                          ->second->hostWire();
  assert(single_wire != nullptr);

  const QPoint p1 = single_wire->P1();
  const QPoint p2 = single_wire->P2();
  return *other_loc == p1 || *other_loc == p2 ||
         geom::is_between(*other_loc, p1, p2);
}
...
class Healer::HealerImpl {
  ...
  vector<Healer::HealingAction>
  processSpecialCase(Node* node, const JointStateAssessor& jsa) const;
  vector<Healer::HealingAction>
  processReshapingCase(Node* node, const JointStateAssessor& jsa) const;
  vector<Healer::HealingAction>
  processRelayingCase(Node* node, const JointStateAssessor& jsa) const;
  vector<Healer::HealingAction>
  processGenericCase(Node* node, const JointStateAssessor& jsa) const;

  std::pair<Node*, std::vector<Wire*>> findStableNode(Node* begin,
                                                      Wire* go) const;
  void stepBackOnMismatch(std::vector<Node*>& passed_nodes,
                          std::vector<Wire*>& passed_wires) const;

public:
  HealerImpl(const std::list<Component*>* components,
             const std::list<Wire*>* wires, const HealerParameters& hp);
  std::vector<Healer::HealingAction> planHealing() const;
};

Healer::HealerImpl::HealerImpl(const std::list<Component*>* components,
                               const std::list<Wire*>* wires,
                               const HealerParameters& hp)
    : m_params{hp} {
  for (auto* comp : *components) {
    for (auto* port : comp->Ports) {
      m_port_groups[port->Connection].push_back(
          std::make_unique<GenericPort>(port, comp));
    }
  }
  for (auto* wire : *wires) {
    m_port_groups[wire->Port1].push_back(
        std::make_unique<GenericPort>(wire, GenericPort::WirePort::One));
    m_port_groups[wire->Port2].push_back(
...
vector<Healer::HealingAction> Healer::HealerImpl::planHealing() const {
  ...
  for (const auto& [node, port_group] : m_port_groups) {
    ...
    if (joint_state.isOnlyNodeMisplaced()) {
      healing_actions.push_back(
          make_unique<MoveNode>(node, port_group.front()->center()));
  ...
  std::ranges::sort(healing_plan, [](const auto& lhs, const auto& rhs) {
...
vector<Healer::HealingAction>
Healer::HealerImpl::processSpecialCase(Node* node,
                                       const JointStateAssessor& jsa) const {
  ...
  const auto other_loc =
      std::ranges::find_if(jsa.uniqueLocations(), [single_wire_port](auto loc) {
        return loc != single_wire_port->center();
      });
  assert(other_loc != jsa.uniqueLocations().end());
...
vector<Healer::HealingAction>
Healer::HealerImpl::processReshapingCase(Node* node,
                                         const JointStateAssessor& jsa) const {
  assert(jsa.uniqueLocations().size() == 2);

  const auto other_loc =
      std::ranges::find_if(jsa.uniqueLocations(),
                           [node](auto loc) { return loc != node->center(); });

  vector<HealingAction> actions;
  actions.push_back(make_unique<MoveNode>(node, *other_loc));

  for (auto port : m_port_groups.at(node)) {
    if (port->center() == *other_loc) {
      continue;
    }
...
vector<Healer::HealingAction> Healer::HealerImpl::processRelayingCase(
    Node* node, const JointStateAssessor& /*jsa*/) const {
  vector<HealingAction> actions;
  ...
  for (auto port : m_port_groups.at(node)) {
    ...
    for (auto* wire : obsolete_wires) {
      if (wire->hasLabel()) {
        labels.emplace_back(wire->releaseLabel());
      }
      if (wire->Port1->hasLabel()) {
        labels.emplace_back(wire->Port1->releaseLabel());
      }
      if (wire->Port2->hasLabel()) {
        labels.emplace_back(wire->Port2->releaseLabel());
      }
    }
    ...
    if (!labels.empty() && !stable_node->hasLabel()) {
      actions.push_back(
          make_unique<ReattachLabel>(labels.front().release(), stable_node));
    }

    for (const auto& port : m_port_groups.at(node)) {
      if (port->center() != node->center() &&
          port->center() != stable_node->center()) {
        actions.push_back(make_unique<ConnectWithWire>(port->center(),
                                                       stable_node->center()));
      }
    }
...
vector<Healer::HealingAction>
Healer::HealerImpl::processGenericCase(Node* node,
                                       const JointStateAssessor& jsa) const {
...
std::pair<Node*, vector<Wire*>>
Healer::HealerImpl::findStableNode(Node* begin, Wire* go) const {
  if (m_params.wireRelayingDepth == 0) {
    return {begin, {}};
  };

  std::size_t max_depth =
      m_params.wireRelayingDepth < 0
  ...
  while (current_node->conn_count() == 2) {
    ...
    if (::qucs_s::hasMismatchedPorts(current_node,
                                     m_port_groups.at(current_node))) {
      is_mismatch = true;
      break;
    }
    ...
    current_node =
        next_wire->Port1 == current_node ? next_wire->Port2 : next_wire->Port1;
...
void Healer::HealerImpl::stepBackOnMismatch(vector<Node*>& passed_nodes,
                                            vector<Wire*>& passed_wires) const {
  assert(hasMismatchedPorts(passed_nodes.front(),
                            m_port_groups.at(passed_nodes.front())));
  assert(hasMismatchedPorts(passed_nodes.back(),
                            m_port_groups.at(passed_nodes.back())));
...
Healer::Healer(const std::list<Component*>* components,
               const std::list<Wire*>* wires, const HealerParameters& hp)
