...
QucsActiveFilter::QucsActiveFilter(QWidget* parent) : QMainWindow(parent) {
  Nfil = 4;
  Fc   = 1000;
  ...
  viewConsole->setStatusTip(
      tr("Enables/disables the filter calculation console"));
  viewConsole->setWhatsThis(
  ...
  QStringList lst_2;
  lst_2 << tr("Butterworth") << tr("Chebyshev") << tr("Inverse Chebyshev")
        << tr("Cauer (Elliptic)") << tr("Bessel") << tr("Legendre")
        << tr("User defined");
  cbxFilterFunc->addItems(lst_2);
  connect(cbxFilterFunc, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotSwitchParameters()));

  btnDefineTransferFunc =
      new QPushButton(tr("Manually define transfer function"));
  btnDefineTransferFunc->setEnabled(false);
  connect(btnDefineTransferFunc, SIGNAL(clicked()), this,
          SLOT(slotDefineTransferFunc()));
  ...
  QStringList lst_3;
  lst_3 << tr("Low Pass") << tr("High Pass") << tr("Band Pass")
        << tr("Band Stop");
  cbxResponse->addItems(lst_3);
  connect(cbxResponse, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotUpdateResponse()));
  connect(cbxResponse, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotUpdateSchematic()));
  connect(cbxResponse, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotSetLabels()));
  connect(cbxResponse, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotSwitchParameters()));
  ...
  QStringList lst_1;
  lst_1 << tr("Multifeedback (MFB)") << tr("Sallen-Key (S-K)")
        << tr("Cauer section");
  //<<tr("Пассивный");
  cbxFilterType->addItems(lst_1);
  connect(cbxFilterType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotUpdateSchematic()));
  ...
  QGroupBox* gpbAFR =
      new QGroupBox(tr("General filter amplitude-frequency response"));
...
void QucsActiveFilter::slotCalcSchematic() {
  ...
  if ((cbxResponse->currentIndex() == tLowPass) ||
    ...
    if (par.Fl > par.Fu) {
      errorMessage(
          tr("Upper cutoff frequency of band-pass/band-stop filter is\n"
  ...
  case topoCauer: {
    if (((ffunc == Filter::InvChebyshev) || (ffunc == Filter::Cauer) ||
      ...
      if (ok) {
        ...
        errorMessage(
            tr("Unable to implement filter with such parameters and topology \n"
               "Change parameters and/or topology and try again!"));
      }
    } else {
      errorMessage(
          tr("Unable to use Cauer section for Chebyshev or Butterworth \n"
  ...
  case topoMFB: {
    if (!((ffunc == Filter::InvChebyshev) || (ffunc == Filter::Cauer))) {
      ...
      errorMessage(
          tr("Unable to use MFB filter for Cauer or Inverse Chebyshev \n"
             "frequency response. Try to use another topology."));
    }
  } break;
  case topoSallenKey: {
  ...
  if (ok) {
    ...
    statusBar()->showMessage(tr("Filter calculation terminated with error!"),
                             2000);
    txtResult->appendHtml("<pre>\r\n" +
...
void QucsActiveFilter::errorMessage(QString str) {
  ...
  QMessageBox* msg = new QMessageBox(
      QMessageBox::Critical, tr("Active filter design"), str, QMessageBox::Ok);
  msg->exec();
  delete msg;
}
