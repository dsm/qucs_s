...
ID_Dialog::ID_Dialog(ID_Text* idText_, QWidget* parent) : QDialog(parent) {
  idText = idText_;
  ...
  ParamTable->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  ParamTable->horizontalHeader()->setSectionsClickable(
  ...
  ParamTable->setHorizontalHeaderLabels(
      QStringList() << tr("display") << tr("Name") << tr("Default")
  ...
  connect(ParamTable, SIGNAL(currentCellChanged(int, int, int, int)),
          SLOT(slotEditParameter()));
...
void ID_Dialog::slotRemoveParameter() {
  ...
  int nextRow =
      (selectedrow == ParamTable->rowCount()) ? selectedrow - 1 : selectedrow;
...
void ID_Dialog::slotOk() {
  ...
  for (int row = 0; row < ParamTable->rowCount(); ++row) {
    ...
    QString name(ParamTable->item(row, 1)->text() + "=" +
                 ParamTable->item(row, 2)->text());
    QString desc(ParamTable->item(row, 3)->text());
    QString type(ParamTable->item(row, 4)->text());

    scratch.push_back(
        std::make_unique<SubParameter>(display, name, desc, type));
  }
  ...
  if (scratch.size() != idText->subParameters.size()) {
    ...
    for (std::size_t i = 0; !changed && i < scratch.size(); i++) {
      changed =
          changed || scratch[i]->display != idText->subParameters[i]->display ||
