...
int Component::textSize(int& textPropertyMaxWidth,
                        int& totalTextPropertiesHeight) const {
...
QRect Component::boundingRect() const noexcept {
  return QRect{x1, y1, x2 - x1, y2 - y1}.normalized().translated(center());
}
...
int Component::getTextSelected(int point_x, int point_y) {
  ...
  for (auto* prop : Props) {
    ...
    QRect text_br{{tx, bounding_rect_top},
                  font_metrics.size(flags, prop->Name + "=" + prop->Value)};
    if (text_br.contains(click)) {
      return text_index;
    }
...
void Component::paint(QPainter* p) {
  ...
  for (auto* prop : Props) {
    ...
    if ((prop->simulators & QucsSettings.DefaultSimulator) !=
...
void Component::drawSymbol(QPainter* p) {
  const bool correctSimulator = (Simulator & QucsSettings.DefaultSimulator) ==
                                QucsSettings.DefaultSimulator;
...
void Component::paintScheme(Schematic* p) {
  ...
  if (Model.at(0) == '.' ||
      isEquation) { // is simulation component (dc, ac, ...) + Equations
  ...
  for (qucs::Line* p1 : Lines) {
    p->PostPaintEvent(_Line, cx + p1->x1, cy + p1->y1, cx + p1->x2,
                      cy + p1->y2);
  }
  ...
  for (auto* pl : Polylines) {
    for (size_t i = 1; i < pl->points.size(); i++) {
      ...
      p->PostPaintEvent(_Line, cx + prev.x(), cy + prev.y(), cx + curr.x(),
                        cy + curr.y());
    }
  }
  ...
  for (qucs::Arc* p3 : Arcs) { // paint all arcs
    p->PostPaintEvent(_Arc, cx + p3->x, cy + p3->y, p3->w, p3->h, p3->angle,
                      p3->arclen);
  }
...
bool Component::rotate() noexcept {
  ...
  if ((Model != "Sub") && (Model != "VHDL") && (Model != "Verilog") &&
    ...
    if (Ports.count() < 1) {
      return false; // do not rotate components without ports
    }
  }
  int tmp, dx, dy;
...
bool Component::mirrorX() noexcept {
  ...
  for (qucs::Arc* p3 : Arcs) {
    ...
    if (p3->angle < 0) {
      p3->angle += 16 * 360; // angle has to be > 0
    }
  }
...
//                   list format is: ( qucs_parameter_name<i> ,
//                   spice_parameter_name<i>, ... )

QString Component::form_spice_param_list(QStringList& ignore_list,
                                         QStringList& convert_list) {
  QString par_str = "";
...
QString Component::getNetlist() {
  ...
  QString s     = "";
  while (iport.hasNext()) {
    s += "R:" + Name + "." + QString::number(z++) + " " + Node1 + " " +
         iport.next()->Connection->Name + " R=\"0\"\n";
  }
  return s;
}

QString Component::getSpiceNetlist(
    spicecompat::SpiceDialect dialect /* = SPICEDefault */) {
  QString s;
  switch (isActive) {
  case COMP_IS_ACTIVE:
  ...
  while (iport.hasNext()) { // Add minR resistors
    s += "R" + Name + QString::number(z++) + " " + Node1 + " " +
         iport.next()->Connection->Name + " 1e-12\n";
  }
...
bool Component::load(const QString& _s) {
  ...
  if (Model == "Sub") {
    ...
    tmp = counts + 1; // "+1" because "counts" could be zero
  }
  ...
  for (auto p1 = Props.begin(); p1 != Props.end(); ++p1) {
    ...
    if (z > counts) {
      ...
      if (Model == "Diode") {
        if (counts < 56) { // backward compatible
          ...
          p1 = Props.begin();
          std::advance(
              p1, std::min<int>(counts - 1, std::distance(p1, Props.end())));
          for (; p1 != Props.begin();) {
            if (counts-- < 19) {
              break;
            }
        ...
        if (counts < 10) { // backward compatible
          ...
          p1 = Props.begin();
          std::advance(p1,
                       std::min<int>(counts, std::distance(p1, Props.end())));
          for (; p1 != Props.begin();) {
            if (counts-- < 4) {
              break;
            }
    ...
    if (Model != "EDD" && Model != "RFEDD" && Model != "RFEDD2P") {
      if ((*p1)->Description.isEmpty() ||
          (*p1)->Description ==
              "Expression") { // unknown number of properties ?
...
int Component::analyseLine(const QString& Row, int numProps) {
  ...
  if ((s == "PortSym") || (s == ".PortSym")) { // backward compatible
    ...
    if (i1 < x1) {
      x1 = i1; // keep track of component boundings
    }
    if (i1 > x2) {
      x2 = i1;
    }
    ...
    if (!getIntegers(Row, &i1, &i2, &i3, &i4)) {
      return -1;
    }
    if (!getPen(Row, Pen, 5)) {
      return -1;
    ...
    auto pp = Props.begin();
    std::advance(pp,
                 std::min<int>((numProps - 1), std::distance(pp, Props.end())));
    for (;;) {
      s = Row.section('"', i1, i1);
      if (s.isEmpty()) {
        break;
    ...
    if (i5 < x1) {
      x1 = i5; // keep track of component boundings
    }
    if (i5 > x2) {
      x2 = i5;
    }
    ...
    if (!getIntegers(Row, &i1, &i2, &i3, 0, &i4)) {
      return -1;
    }
    ...
    Texts.append(new Text(i1, i2, s, Color,
                          static_cast<double>(QFontInfo{Font}.pixelSize()),
...
GateComponent::GateComponent() {
  ...
  Props.append(
      new Property("in", "2", false, QObject::tr("number of input ports")));
  Props.append(
      new Property("V", "1 V", false, QObject::tr("voltage of high level")));
  Props.append(new Property("t", "1 ns", false, QObject::tr("delay time")));
  Props.append(new Property("TR", "10", false,
                            QObject::tr("transfer function scaling factor")));

  // this must be the last property in the list !!!
  Props.append(
      new Property("Symbol", "old", false,
...
QString GateComponent::spice_netlist(spicecompat::SpiceDialect dialect) {
  ...
  s += " " + tmp_model + "\n";
  s += QStringLiteral(
           ".model %1 %2(rise_delay=%3 fall_delay=%3 input_load=5e-13)\n")
           .arg(tmp_model)
           .arg(type)
           .arg(td);
  return s;
...
void GateComponent::createSymbol() {
  ...
  if (Props.back()->Value.at(0) == 'D') { // DIN symbol
    ...
    if (Model.at(z) == 'O') {
      Lines.append(
          new qucs::Line(-11, 6 - y, -6, 9 - y, QPen(Qt::darkBlue, 0)));
      Lines.append(
          new qucs::Line(-11, 12 - y, -6, 9 - y, QPen(Qt::darkBlue, 0)));
      Lines.append(
          new qucs::Line(-11, 14 - y, -6, 14 - y, QPen(Qt::darkBlue, 0)));
      Lines.append(
          new qucs::Line(-11, 16 - y, -6, 16 - y, QPen(Qt::darkBlue, 0)));
      ...
      if (Model.at(1) == 'N') {
        Ellipses.append(new qucs::Ellips(xr, -4, 8, 8, QPen(Qt::darkBlue, 0),
                                         QBrush(Qt::darkBlue)));
    ...
    Arcs.append(
        new qucs::Arc(-30, -y, 40, 30, 0, 16 * 90, QPen(Qt::darkBlue, 2)));
    Arcs.append(
        new qucs::Arc(-30, y - 30, 40, 30, 0, -16 * 90, QPen(Qt::darkBlue, 2)));
    ...
    if (Model.at(0) == 'X') {
      ...
      if (Model.at(1) == 'N') {
        ...
        Arcs.append(
            new qucs::Arc(-5, -5, 10, 10, 0, 16 * 360, QPen(Qt::darkBlue, 1)));
...
Component* getComponentFromName(QString& Line, Schematic* p) {
  ...
  if (!c) {
    ...
    if (QucsMain != nullptr) {
      QMessageBox* msg =
          new QMessageBox(QMessageBox::Warning, QObject::tr("Warning"),
                          QObject::tr("Format Error:\nUnknown component!\n"
                                      "%1\n\n"
      ...
      QString err_msg =
          QStringLiteral("Schematic loading error! Unknown device %1")
              .arg(cstr);
      qCritical() << err_msg;
      return 0;
    }
