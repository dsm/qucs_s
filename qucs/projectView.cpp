...
void ProjectView::setProjPath(const QString& path) {
  ...
  if (m_valid) {
    ...
    if (m_projName.endsWith("_prj")) {
      ...
      qWarning() << "ProjectView::setProjPath() : path does not end in '_prj' ("
                 << m_projName << ")";
    }
  }
  refresh();
...
void ProjectView::refresh() {
  ...
  QStringList files =
      workPath.entryList(QStringList() << "*", QDir::Files, QDir::Name);
  ...
  for (it = files.begin(); it != files.end(); ++it) {
    ...
    fullExtName =
        QFileInfo(workPath.filePath(fileName)).completeSuffix().toLower();
    ...
    if (extName == "dat" || fullExtName == "dat.ngspice" ||
      ...
      if (n >= 0) {
        if (n > 0) { // is a subcircuit
          columnData.append(
              new QStandardItem(QString::number(n) + tr("-port")));
        }
        appendChild(6, columnData);
      }
    } else if (extName == "sym") {
      appendChild(7, columnData);
    } else if ((extName == "cir") || (extName == "ckt") || (extName == "sp")) {
