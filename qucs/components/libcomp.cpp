...
LibComp::LibComp() {
  ...
  Props.append(
      new Property("Lib", "", true, QObject::tr("name of qucs library file")));
...
int LibComp::loadSection(const QString& Name, QString& Section,
  ...
  QFile file(misc::properAbsFileName(
      Directory.absoluteFilePath(Props.at(0)->Value + ".lib"),
      containingSchematic));
  ...
  if (Start < 0) {
    return -4; // component not found
  }
  Start = Section.indexOf('\n', Start);
  if (Start < 0) {
    return -5; // file corrupt
  }
  Start++;
  End = Section.indexOf("\n</Component>", Start);
  if (End < 0) {
    return -6; // file corrupt
  }
  ...
  if (Includes) {
    ...
    if (StartI >= 0) { // includes found
      ...
      if (StartI < 0) {
        return -10; // file corrupt
      }
      EndI = Section.indexOf('>', StartI);
      if (EndI < 0) {
        return -11; // file corrupt
      }
      StartI++;
      EndI--;
  ...
  if (Start < 0) {
    if ((Name == "Symbol") && (!libDefaultSymbol.isEmpty())) {
      // component does not define its own symbol but the library defines a
      // default symbol
...
int LibComp::loadSymbol() {
  ...
  while (!stream.atEnd()) {
    ...
    if (Result < 0) {
      return -13; // line format error
    }
    z += Result;
  }

  x1 -= 4;
  x2 += 4; // enlarge component boundings a little
  y1 -= 4;
  y2 += 4;
...
QString LibComp::getSubcircuitFile() {
  QDir Directory(QucsSettings.LibDir);
  QString FileName = misc::properAbsFileName(
      Directory.absoluteFilePath(Props.first()->Value) + ".lib");
  FileName.chop(4);
  return FileName;
}
...
bool LibComp::createSubNetlist(QTextStream* stream, QStringList& FileList,
  ...
  // also include files
  int error = 0;
  for (QStringList::Iterator it = Includes.begin(); it != Includes.end();
       ++it) {
    QString s = getSubcircuitFile() + "/" + *it;
    if (FileList.indexOf(s) >= 0) {
      continue;
    }
    FileList.append(s);
...
QString LibComp::spice_netlist(
    spicecompat::SpiceDialect dialect /* = spicecompat::SPICEDefault */) {
  Q_UNUSED(dialect);

  QString s =
      SpiceModel + Name + " 0 "; // connect ground of subckt to circuit ground
  for (Port* p1 : Ports) {
    s += " " + p1->Connection->Name; // node names
  }
  s += " " + createType();
