...
SpiceDialog::SpiceDialog(QucsApp* App_, SpiceFile* c, Schematic* d)
  ...
  PrepCombo->insertItems(0, QStringList() << "none" << "ps2sp" << "spicepp"
                                          << "spiceprm");
...
void SpiceDialog::slotButtBrowse() {
  ...
  QString s = QFileDialog::getOpenFileName(
      ...
      tr("SPICE netlist") + QStringLiteral(" (") +
...
bool SpiceDialog::loadSpiceNetList(const QString& s) {
  ...
  if (preprocessor != "none") {
    ...
    if (!piping) {
      ...
      connect(SpicePrep, SIGNAL(readyReadStandardOutput()),
              SLOT(slotSkipOut()));
      connect(SpicePrep, SIGNAL(readyReadStandardError()),
              SLOT(slotGetPrepErr()));
    } else {
      connect(SpicePrep, SIGNAL(readyReadStandardOutput()),
              SLOT(slotGetPrepOut()));
    ...
    QMessageBox* MBox = new QMessageBox(
        QMessageBox::NoIcon, tr("Info"),
        tr("Preprocessing SPICE file \"%1\".").arg(FileInfo.filePath()),
        QMessageBox::Abort, this);

    connect(SpicePrep, SIGNAL(finished(int, QProcess::ExitStatus)), MBox,
            SLOT(close()));
    ...
    if (piping) {
      ...
      if (!PrepFile.open(QIODevice::WriteOnly)) {
        QMessageBox::critical(
            this, tr("Error"),
            tr("Cannot save preprocessed SPICE file \"%1\".").arg(PrepName));
        return false;
      }
  ...
  if (QucsSettings.DefaultSimulator == spicecompat::simQucsator) {
    ...
    Arguments << "-if" << "spice" << "-of" << "qucs" << "-i"
              << FileInfo.filePath();

    qDebug() << "Command :" << Program << Arguments.join(" ");

    connect(QucsConv, SIGNAL(readyReadStandardOutput()),
            SLOT(slotGetNetlist()));
    ...
    connect(QucsConv, SIGNAL(finished(int, QProcess::ExitStatus)), MBox,
            SLOT(close()));
  ...
  for (int i = 0; i < PortsList->count(); i++) {
    ...
    plist =
        NodesList->findItems(tmp, Qt::MatchCaseSensitive | Qt::MatchExactly);
    if (!plist.isEmpty()) {
      delete plist[0];
    } else {
